...
The dump the research doc quotes is `exports/orphan_backref_probe.txt` (and
`exports/bill/orphan_backref_probe.txt` for the vendor side). Like the cycle
harnesses, this probe writes there only when the writing switch is on; an
ordinary run puts the dump in a scratch directory.

Run via:

    ./scripts/test.sh latest tests/research/test_orphan_detection_probe.py
...
from tests.research.test_post_pay_unpost_cycle import exports_dir
...
def test_orphan_backreference_probe(tmp_path):
    ...
    dump_path = os.path.join(exports_dir(tmp_path), "orphan_backref_probe.txt")
    pre_record = {}
    post_record = {}
    ...
    with open(dump_path, "w") as out:
        ...
        for k in ("description", "notes", "txn_type",
            ...
            prefix = "   " if before == after else " * "
            out.write(f"{prefix}{k:32s} {before!r}  →  {after!r}\n")
        out.write("\n  per-split changes:\n")
        ...
        for i in range(max(len(pre_splits), len(post_splits))):
            ...
            for k in ("memo", "action", "lot_ptr", "lot_guid",
                ...
                prefix = "     " if b == a else "   * "
                out.write(f"{prefix}{k:30s} {b!r}  →  {a!r}\n")
...
def test_orphan_backreference_probe_bill(tmp_path):
    ...
    dump_path = os.path.join(exports_dir(tmp_path, "bill"),
                             "orphan_backref_probe.txt")
    pre_record = {}
    post_record = {}
