"""What GnuCash's own Balance Sheet and Income Statement reports print for a multi-currency book, on this build (Q-042).

The book is the one `what_gnucash_currency_conversion_calls_return_probe.py`
...
Everything goes to files under the output directory passed as the first
...
Run: ./scripts/run.sh <tag> env PYTHONPATH=/workspace python3 tests/research/what_gnucash_own_balance_sheet_and_income_statement_print_probe.py <out-dir>
"""
...
from tests.research import what_gnucash_currency_conversion_calls_return_probe as base
