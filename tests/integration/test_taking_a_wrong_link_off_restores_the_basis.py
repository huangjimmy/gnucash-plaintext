...
receivable and moves it to the account `--to` states, restated into that account's
...
def test_a_basis_that_had_no_balance_comes_back_holding_all_of_it(tmp_path):
    ...
    It is the same answer every other writer reaches for that transaction: the
...
def _one_deposit_settling_both_invoices(runner, tmp_path, link_both=True):
    ...
    A receivable split per invoice, each stated by its own `payment:` block.
...
def test_a_deposit_shared_with_another_invoice_is_not_opened_whole(tmp_path):
    ...
    payment off moves that settlement to a CAD account, which prices the
