"""A document settled from an owner's credit survives being rebuilt.

Exporting a book and importing it into a fresh one is how this tool moves a
book, and a document settled out of credit used to come back unpaid: the
...
`invoice_postlot_handler: assertion 'lot' failed` — so the documents came back
with no lot at all and `IsPaid = False`, including ones settled by an ordinary
bank payment.

The export now records what settled the document instead of the request that
...
def _settled(book):
    """{document id: (lot balance as a string, IsPaid)} for every document."""
...
def _credit_book(runner, tmp_path, kind='invoice'):
    """A book where one document was settled out of the owner's credit."""
...
def test_a_document_settled_by_a_divided_credit_comes_back_settled(tmp_path):
    ...
    side, the part that settled the document, and the credit parked for what
    was left — and a rebuild is where this whole area has failed silently
    before: every document of an owner coming back unpaid, with nothing in
    the file to say so.
    """
