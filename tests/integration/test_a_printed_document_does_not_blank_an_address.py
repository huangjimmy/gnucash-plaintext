...
`print-invoice --format plaintext` is a document a person keeps, and reading
...
string. So reading a printed document back emptied the owner's address, and
the next `print-invoice` printed a document with nothing where the address had
been.
...
DOCUMENTS = str(Path('tests/fixtures/a_document_for_each_owner.txt'))
...
def book(tmp_path):
    ...
    for ledger in (LEDGER, DOCUMENTS):
        result = CliRunner().invoke(cli, [
            'import', *([] if ledger == DOCUMENTS else ['--new']), str(path),
            ledger, '--include-business-objects'])
        assert result.exit_code == 0, result.output
    return path


class TestReadingAPrintedDocumentBack:
    @pytest.mark.parametrize('command,document,oid', [
    ...
    def test_the_owners_address_survives(self, book, tmp_path, command,
        ...
        printed = tmp_path / f'{document}.txt'
    ...
    def test_a_key_the_format_does_not_know_survives_too(
        ...
        """
        printed = tmp_path / f'{document}-custom.txt'
        rendered = CliRunner().invoke(cli, [
            command, str(book), document, '--format', 'plaintext',
            '--output', str(printed)])
        assert rendered.exit_code == 0, rendered.output
