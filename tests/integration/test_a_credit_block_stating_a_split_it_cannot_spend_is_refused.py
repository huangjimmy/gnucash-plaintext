"""A credit block that states a split the book cannot spend as that credit is refused.

`from_credit: true` spends a split the book already holds, stated by
`txn_guid:` and `txn_split_guid:`. The book here holds one: 50.00 left over
from overpaying INV-001. Each test writes something else in the block, and the
import refuses the invoice and says what is wrong with what the block states.
"""
