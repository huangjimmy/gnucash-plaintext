"""The two boxes a reader fills in on a printed document, set from here.
...
document, and the styling of the page. Until now the only way to set either
...
document is styled is not that. An export of a book carrying them is the same
...
def _printed(book, tmp_path, *args, name='page.html'):
    ...
    result = CliRunner().invoke(cli, [
        'print-invoice', str(book), DOCUMENT, '--format', 'html',
        '--output', str(out), *args])
    assert result.exit_code == 0, result.output
...
class TestABillIsADocumentToo:
    """The setting is on the book, so both of its documents carry it.
...
class TestWhateverReportDraws:
    ...
    document the book prints, so the sentence reaches whichever report draws.
...
class TestTheyAreNotPartOfTheFormat:
    ...
    books that differ only in how their documents are styled export the same
