"""A payment block giving its record's own posting is refused before anything changes.

A posted record's ledger is read back with its payment's `txn_guid:` given the
...
destroyed that very posting, the guid then gave nothing, and the payment the
...
def test_a_new_invoice_giving_another_invoices_posting_is_refused(tmp_path):
    """INV-002 is new, and its payment gives INV-001's posting transaction.
