...
page for `东京物産株式会社` could print as `????????`, and the only sign of
...
description, and non-ASCII in both free-text blocks and the page's notes.
...
an ASCII page by `test_a_printed_pdf_can_be_selected_and_copied`.
"""
...
def _book_with_both_pages(tmp_path):
...
class TestEveryPartOfThePage:
    ...
    def test_the_pages_notes_keep_theirs(self, printed):
...
def test_the_written_file_is_utf8_whatever_the_locale_says(tmp_path):
    ...
    HTML, the per-page HTML, and the plaintext — and `Path.write_text`
    with no encoding takes the locale's. That fails two ways on this page
...
def _assert_the_locale_took(env, cwd):
    ...
    `?` per character on a page that still looks like a page, where
...
class TestUnderALocaleThatCannotHoldThePage:
    ...
    def test_a_ledger_round_trips_through_export_and_import(self, tmp_path):
        ...
        cannot hold either page, and the customer and vendor have to
