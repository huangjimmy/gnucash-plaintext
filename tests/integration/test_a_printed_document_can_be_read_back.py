"""Correcting a printed document and reading it back must not duplicate money.

`print-invoice --format plaintext` and `print-bill` write a document block, and
a document block is what `import` reads — so a reader who prints one, fixes a
...
two writers disagreed about is now one function, so a printed document is a
