"""A printed page's totals are in the invoice's currency.
...
it prices a page from the page — so the line rows, the subtotal, the
tax and the amount due are all in the currency the page is written in.
...
and the report is not. 5.10 states the payment in the invoice's currency and
...
currency against a USD invoice.

That row is the report's, not ours, so what these tests read is the totals: the
figure the whole defect was about. That a paid invoice shows nothing still due
is `test_a_printed_page_shows_what_is_still_due`, on a CAD invoice paid
...
class TestAUsdInvoiceWhoseIncomeAccountIsCad:
    ...
    def test_the_entry_rows_are_the_pages_own_currency(self, printed):
    ...
    def test_the_total_is_the_hundred_dollars_the_page_is_for(self,
                                                                 printed):
...
class TestAUsdBillWhoseExpenseAccountIsCad:
    ...
    page showed the book's valuation of what is owed rather than the
    ...
    def test_every_total_is_in_the_bills_own_currency(self, printed_bill):
        ...
        # in CAD. Nothing on a page addressed to a US vendor says CAD.
