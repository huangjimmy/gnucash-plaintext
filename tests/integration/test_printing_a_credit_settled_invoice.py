"""An invoice settled out of credit prints something that can be read back.

`print-invoice --format plaintext` carries the guids that make a printed
page re-importable, and this release documents and tests that path. It
holds for an invoice a bank paid.

An invoice settled out of an owner's credit has no bank side at all. The export
...
the book the page was printed from.
...
class TestReadingItBack:
    def test_the_page_it_was_printed_from_accepts_it(self,
                                                         book_and_printed):
