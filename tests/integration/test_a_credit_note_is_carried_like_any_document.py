"""A credit note is a document this format carries, by one key on the block.
...
way that window builds one — flag set, quantity stored negative:
    ...
    document          stored qty   its own total   what it posts
...
difference. It is written before anything else about the document because it
is what the rest of the document means: the same lines and the same accounts
...
and is what these tests hold shut. Refusing to export the document stopped
...
LEDGER = str(Path('tests/fixtures') /
             'a_credit_note_and_the_document_it_reverses.txt')
...
def imported(tmp_path):
    return _documents(_book(tmp_path))


class TestWhatTheImportPutsInTheBook:
    """The four documents post the way GnuCash posts them.
    ...
    def test_and_the_credit_note_posts_the_other_way(self, imported):
        ...
        direction — the tax among them. Without it this document rebuilt as
...
class TestWhatTheExportWrites:
    ...
    def test_and_only_on_the_documents_that_carry_it(self, exported):
        """Two of the four. `credit_note: false` on every ordinary document
        is a line saying nothing about the overwhelming majority of them, and
        its absence already means what a fresh document holds."""
...
class TestTheRoundTrip:
    def test_a_book_rebuilt_from_the_export_posts_the_same_way(self, tmp_path):
        ...
        assert _documents(rebuilt) == _documents(source)
...
class TestEditingTheFlagInTheLedger:
    def test_turning_a_credit_note_into_an_invoice_reports_it(self, tmp_path):
        """Left out of the comparison that decides `unchanged`, a ledger that
        changed the flag imported as `unchanged` and the book went on posting
        the document the other way."""
        book = _book(tmp_path)
...
class TestUnpostingItAndChangingTheFlagAtOnce:
    ...
    `posted: none` on a document that is posted, with everything else
    ...
    re-posting the document put it back the wrong way round.
    """
    ...
    def test_the_book_stops_holding_a_credit_note(self, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        assert _documents(book)['CN-001']['credit_note'] is False
...
class TestWhatThePrinterWrites:
    """A printed page states figures, and they have to be the document's.
    ...
    def _printed(self, tmp_path, command, document):
        book = _book(tmp_path, f'{document}.gnucash')
        page = tmp_path / f'{document}.txt'
        result = CliRunner().invoke(cli, [
            command, str(book), document,
    ...
    def test_a_printed_credit_note_states_its_own_total(self, tmp_path):
        ...
        these would be the negative of what the document states."""
