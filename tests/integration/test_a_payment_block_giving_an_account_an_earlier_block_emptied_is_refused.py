"""A payment block giving an account an earlier block of the same invoice emptied is refused.

INV-150's first payment block moves a deposit's 100.00 suspense split onto the
receivable. Its second block gives the deposit's 50.00 clearing split and says
...
block gives. Measured on 5.10: the import is refused with that account and the
...
TWO_BLOCKS = 'tests/fixtures/inv_150_giving_an_account_its_first_payment_emptied.txt'
