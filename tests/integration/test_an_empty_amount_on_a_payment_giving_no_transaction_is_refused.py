"""An empty `amount:` on a payment block that gives no transaction is refused before anything is compared.

A payment block giving its transaction need not state an amount, so there an
empty one states nothing. A block giving none records the payment from what it
