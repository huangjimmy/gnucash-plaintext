...
drawdown against any cost basis to show for it.

Every check also runs before any balance is written, so a sale measured against
several cost bases never lowers one of them and then fails on the next.
"""
...
def test_a_refused_sale_is_not_saved_while_the_rest_of_the_file_is(tmp_path):
    ...
    assert 'exceeds its cost basis balance' in result.output, result.output
    ...
    assert 'Total USD cost basis balance: 225.00 USD' in listing, listing     # 200 + 25, none sold
