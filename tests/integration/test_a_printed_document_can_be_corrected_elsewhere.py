"""A printed document read into a foreign book can then be edited there.
...
how every other document in this format is corrected.

That second run rebuilds the document, and rebuilding unposts it. An unpost
...
rebuild's own orphan and refuse the import, leaving the document half-built
...
def elsewhere_holding_the_printed_invoice(tmp_path):
    """Another book, built from the printed document rather than the ledger."""
...
class TestCorrectingItThere:
    def test_the_edited_document_re_imports(
            self, elsewhere_holding_the_printed_invoice, tmp_path):
        """The ordinary way every document in this format is corrected."""
