"""A payment stating a guid that matches nothing is refused where the deposit it describes has no readable owner.

Beta's INV-B is settled by one 100.00 deposit. Acme's INV-A then states a
...
STATING_A_GUID_THAT_MATCHES_NOTHING = (
    'tests/fixtures/a_payment_stating_a_guid_that_matches_no_transaction.txt')
...
def test_the_block_is_a_payment_of_its_own_where_both_owners_read(tmp_path):
    ...
    result = _done('import', book, STATING_A_GUID_THAT_MATCHES_NOTHING,
...
def test_the_import_is_refused_where_an_owner_has_no_id(tmp_path, emptied):
    ...
    result = _run('import', book, STATING_A_GUID_THAT_MATCHES_NOTHING,
                  '--include-business-objects')
