...
basis is the fault hardest to notice. `fx-balances` walks the bases, so
it never shows the figure; `--verify-costs` walked the same bases, so it
...
def _strand_the_balance(book_path, guid):
    ...
    figure to say what the USD cost — while the balance the basis had goes on
...
def test_a_stranded_balance_is_reported(tmp_path):
    ...
    assert DEPOSIT_SPLIT not in listing.output, (
        'the split has stopped being a basis, so the listing rightly drops '
        f'it:\n{listing.output}')
...
def test_a_split_that_never_had_a_figure_is_not_reported(tmp_path):
    ...
    try:
        ...
        repo.close()
    assert plain, 'the book should hold splits with no basis figure on them'
...
def test_a_balance_on_a_split_that_never_could_be_a_basis_is_reported(tmp_path):
    """A balance written onto a CAD split, which no basis can live on.

    The import refuses that line — a split in the book's own currency holds no
    foreign currency for a basis to be about — but a book can be handed one by
...
class TestABalanceAFileStatesOnASplitThatSpends:
    ...
    the book's own currency holds no foreign currency for a basis to be about.
