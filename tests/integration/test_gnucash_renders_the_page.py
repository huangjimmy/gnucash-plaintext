...
`services/gnucash_report` hands the book and the page's guid to GnuCash's
...
totals is computed here, so the page a customer receives is the one
...
def rendered(book):
    ...
    from services.gnucash_report import render_page_html
    ...
    try:
        ...
        query.destroy()
        yield render_page_html(repo.session, guid)
    finally:
        repo.close()


class TestTheFiguresAreTheInvoicesOwn:
    ...
    def test_no_figure_is_priced_in_the_books_currency(self, rendered):
        ...
        test: a page carrying `C$` at all is one pricing this USD invoice
        in the book's currency."""
        assert 'C$' not in rendered, rendered[-1500:]


class TestWhenItCannotDrawThePage:
    """A refusal is a sentence, and it reaches the person who ran the command.

    GnuCash's report resolves a page from a guid against whichever book is
    current. Asked for one the open book does not hold, it draws its "no
    invoice selected" page instead — which is not a page, and returning it
    ...
    def test_a_page_this_book_does_not_hold_is_refused(
        ...
        try:
            with pytest.raises(PageNotRenderedError) as refused:
                render_page_html(repo.session, another_books_guid)
        finally:
            repo.close()
    ...
    def test_naming_the_report_does_not_lose_the_refusal(
        ...
        `gnc:html-make-generic-warning` for a page the book does not hold:
        ...
        one.", against 4192 bytes for a real page — so it is checked on
    ...
    def test_asking_without_a_session_says_so(self):
        ...
        with pytest.raises(PageNotRenderedError) as refused:
            render_page_html(None, 'whatever')
