"""A `prepayment:` beside a split the block gives, with no `amount:`, is weighed when the payment is applied.

The check made before an invoice is compared weighs a block's residue only where
the block states an `amount:`. A block that gives its transaction need not state
one, so its residue is weighed where the settlement is placed, against the
receivable splits of the wire the block does not give. Measured on 5.10: 60.00
...
WIRE = 'tests/fixtures/a_wire_settling_c004_with_a_residue_given_by_split.txt'
