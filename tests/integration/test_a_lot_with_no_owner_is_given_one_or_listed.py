"""A credit in a lot with no owner: listed under the owner its transaction gives, or warned about, and given one by `lot_owner:`.
...
it is listed under that owner. It is warned about only where nothing gives an
owner, with how to give it one. A `lot_owner:` gives a lot with no owner that
owner, and one giving a lot owned by someone else is refused rather than
passed over.
"""
...
def test_a_lot_owner_gives_that_lot_its_owner(tmp_path, fixture, account, owner_file,
...
def test_a_split_given_a_lot_with_no_owner_is_refused(tmp_path):
...
def test_a_split_given_no_lot_passes_a_lot_with_no_owner_by(tmp_path):
...
def test_a_credit_read_back_under_another_customer_is_refused(tmp_path):
    ...
    assert 'gives `lot_owner: customer:C002`' in result.output, result.output
    listed = _run('find-prepayments', book)
...
def test_a_settlement_read_back_under_another_customer_is_refused(tmp_path):
    """The split settling C001's invoice, given `lot_owner: customer:C002`, was read by nothing."""
    book = tmp_path / 'book.gnucash'
    _imported('--new', book, ACCOUNTS)
    _imported(book, OVERPAID, '--include-business-objects')
    _imported(book, C002, '--include-business-objects')
