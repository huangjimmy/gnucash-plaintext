...
def test_the_applied_part_can_be_priced_again_after_an_unpost(tmp_path):
    ...
    `_mark_applied_from_credit`, which is the other place a spent credit loses
    its keys and re-reads the frame this leaves, so a cost dropped here is
    dropped for good.
    """
    runner = CliRunner()
    book = _overpaid_book(runner, tmp_path)
...
def test_a_balance_that_will_not_parse_survives_a_division_as_it_reads(tmp_path):
    ...
    second = tmp_path / 'second.txt'
    second.write_text(
        Path('tests/fixtures/fx_invoice_stating_a_part_sold_credit.txt').read_text()
...
def test_dividing_a_credit_valued_in_another_currency_balances(tmp_path):
    ...
    second = tmp_path / 'second.txt'
    second.write_text(
        Path('tests/fixtures/fx_invoice_stating_a_credit_valued_in_cad.txt').read_text()
        .replace('TXN_GUID', credit_txn).replace('SPLIT_GUID', credit_split))
    result = runner.invoke(cli, ['import', str(book), str(second),
                                 '--include-business-objects', '--fx-rates', RATES])
    assert result.exit_code == 0, result.output
