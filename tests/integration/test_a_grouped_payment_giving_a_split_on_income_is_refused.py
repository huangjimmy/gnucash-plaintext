"""A grouped payment block giving a split on an income account is refused.

INV-USD-001's block gives two splits of one transaction with a `Transaction`
...
GIVES_BOTH = FIXTURES / 'a_payment_giving_two_settling_splits.txt'
...
def test_it_is_refused_giving_that_split_and_its_account(tmp_path):
    book = tmp_path / 'book.gnucash'
    _done('import', '--new', book, BOOK, '--include-business-objects', '--fx-rates', RATES)
    _done('import', book, WITH_INCOME)

    result = _run('import', book, GIVES_BOTH, '--include-business-objects', '--fx-rates', RATES)
