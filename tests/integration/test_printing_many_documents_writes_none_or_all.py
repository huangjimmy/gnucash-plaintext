"""A refused figure must not leave a directory of half the documents.

`print-invoice … -o out/` and `print-bill … -o out/` write one file per
document. A printed `payment:` block now states the amount at the unit its
...
documents before the offender were on disk, the ones after it were not, and the
refusal said nothing about which. A reader who sends that directory on has a
set of documents that looks complete.
...
The combined form of these two commands already builds every document into a
list before writing; only the per-document form did not.
"""
...
def _two_invoices_one_unwritable():
    ...
    The first stays writable on purpose — a book where every document is
...
class TestPrintingInvoicesOneFileEach:
    ...
    def test_it_leaves_no_documents_behind(self,
    ...
    def test_a_book_it_can_write_still_writes_every_document(self, tmp_path):
        """The good path is unchanged: one file per document, all of them."""
...
class TestPrintingBillsOneFileEach:
    ...
    def test_it_leaves_no_documents_behind(self, book_with_one_unwritable_bill,
                                           tmp_path):
        outdir = tmp_path / 'out'
        CliRunner().invoke(cli, [
            'print-bill', book_with_one_unwritable_bill, '*',
