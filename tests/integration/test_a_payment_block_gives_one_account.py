"""A payment block gives exactly one account the money came from.
...
a block carrying neither gives no account at all, so the import refuses each.
"""
...
@pytest.mark.parametrize('fixture, said', [
    ('tests/fixtures/a_payment_giving_two_different_accounts.txt',
     "payment declares both account: 'Assets:Bank' and bank_account: "
     "'Equity:Owner' — they must name the same account"),
    ('tests/fixtures/a_payment_giving_no_account.txt',
