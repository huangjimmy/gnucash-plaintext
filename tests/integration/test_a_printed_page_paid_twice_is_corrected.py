"""Correcting a printed page that was settled by two payments at once.

The single-payment case is covered: a printed page read into a foreign
...
listed its splits in the order the page lists its payments. Measured by
walking that list the other way, the correction was refused.

The pairing is not visible in the book — both settlements are this page's
...
def _what_the_page_says(book, tmp_path, name='out.txt'):
    out = tmp_path / name
...
def elsewhere(tmp_path):
    """A second book built from the printed page, not from the ledger."""
    runner = CliRunner()
...
class TestCorrectingIt:
    def test_the_edited_page_re_imports(self, elsewhere, tmp_path):
    ...
    def test_each_payment_keeps_its_own_date_and_figure(self, elsewhere,
        ...
        text = _what_the_page_says(book, tmp_path)
    ...
    def test_a_block_naming_no_orphan_of_its_own_is_not_given_one(
        ...
        Reached by editing one payment's amount in a page whose blocks
        ...
        # gets it — one mismatch does not cost the page the rest.
    ...
    def test_each_block_reattaches_its_own_movement(self, elsewhere, tmp_path):
        ...
        Both settlements are this invoice's and both go back into its lot, so
        ...
        The rebuild is asked for the way a posted invoice's is: unpost it,
        then import. Nothing else about the file changes, so what the run
        does is put both settlements back — which is the pairing this is
        about.
        """
        book, printed = elsewhere
        before = _payments_by_amount(book)
        unposted = CliRunner().invoke(cli, ['unpost-invoices', str(book),
                                            'INV-TWICE'])
        assert unposted.exit_code == 0, unposted.output
        edited = tmp_path / 'edited.txt'
        edited.write_text(printed.read_text())
        ...
        # The blocks are read in the order the page writes them — 60.00
