"""A `posted:` block that states no receivable is refused for the field it leaves out.

A posted invoice is posted to the account its `posted:` block states in
...
STATED = '\t\tar_account: "Assets:Accounts Receivable"\n'
...
def test_it_is_refused(tmp_path, new_book):
    ...
    assert STATED in invoice, invoice
    book = tmp_path / 'book.gnucash'
    ledger = tmp_path / 'ledger.txt'
    if new_book:
        ledger.write_text(ACCOUNTS + '\n' + invoice.replace(STATED, ''))
        ...
        ledger.write_text(invoice.replace(STATED, ''))
