...
settling *another* owner's invoice. Neither excuses this shape — one customer,
...
onto this invoice — is not an operation at all: customer A's receipt cannot
settle customer B's invoice, so the match can only be coincidence.

What the refusal owes the reader is which invoice the money it found already
...
SAME_MEMO = str(FIXTURES / 'a_second_invoice_of_the_same_owner.txt')
OWN_MEMO = str(
    FIXTURES / 'a_second_invoice_of_the_same_owner_with_its_own_memo.txt')
...
class TestASecondBlockThatSaysNothingNew:
    ...
    def test_it_names_the_invoice_that_money_already_settles(
