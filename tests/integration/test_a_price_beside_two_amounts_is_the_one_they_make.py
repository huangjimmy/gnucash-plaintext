"""A split stating both amounts has the price they make, and a price stated beside them that differs is warned about.
...
`share_price:` beside `value:` as information, the price the two amounts make.
...
WARNED = 'the price is that division'
...
def test_a_new_transaction_warns_and_takes_the_price_the_amounts_make(tmp_path):
...
def test_a_price_the_amounts_make_says_nothing(tmp_path):
