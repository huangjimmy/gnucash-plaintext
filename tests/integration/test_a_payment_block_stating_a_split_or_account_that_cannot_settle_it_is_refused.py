"""A payment block stating a split or an account that cannot settle its record is refused.
...
import did write was the block's memo, onto whatever split the block stated.
...
- **another invoice's split.** One 200.00 deposit settles INV-A and INV-B, a
  receivable split each. INV-A's block, read on its own stating INV-B's split
  and a new memo, wrote that memo onto INV-B's settlement.
- **the payment's own bank split** stated in `txn_split_guid:`. The memo was
  written onto the bank split, and the split settling the invoice kept its
  own.
- **the invoice's own receivable** stated as `bank_account:`. The memo was
