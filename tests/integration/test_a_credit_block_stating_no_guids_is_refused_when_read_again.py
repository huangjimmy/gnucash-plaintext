"""A `from_credit:` block that states no guid is refused when the invoice is read again.

A credit block states the credit it spends with `txn_guid:` and
`txn_split_guid:`. INV-001 is posted and paid from the bank, and its export is
read back with the payment block rewritten as a credit stating neither. The
comparison cannot match a block stating no guid to the payment the book holds,
so the invoice reads as changed, and the block is refused for the guids it
does not state. The book keeps the payment it had.
"""
...
def test_it_is_refused_and_the_payment_stays(tmp_path):
    ...
    assert result.exit_code != 0, result.output
    assert ('a payment with `from_credit: true` must state the credit it applies, '
            'in `txn_guid:` and `txn_split_guid:`') in result.output, result.output
