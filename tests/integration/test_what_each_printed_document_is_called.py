"""A document's id is theirs to choose, and it is used as a file name.

`print-invoice`/`print-bill` with `-o out/` write one file per document, named
after the document. That is the right thing for a reader opening the
...
* an id holding a separator (`2026/001`, an ordinary way to number an invoice)
  ...
  after every document had been rendered and none written;
* two documents may share an id — GnuCash does not stop it, and both survive a
...
def _an_invoice(document_id, customer_id='C-NAME'):
    return f'''
invoice "{document_id}"
...
def _ledger(tmp_path, *document_ids):
    text = ('customer "C-NAME"\n\tname: "Naming Co."\n\tcurrency: CAD\n'
            + ''.join(_an_invoice(i) for i in document_ids))
...
class TestTwoDocumentsWithOneId:
    ...
    def test_each_is_named_so_the_other_is_findable(self, book_with_two,
        ...
        `SAME-ID.html` had no reason to suspect a second document at all.
        """
        outdir = tmp_path / 'out'
        assert CliRunner().invoke(cli, [
            'print-invoice', str(book_with_two), '*', '--format', 'html',
    ...
    def test_a_third_id_spelled_like_one_of_those_names_still_gets_its_own(
        ...
        A document whose id happens to read like another's `<id>_<guid>` would
...
class TestABillIsNamedTheSameWay:
    ...
    def _a_book_with_a_bill(self, tmp_path, document_id):
        book = _a_book(tmp_path)
...
class TestAnIdTooLongToBeAFilename:
    ...
    from the write — after every document in the run had been rendered, which
...
class TestAnIdThatIsNoNameAtAll:
    """`..` is a document id a ledger can state, and it addresses a directory
    rather than a file. It becomes `document`, because a file has to be called
    something."""
