"""An invoice block with no lines does not empty the invoice.
...
invoice with no lines at all. A file cut short by a failed write does that,
and so does a copy-paste that stopped early.

Tax tables have refused this all along ("must have at least one entry"). An
invoice does now too, when the book has lines to lose: creating an empty
invoice is still allowed, since there is nothing there to destroy.
"""
