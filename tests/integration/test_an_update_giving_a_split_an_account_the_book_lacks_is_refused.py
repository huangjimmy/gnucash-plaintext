"""An update giving a split an account the book does not have is refused.

`import --strategy update` puts a split on the account its block gives. An
account is created by an `open` directive, never by a transaction, so a block
giving one the book lacks is a mistyped name or a missing `open`. It is refused
