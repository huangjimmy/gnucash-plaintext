"""Two `price` blocks in one file giving one guid are both refused.

A guid is one price. Two new prices given one guid would both take it, and the
...
So every block giving a guid another block in the same file gives is refused,
...
class TestTwoNewPricesGivenOneGuid:
    FIXTURE = 'two_new_prices_given_one_guid.txt'
    ...
    def test_both_are_refused(self, tmp_path):
        ...
        assert f'this file gives guid {NEW_GUID} in 2 price blocks' in result.output, result.output
...
class TestTwoEditsOfOnePrice:
    FIXTURE = 'the_amzn_price_given_two_values_through_its_guid.txt'
    ...
    def test_both_are_refused(self, tmp_path):
        ...
        assert re.search(r'Prices:\s+0 created, 0 updated, 0 unchanged, 2 refused',
                         result.output), result.output
        assert f'this file gives guid {AMZN_GUID} in 2 price blocks' in result.output, result.output
