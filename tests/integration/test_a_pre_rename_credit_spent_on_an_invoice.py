"""Spending a pre-rename credit on an invoice leaves it holding nothing.
...
currency has been spent on the invoice.

Cleared by the new name alone, a credit applied whole keeps the old key on the
split that spent it. Inert in the book, since a split in an invoice's lot is no
...
def _the_credit_carries_the_old_key(book):
    ...
    engine hands the invoice when the credit is applied whole.
    """
...
class TestWhatTheSettlementHolds:
    ...
    def test_the_clear_survives_the_invoice_being_unposted(self, tmp_path):
        """Loose again, it still records no balance for what it spent.

        This is where a stale figure would start being read. In the invoice's
        ...
        removes it, and by the time anything reads it the invoice it was
