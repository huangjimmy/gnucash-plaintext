"""One document's unpost must not blind the duplicate guard to another's money.

A deposit can settle two documents at once, a portion each. Rebuilding one of
...
first document's mark hides the whole deposit from the second's check — and the
second document, whose `txn_guid:` names nothing, is handed a fresh payment for
money that moved once.

The mark names the document it was settling (CLAUDE.md finding 10), for exactly
this: one transaction can carry orphans from two documents, and which of them
...
def _documents(txn_a, split_a, txn_b, split_b) -> str:
...
def book_with_both_settled(tmp_path):
    ...
    settle.write_text(_documents(txn, parts['Portion for Alpha'],
...
class TestBothGuidsMistyped:
    ...
    Both documents are rebuilt, so each has an orphan of its own in the shared
    ...
    which portion belonged to which document without it.

    The refusal still belongs to a mistyped guid on a document that is
    ...
    def test_neither_portion_is_paid_a_second_time(self, book_with_both_settled,
        ...
        document's mark hid the whole deposit from the second document's
    ...
    def test_both_documents_are_still_settled(self, book_with_both_settled,
                                              tmp_path):
        ...
        # Each document names that one deposit as what paid it.
