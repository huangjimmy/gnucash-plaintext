"""A transfer's receiving split an earlier release gave a cost basis balance.
...
of that account gives instead: the guid of the cost basis the dollars came from.
...
def _give_the_transfer_a_balance(book):
...
def test_verify_costs_reports_the_balance_and_says_why_it_is_no_cost_basis(tmp_path):
    ...
    _give_the_transfer_a_balance(book)
    ...
    assert ("this split stores cost_basis_balance: '400.00', but it is no cost "
            ...
            "open a cost basis for. A spend out of this account gives the guid "
