...
`print-invoice` produces a document for the recipient — it carries `# Issued
...
`export` round-trips keys the format has no setter for. The printed document
...
document can say less without losing anything.
"""
...
class TestWhatThePrintedInvoiceSays:
    ...
    def test_it_still_says_who_the_customer_is(self, book, tmp_path):
        """Enough to be a document, which is what it is for."""
        text = self._printed(book, tmp_path)
...
class TestPrintingItDoesNotLoseIt:
    def test_the_key_is_still_on_the_customer_after_a_printed_re_import(
            self, book, tmp_path):
        """The document says less, and re-reading it takes nothing away."""
