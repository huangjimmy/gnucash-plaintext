"""Two payment blocks stating the same split are refused, whatever their memos say.

One split is one settlement, stated once. A paid record is imported and
exported, and its block is read back with the `payment:` block written twice,
both copies stating the same `txn_guid:` and `txn_split_guid:`. Measured on
5.10 before this:
...
def test_the_same_transaction_stated_twice_without_its_split_owes_nothing_for_the_second(
        tmp_path):
    """The second copy states `txn_guid:` alone, so no split is stated twice. The
