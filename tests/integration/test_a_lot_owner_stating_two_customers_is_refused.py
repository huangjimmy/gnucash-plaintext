...
`lot_owner: customer:ID:guid` states the owner by both, and the guid decides.
...
CREDIT = 'tests/fixtures/a_credit_whose_lot_owner_states_one_customers_id_and_anothers_guid.txt'
...
def test_the_transaction_is_refused(tmp_path):
    ...
    assert 'attributed to two customers' not in out.read_text(), out.read_text()
