...
;;
;; That option is how a document reaches a report — this tool sets it to the
;; guid of the invoice or bill being printed — so a report without it cannot
;; be told which document to draw. GnuCash ships several such reports (a
...
(define (a-report-that-takes-no-document-renderer report-obj)
  (let ((document (gnc:make-html-document)))
    (gnc:html-document-set-title! document "A Report That Takes No Document")
    (gnc:html-document-add-object!
      document
      (gnc:make-html-text
        (gnc:html-markup-p "THIS REPORT DRAWS NO DOCUMENT")))
    document))
...
(gnc:define-report
  ...
  'options-generator
    (lambda ()
      ;; Deliberately bare: no invoice option, nothing for the document.
      (if (defined? 'gnc-new-optiondb)
          (gnc-new-optiondb)
          (gnc:new-options)))
  'renderer a-report-that-takes-no-document-renderer)
