...
2. Signature-based matching: For new transactions (e.g., from QFX) using (date + accounts + doc_link)

## Why doc_link is part of the signature

doc_link is explicitly authored — the user sets it intentionally to link a transaction to a
receipt or external document. It is never auto-generated by GnuCash. Therefore:

  - Two transactions with the same date and accounts but DIFFERENT doc_links are genuinely
    distinct transactions (e.g. two grocery trips on the same day, each with a separate receipt).
    Including doc_link in the signature prevents these from being falsely collapsed as duplicates.

  - Strict equality (not wildcard) is correct because:
    a) Export always round-trips doc_link: export_transactions.py faithfully writes doc_link
       to plaintext, so a re-import of exported data will have the same doc_link as GnuCash.
    b) QFX/TSV imports have no doc_link (None), and the matching GnuCash transactions also
       have no doc_link (None), so None == None → correctly identified as duplicates.

  - Do NOT introduce None-as-wildcard. If a user manually attaches a receipt to a previously
    imported transaction inside the GnuCash UI and then tries to re-import the original
    plaintext file (which has no doc_link), the mismatch is intentional: the plaintext file
    is stale and should be re-exported before re-importing.
...
class TransactionMatcher:
    ...
    - DUPLICATE: Same signature (date + accounts + doc_link), amounts match
    - CONFLICT: Same signature (date + accounts + doc_link), amounts differ
    ...
    def get_signature(self, transaction) -> Tuple[str, Tuple[str, ...], Optional[str]]:
        """
        Extract transaction signature: (date, sorted_account_names, doc_link).

        doc_link is included because it is explicitly authored — two transactions
        with the same date and accounts but different doc_links are distinct transactions.

        Args:
            transaction: GnuCash Transaction object

        Returns:
            Tuple of (date_string, tuple_of_sorted_account_names, doc_link)

        Example:
            ("2024-01-15", ("Assets:Bank:Checking", "Expenses:Groceries"), "receipts/2024-01-15.txt")
        """
        ...
        for split in splits:
            ...
            account_names.append(self._get_account_full_name(account))
        ...
        return (date_str, tuple(sorted(account_names)), doc_link)
    ...
    def get_signature_for_plaintext(
        ...
        Args:
            ...
            doc_link: doc_link value from plaintext metadata, or None if not set
    ...
    def has_duplicate_signature(
        ...
        Returns:
            True if a transaction with the exact same signature exists
        """
        target_sig = self.get_signature_for_plaintext(date_str, account_names, doc_link)
    ...
    def get_duplicate_count(
        ...
        """
        seen_signatures: Set[Tuple[str, Tuple[str, ...], Optional[str]]] = set()
        duplicate_count = 0
