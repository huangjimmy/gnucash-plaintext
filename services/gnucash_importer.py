...
from services.positions_in_the_file import (
    replace_the_positions_with_their_guids,
    the_ways_to_write_it,
)
...
def _split_value_fraction(split_directive: 'PlaintextDirective') -> Fraction:
    ...
    converts the amount. A split that states neither is worth its amount — the
...
def string_to_gnc_numeric_quantity(s):
    """A line's quantity or price exactly as the file states it: `3`, `33.3333333` or `100/3`.

    A quantity or a price is a ratio, not money, so it has no smallest unit to
    be rounded to. Read at six decimals, `price: 33.3333333` was stored as
    33333333/1000000 and exported as `33.333333`, a figure the file never
    stated (measured on 5.10).
    """
    from gnucash import GncNumeric

    exact = Fraction(str(s))
    # GnuCash keeps a figure as a 64-bit numerator over a 64-bit denominator,
    # so a figure needing more cannot be held exactly, and is not rounded to
    # one that can. Passed on, the binding refused it with `in method
    # 'gnc_numeric_create', argument 2 of type 'gint64'`.
    if max(abs(exact.numerator), exact.denominator) > 2 ** 63 - 1:
        raise ValueError(
            f'{s} cannot be held exactly: GnuCash keeps a quantity or a price '
            f'as a whole number over a whole number, each at most '
            f'{2 ** 63 - 1}, and this one needs more digits than that')
...
class BusinessObjectImportResult:
    ...
    def tally(self, kind: str, status: str) -> None:
        # Every status is one of the four keys: each import function returns
        # only those, and an unknown one would raise `KeyError` here anyway.
...
def _find_transaction_by_guid(book, guid: str):
    ...
    Returns None when no transaction has that GUID.

    Every caller hands over a guid that parses: one `_normalise_guid` has
    already read, or a payment block's, which
    `_refuse_a_payment_guid_nothing_can_parse` reads before any payment is
    applied.
    """
    from gnucash import Transaction
    from gnucash.gnucash_core_c import GncGUID, string_to_guid, xaccTransLookup
    gnc_guid = GncGUID()
    string_to_guid(guid, gnc_guid)
...
def _normalise_guid(guid) -> str:
    ...
    # one handed the format a value it could take and could not write back:
...
def _find_customers_by_id(book, id_: str):
    """Return all Customer records whose user-facing id is `id_`."""
...
def _find_invoices_by_id(book, id_: str):
    """All customer invoices whose id is `id_`.
...
def _find_bills_by_id(book, id_: str):
    """All vendor bills whose id is `id_`."""
    from gnucash import Query
    q = Query()
    q.search_for('gncInvoice')
    q.set_book(book)
...
def _find_invoice_by_guid(book, guid_norm: str):
    """Customer invoice whose GUID is `guid_norm`, or None.
    ...
    # The shared handle, which declares both functions and opens the library
    # the bindings hold the book in (CLAUDE.md finding 2).
    lib = load_gnc_engine()
    ...
    for r in q.run():
        ...
        if inv.GetOwnerType() != _GNC_OWNER_CUSTOMER:
            continue
        # A record the query returns is an entity, so it always has a guid.
...
def _find_bill_by_guid(book, guid_norm: str):
    """Vendor bill whose GUID is `guid_norm`, or None."""
    import ctypes
...
def _taxtable_guid_str(tt_ptr) -> str:
    """Read a tax-table's GUID via ctypes from its raw pointer."""
    import ctypes

    from infrastructure.gnucash.engine import load_gnc_engine
    lib = load_gnc_engine()
    buf = ctypes.create_string_buffer(40)
    # A tax table the book lists is an entity, so it always has a guid.
...
def _taxtable_name_str(tt_ptr) -> str:
    """Read a tax-table's name via ctypes from its raw pointer."""
...
def _guid_of_a_qof_instance(obj) -> str:
    ...
    lib = load_gnc_engine()
    buf = ctypes.create_string_buffer(40)
    # Anything the engine holds is an entity, so it always has a guid.
...
def _entity_in_collection(book, type_name: bytes, guid_norm: str):
    ...
    lib = load_gnc_engine()
    # `qof_book_get_collection` makes the collection where the book has none
    # yet, so it always answers with one.
...
def _guid_in_use_anywhere(book, guid_norm: str) -> Optional[str]:
    ...
    g = GncGUID()
    # Every caller hands over a guid `_normalise_guid` has already read, so it
    # parses.
    string_to_guid(guid_norm, g)
...
def _the_guids_a_payment_block_states(pay_dir) -> Set[str]:
    """Every transaction and split guid a `payment:` block states, normalised.
    ...
    Empty where the block states none, which is a payment the run is making
    ...
    for key in ('txn_guid', 'txn_split_guid'):
        ...
        if wrote:
            stated.add(_normalise_guid(wrote))
    block_txn, block_splits = the_settlement_a_block_states(pay_dir)
    if block_txn:
        stated.add(_normalise_guid(block_txn))
    # Every `PaymentSplit` states a guid that parses: an empty or misspelt one
    # is refused by `_refuse_a_payment_guid_nothing_can_parse` before any
    # payment is applied.
    stated.update(_normalise_guid(guid) for guid in block_splits)
    return stated


def _transactions_a_payment_block_edits(book, stated: Set[str]) -> list:
    """The transactions those guids reach.

    A guid matches either a transaction or one of its splits, and both answer
    ...
    found = {}
    for guid in stated:
        # Every guid here has been through `_normalise_guid`, so it parses.
...
def _priced_bases_in(transactions) -> Dict[str, Fraction]:
    ...
    Every priced cost basis, whether or not a balance is written on it. A cost basis with
    ...
    no cost was written and it stopped being a cost basis: the row left the listing
    ...
    split unable to price a cost basis by editing the transaction that split is
    ...
    for transaction in transactions:
        for split in transaction.GetSplitList():
            try:
                ...
                # A cost basis whose own figures will not read is
                # `--verify-costs`'s to report. Nothing here can price it, so
                # nothing here can tell whether a link changed that.
                continue
            # Never None: `establishes_cost_basis` answers True only where
            # `cost_of` returns a figure.
...
def _the_record_prices_this_currency(record, split) -> bool:
    ...
    day it was posted, and keeping the deposit's cost basis as well would put
    two cost bases on one lot of money.
    ...
    """
    # The posted record a payment block was just applied to, so it has a posting.
    posting = record.GetPostedTxn()
    currency = split_commodity(split)
    for each in posting.GetSplitList():
        # One question of each split, in whatever order the posting lists
        # them: its own currency and a cost basis in it.
        try:
            if split_commodity(each) == currency and establishes_cost_basis(each):
                return True
        except Exception:
            continue
    return False
...
def _keep_or_discard_a_cost_basis(book, priced_before, is_bill, record=None,
    ...
    **A part payment discards it too**, and what that costs is worth stating.
    ...
    keeping the deposit's cost basis beside it counts the same money twice — and
    the two are not both refused for long: once the rest of the invoice is
    paid, its lot closes, the receivable's cost basis becomes sellable, and the
    ...
    **Paying an invoice in full**, it is neither a purchase nor a borrowing
    ...
    the cost basis the import opened is discarded, or the same USD has two — which
    ...
    the cost basis and the bank split gets none.

    **Paying a bill**, it is still a borrowing. USD drawn on a credit line to
    pay a supplier is still owed once the payable is paid, so the cost basis
    ...
    cost basis being discarded. The `share_price:` on the split and the record's
    ...
    record's cost basis would silently re-price it, and `_require_stated_cost`
    ...
    record's cost basis keeps every figure checked, and that is what the message
    asks for.
    ...
    # per cost basis would make the cost of a payment block the product of the
    ...
    for guid, cost in priced_before.items():
        ...
        if not has_cost_basis_balance(split) and is_a_spent_credit(split):
            ...
            # customer's credit takes the cost basis keys off the part that was
            # spent and moves the balance onto the remainder, so the split
            # stops being a cost basis with nothing left on it — which is the state
        ...
        if is_bill or not _the_record_prices_this_currency(record, split):
            ...
            # went from one cost basis to "No foreign-currency cost bases found",
...
def _the_lot_guid_stated(metadata) -> Optional[str]:
...
def _refuse_to_destroy_a_split_in_a_lot(split) -> None:
    ...
    The same money as `_splits_and_the_accounts_their_blocks_state` refuses to
...
def _the_owner_a_lot_owner_line_states(book, kind: str, owner_id: str, owner_guid):
    """The customer or vendor a `lot_owner:` line states.

    Refused where the book has no such customer or vendor, or where the line's
    guid is another one's: the guid is authoritative.
    """
    lookup = book.CustomerLookupByID if kind == 'customer' else book.VendorLookupByID
    owner = lookup(owner_id) if owner_id else None
    if owner is None or not owner.GetID():
        raise Exception(
            f'lot_owner states {kind} {owner_id!r}, and this book has no {kind} '
            f'with that id')
    if owner_guid and owner.GetGUID().to_string() != owner_guid:
        raise Exception(
...
def _the_live_lot_and_its_owner(split):
    """The split's lot, and whose that lot is as `(kind, id)`.

    `(None, None)` for a split in no lot, or holding a lot its account no
    longer lists (`_lot_is_still_on_its_account`); `(lot, None)` for a lot with
    no owner, which GnuCash's View → Lots makes.

    A posted invoice's or bill's lot is its record's owner's, read off the
    record: GnuCash keeps no owner on that lot. Measured on 5.10,
    `gncOwnerGetOwnerFromLot` fails for it, and only an unpost attaches one
    (CLAUDE.md finding 10). Read off the lot, a `lot_owner:` stating C002 on the
    settlement of C001's invoice met a lot with no owner, was not refused, and
    would have made C002 the owner of the invoice's lot.
    """
    from infrastructure.gnucash.engine import load_gnc_engine

    lot = split.GetLot()
    lot = lot if lot is not None and _lot_is_still_on_its_account(split, lot) else None
    lib = load_gnc_engine()
    buffer = ctypes.create_string_buffer(256)
    owner_p = ctypes.cast(buffer, ctypes.c_void_p)
    record = lib.gncInvoiceGetInvoiceFromLot(qof_pointer(lot)) if lot is not None else None
    if record:
        # The customer or vendor, where the record is a job's.
        owner_p = lib.gncOwnerGetEndOwner(lib.gncInvoiceGetOwner(record))
    elif lot is None or lib.gncOwnerGetOwnerFromLot(qof_pointer(lot), owner_p) != 1:
        return lot, None
    raw_id = lib.gncOwnerGetID(owner_p)
    return lot, (_OWNER_KINDS.get(lib.gncOwnerGetType(owner_p)),
                 raw_id.decode('utf-8', errors='replace') if raw_id else '')


def _refuse_to_move_a_split_to_another_owner(book, split, metadata) -> None:
    """A split in one owner's lot may not be moved to another owner by a `lot_owner:` edit.

    Whose money a split is does not change by re-importing it, for the reason
    a `lot_guid:` does not move it (`_refuse_to_move_a_split_between_lots`).
    The update path reads `lot_owner:` to put a split in no lot in its owner's
    lot, and to attach the owner the line states to a lot with no owner. So a
    line moving C001's credit, or the settlement of C001's invoice, to C002 was
    otherwise read by nothing: the run said the transaction was updated and
    the book still said C001.
    """
    stated = str(metadata.get('lot_owner') or '').strip()
    if not stated:
        return
    lot, held = _the_live_lot_and_its_owner(split)
    if held is None:
        return
    kind, owner_id, owner_guid = _parse_lot_owner(stated)
    owner = _the_owner_a_lot_owner_line_states(book, kind, owner_id, owner_guid)
    if held == (kind, owner.GetID()):
        return
    if gc.gncInvoiceGetInvoiceFromLot(qof_instance(lot)):
        remedy = ('take it off that invoice or bill first, with `unapply-payment` '
                  'or `unlink`')
    else:
        remedy = ("spend it on an invoice or a bill of its owner's, or take it off "
                  "with that owner's own records")
    raise Exception(
        f'this split is in {held[0]} {held[1]!r}\'s lot and the file states '
        f'`lot_owner: {stated}`. Whose money a split is does not change by '
        f'being re-imported: {remedy}')


def _attach_the_owner_to_a_lot_with_none(book, split, stated: str) -> None:
    """Attach the owner `lot_owner:` states to the lot a split is in, where that lot has none.

    GnuCash's View → Lots makes such a lot: "New Lot" attaches no owner. A
    credit in it can be spent by no invoice or bill, and `find-prepayments`
    says so and sends the reader here. Read only for a split in no lot, the
    line did nothing at all.
    """
    lot, held = _the_live_lot_and_its_owner(split)
    if lot is None or held is not None:
        return
    from infrastructure.gnucash.engine import load_gnc_engine
    ...
    buffer = ctypes.create_string_buffer(256)
    owner_p = ctypes.cast(buffer, ctypes.c_void_p)
    initialise = (lib.gncOwnerInitCustomer if kind == 'customer'
                  else lib.gncOwnerInitVendor)
    initialise(owner_p, int(owner.instance))
    lib.gncOwnerAttachToLot(owner_p, qof_pointer(lot))
...
def _refuse_to_move_a_split_between_lots(split, metadata) -> None:
    ...
    with the split's guid in `txn_split_guid:`.
    """
    wanted = _the_lot_guid_stated(metadata)
    if not wanted:
        return
    ...
    if held and held != wanted:
        raise Exception(
            f'this split is in lot {held} and the file states lot {wanted}. '
            ...
            f'with this split\'s guid in `txn_split_guid:`')
...
def _force_the_lot_guid(lot_ptr, guid_norm: str) -> None:
    """Set on a lot the import has just created the guid its file states.
...
def _attach_lot_owner_split(book, split, split_account, kind, owner_id,
    ...
    **`lot_guid` says which credit**, where the block names one. An owner may
    ...
    `lot_owner:` is authoritative, not informational: if `owner_guid` is stated it
    ...
    want_type, side = ((11, 'Accounts Receivable (customer)') if kind == 'customer'
                       else (12, 'Accounts Payable (vendor)'))
    ref = _the_owner_a_lot_owner_line_states(book, kind, owner_id, owner_guid)
    ...
    if best_lot is not None:
        ...
        #
        # Nothing to check afterwards. `gnc_lot_add_split` leaves out a split
        # from another account, and says nothing, but this split is on
        # `split_account` — both callers put it there first — and the lot is on
        # that account too: a named one was refused otherwise, and a searched
        # one came off that account's own lot list. The join used to be judged
        # by whether the lot's balance moved, which a split of nothing never
        # does: a 0.00 split View → Lots had put in a credit lot, exported and
        # read into a fresh book, was refused as "GnuCash refused the lot
        # membership" (measured on 5.10).
...
# credit and strip the cost basis off a settlement the bank really paid.
#
...
#: it writes with a `guid:`. Read once from the file, because whether a
...
#: The posting transaction each of this file's invoices states by
...
#: The transactions this file states that carry a `guid:`, and each of their
#: splits that does, by its guid, as its block and its transaction's guid.
#: Read where `--atomic` checks the book the file leaves rather than the book
#: half way through it (Q-053).
_TRANSACTIONS_THE_FILE_RESTATES: set = set()
_DRAWS_THE_FILE_STATES: dict = {}
#: Each transaction's guid to the guids of its splits the file states, and,
#: built on first use, each split the file's blocks draw on to the
#: transactions drawing on it (`_what_the_file_draws_on_each_split`).
_SPLITS_THE_FILE_STATES_OF_EACH_TRANSACTION: dict = {}
_WHAT_THE_FILE_DRAWS_ON: Optional[dict] = None

#: Each invoice and bill this file states, as `(kind, id)`, to the
#: transactions its `payment:` blocks state. Kept only where the run applies
#: the blocks: a record the file states and whose blocks state no transaction
#: is one the file states unpaid by any transaction the book holds (Q-053).
_RECORDS_THE_FILE_STATES: dict = {}

#: The payments this run took off an invoice or bill the file states unpaid
#: by them, a sentence each (Q-053).
_PAYMENTS_THE_RUN_TOOK_OFF: list = []

#: The transactions this file states that the run could not import, by guid,
#: with why. A `payment:` block stating one of them finds no transaction in the
...
def forget_a_transaction_the_run_refused(directive) -> None:
    """Forget that the run refused this file's transaction, before `--atomic` applies it again (Q-053)."""
    # A guid nothing can parse was never noted, and is the block's own
    # refusal again when it is applied.
    stated = directive.metadata.get('guid')
    if not _states_a_guid(stated):
        return
    try:
        _TRANSACTIONS_THE_RUN_REFUSED.pop(_normalise_guid(stated), None)
    except ValueError:
        return
...
def mark_splits_orphaned_by_unpost(record) -> None:
    ...
    cost basis off currency the bank really paid and the book still holds, and the
    ...
    """
    # Asked only of a posted record, immediately before it is unposted:
    # `_execute_unpost` reports an unposted one as not posted first, and the
    # importer unposts only a record the book has posted. A posted record has
    # its lot and its posting, and a record the book holds has a guid.
    lot = record.GetPostedLot()
    guid = _swig_invoice_guid_str(record)
    posting_ptr = int(record.GetPostedTxn().instance)
...
def still_orphaned_by_an_unpost(book, txn_guid: str) -> bool:
    """Whether a transaction still holds a split an unpost left loose.

    `import` hears of an orphan at the unpost that makes it, and a later step
    of the same run can put that settlement back, which takes the mark off
    (`_attach_split_to_lot`). So the warning is printed for the transactions
    still marked once the book is saved. Printed at the unpost, it said a
    payment was orphaned in a run that put the payment back, and in a run that
    was refused and saved nothing.

    **Any split of the transaction, not this record's.** One transaction can
    carry marks from two records — one deposit settling two invoices, each
    unposted — so the blunt question looks wrong beside `_retarget_choices`,
    which reads the guid to find the settlement that was its own. It is not,
    and the reason is worth writing down because the shape that would make it
    wrong cannot arise: a run that unposts a record never also restores that
    record's settlement.

    Only two things reach an importer-side unpost. A file saying `posted:
    none` asks for one out loud, and such a file carries no `payment:` block
    at all. Or a `payment:` block fails to pair with what the lot holds — and
    a block that states `txn_guid:`/`txn_split_guid:` is paired on those guids
    alone (`_pair_off_payments`, `_single_payment_matches`: for a retarget
    block "date/amount/memo on the directive aren't authoritative"), so the
    only edit that makes such a block fail to pair is changing the guids,
    which stops it describing the settlement it would have restored. A
    guid-less block pairs on date, amount and memo, and one that no longer
    matches them is a block describing other money, which is why the payment
    it displaces is left orphaned on purpose
    (`inv_001_paid_60_on_the_16th_and_40_on_the_20th.txt`).

    Measured on 5.10: one deposit settling INV-SHARE-A and INV-SHARE-B, B
    unposted so its mark sits on the shared transaction, then A re-imported
    with its payment dated a day later — A reads back `unchanged`, because the
    block states its guids. Scoping this to the record would therefore change
    no run's output, while costing the warning the one thing it is for.
    """
    transaction = _find_transaction_by_guid(book, _normalise_guid(txn_guid))
    return transaction is not None and any(
        _orphaned_from(split) for split in transaction.GetSplitList())
...
def refuse_a_stated_orphan_mark(metadata, where: str) -> None:
    ...
    spent from a credit keeps a cost basis for currency it no longer holds. Worse,
...
def refuse_a_stated_brought_in(metadata, where: str) -> None:
    ...
    `refuse_a_stated_orphan_mark` states.
    """
...
def _refuse_to_discard_a_part_sold_balance(split, guid: str,
    ...
    Sales it *does* record are refused a few lines above, by their disposals.

    Discarding such a balance loses the only statement of that 720.00 there
    is, and nothing afterwards can recover it: the split becomes a settlement,
    which is no cost basis, and taking the payment off later opens it at the
    whole 2,720.00 — the book then offering currency that is gone, with every
    check passing, because 2,720.00 is exactly what the split brought in.

    So the link stops, and says what to write if the difference is not
    currency this book should account for.

    A figure that will not parse stops it too, and for the same loss. It is a
    balance somebody wrote, and `cost_basis_balance_of` answers None for it
    exactly as it does for a split that has none — so read alone, that check
    would let the strip delete the text `--verify-costs` is reporting, and a
    later unapply would open the split at its full amount over currency that
    may be sold. `malformed_cost_basis_balance_of` is what tells the two
    apart, and every other reader of a balance in this module asks it.
    """
    unreadable = malformed_cost_basis_balance_of(split)
    if unreadable:
        raise Exception(
            f'linking this payment discards the cost basis on split {guid}, '
            f'and its balance reads {unreadable!r}, which is not a figure. '
    ...
    currency = split_commodity(split)
    raise Exception(
        f'linking this payment discards the cost basis on split {guid}, and '
        f'its balance is {money_text(balance, unit)} {currency} of the '
...
def _strip_a_settlements_basis(split) -> None:
    ...
    A settlement holds no cost basis: the currency was spent on the invoice, and
...
def _carry_the_orphan_mark(guid: str, residue) -> None:
    """Write on the residue of a divided orphan the mark its source carried.
    ...
    """
    # Never empty: the one caller passes the mark of a split it has just found
    # to be a bank-paid orphan, and a split is always in a transaction.
...
def _refuse_if_nothing_owed(owed: Fraction, kind: str, doc_id: str) -> None:
    ...
    `txn_split_guid:` states a split outright and attaches it without comparing
...
def _refuse_if_below_the_accounts_unit(owed: Fraction, post_account,
    ...
    raise Exception(
        ...
        f'another way, or set a finer `commodity_scu:` on the account')


def _takeable_from(owed: Fraction, post_account) -> Fraction:
    """How much of `owed` a payment can actually take on `post_account`.
...
def _settle_from_one_split(lib, book, record, existing_tx, counter_split,
    ...
    opens a cost basis — and where the money came from decides at what cost. A bank
    ...
    if (covering * unit).denominator != 1:
        raise Exception(
            ...
            f'halves would no longer sum to what the transaction holds. State '
            f'the split\'s guid in `txn_split_guid:` to attach it whole, or set '
            f'a finer `commodity_scu:` on the account.')
    ...
    # Read before the division too, and for the same reason as the cost basis keys
    ...
    if from_credit:
        ...
        # follows: the mark goes forward, and no cost basis opens, because a
...
def _carry_basis_to_residue(residue, carried, remainder: Fraction,
                            book, spent_guid: str) -> None:
    """Move a divided credit's cost basis onto the part of it that is left, and the
    disposals already drawing on it with the cost basis.

    A credit whose cost basis has no `cost_basis_balance` written on it keeps none:
    ...
    the largest figure the division could produce onto a cost basis nobody can
    ...
    # measured against the residue. The split the disposals used to state
...
def _mark_spent_credit(split) -> None:
    ...
    What it carried as a credit comes off. Its cost basis is gone — the currency
    ...
    sale's guid matches a cost basis that was consumed rather than one that never
    ...
    #
    # It does not make this split a cost basis in the meantime: settling a record
...
def _retarget_with_prepayment_split(lib, book, record, existing_tx,
    ...
    searching the account for it: whatever cost basis the money carried belongs on
...
def _attach_existing_tx_as_posted(invoice_or_bill, existing_tx, ar_ap_account,
    ...
    # Both callers found `existing_tx` before calling this, and call it only
    # where they did.
...
def _named_address_matches(owner, md: dict) -> bool:
    ...
    Only the named ones, for the reason `_write_named_address` states: what a
...
def _correct_payment_memos(record, directive, book) -> None:
    """Write a payment block's `memo:` onto the transaction it states.
    ...
    *are* collected and the run saves around them. If an invoice's or bill's errors
    ...
    Only for a block stating its transaction. A memo is otherwise part of what
    ...
    changed matches no payment at all. One stating its transaction matches
    ...
    **One payment, one memo, every split it names.** A grouped block is one
    ...
    for child in directive.children:
        if child.type != DirectiveType.PAYMENT:
            continue
        block_txn, block_splits = the_settlement_a_block_states(child)
        ...
        if _the_block_is_stating_the_bank_splits_memo(
            ...
            # person cannot write on a settling split — it is how a ledger an
            ...
            _echo_note(
                ...
                f'file already states on the bank split, which is how a ledger '
    ...
    **The split that settles the invoice or bill** — the one `txn_split_guid:`
    ...
    if _states_a_guid(named):
        ...
        return next((split for split in transaction.GetSplitList()
                     if split_guid(split) == wanted), None)
    # Only `_correct_payment_memos` gets here, with the record in hand and
    # after `_settles_this_record` found one of this transaction's splits in
    # its lot. `_single_payment_matches` passes no record, and only where the
    # block states its split, which answers above.
    return _the_settlement_in_this_records_lot(record, transaction)
...
def _the_settlement_in_this_records_lot(record, transaction):
    ...
    `_settles_this_record` states: a lot does not list a split attached in
    the session that attached it (CLAUDE.md §9).
    """
    # Asked only once `_settles_this_record` has found such a split in the
    # same lot, with nothing changed in between, so there is one.
    wanted = qof_pointer(record.GetPostedLot())
    return next(split for split in transaction.GetSplitList()
                if split.GetLot() is not None
                and qof_pointer(split.GetLot()) == wanted)
...
def _the_bank_split_named(transaction, metadata):
    ...
    By account, not "the first that is not a receivable", because a payment
    carrying a fee has two splits that are neither and the block declares
    which one it means.
    """
    from gnucash import gnucash_core_c as gc

    wanted = metadata.get('account') or metadata.get('bank_account')
    if wanted:
        # There is one. Nothing reads a memo before the refusals at the top of
        # `import_invoice` and `import_bill` have run:
        # `_refuse_a_payment_account_its_transaction_did_not_use` refuses a
        # block whose transaction has no split on its account, and
        # `_refuse_a_credit_block_stating_an_account_or_a_date` a credit block
        # stating an account at all. A split always has an account (CLAUDE.md
        # finding 12).
        return next(split for split in transaction.GetSplitList()
                    if get_account_full_name(split.GetAccount()) == wanted)
    ...
    for split in transaction.GetSplitList():
        if gc.xaccAccountGetType(split.GetAccount().instance) not in (
...
def _its_counterpart(transaction, target, metadata):
    ...
    **Whatever type the record's account is.** Both callers pass a split that
    settles the record, in its lot: the settlement a block states, or the one
    the record's unpost marked. A bill posted to a plain liability is settled
    there as a bill posted to a payable is, and `ApplyPayment` writes its one
    memo on both sides of it the same way. Asked of the account's type, a
    corrected memo reached only the settlement on such a bill, and the bank
    split kept the old one.
    ...
    **And nothing follows on a payment settling several of them**, whose
    ...
    it, no single block owns it, and the wording a bank feed wrote for the
    ...
    # A credit block has no counterpart at all. The transaction it states is
    ...
    if (_BLOCKS_SETTLING_FROM_A_TRANSACTION.get(
        ...
        return None
    # Never the target itself: the target is on the account the record posts
    # to, and a block stating that account as the one the money moved through
    # is refused before anything is compared
    # (`_refuse_a_payment_block_that_cannot_settle_its_record`).
...
def note_what_the_file_states(directives, applies_payments: bool = False) -> None:
    ...
    **What each split's memo is**, from the transaction section. Read from
    ...
    written down — so a `payment:` block stating one of its splits met an
    ...
    **Which split each block applies to its record**, and only where
    ...
    _POSTINGS_THE_FILE_STATES.clear()
    _TRANSACTIONS_THE_RUN_REFUSED.clear()
    _TRANSACTIONS_THE_FILE_RESTATES.clear()
    _DRAWS_THE_FILE_STATES.clear()
    _SPLITS_THE_FILE_STATES_OF_EACH_TRANSACTION.clear()
    global _WHAT_THE_FILE_DRAWS_ON
    _WHAT_THE_FILE_DRAWS_ON = None
    _RECORDS_THE_FILE_STATES.clear()
    _PAYMENTS_THE_RUN_TOOK_OFF.clear()
    for directive in directives:
        if directive.type == DirectiveType.TRANSACTION:
            _note_what_the_transaction_draws_on(directive)
            ...
            paid_by = set()
            if applies_payments:
                _RECORDS_THE_FILE_STATES[(kind, str(directive.props.get('id', '')))] = paid_by
            for child in directive.children:
                if child.type == DirectiveType.PAYMENT:
                    paid_by.update(_the_transactions_a_payment_block_states(child))
                ...
                if not applies_payments or not _states_a_guid(applied):
                    continue
                # The whole split: a block stating `txn_split_guid:` applies
...
def _note_what_the_transaction_draws_on(directive) -> None:
    """Record the transaction block's guid, and what each of its splits draws on, for `--atomic` (Q-053).

    A guid nothing can parse is the block's own refusal when the block is
    reached, and a transaction stating one is left out here.
    """
    try:
        restated = the_guid_a_block_names(directive.metadata)
        splits = [(the_guid_a_block_names(split.metadata), split)
                  for split in directive.children]
    except ValueError:
        return
    if restated is None:
        return
    _TRANSACTIONS_THE_FILE_RESTATES.add(restated)
    for declared, split in splits:
        if declared is not None:
            _DRAWS_THE_FILE_STATES[declared] = (split, restated)
            _SPLITS_THE_FILE_STATES_OF_EACH_TRANSACTION.setdefault(restated, set()).add(declared)


def _the_draw_a_split_block_states(split) -> Optional[str]:
    """The cost basis a split block draws on, as the book writes a guid; None where it draws on none.

    Read when it is asked, not when the file is read: under `--strategy
    update` a position is replaced with the guid it points at before any
...
def _the_transactions_a_payment_block_states(pay_dir) -> set:
    """The guids of the transactions a `payment:` block states, by `txn_guid:` or by a `Transaction` under it.

    A guid nothing can parse is left out here; the block refuses it when the
    invoice or bill is imported.
    """
    stated = [pay_dir.metadata.get('txn_guid')] + [
        child.props.get('guid') for child in pay_dir.children
        if child.type == DirectiveType.PAYMENT_TRANSACTION]
    guids = set()
    for guid in stated:
        if not _states_a_guid(guid):
            continue
        try:
            guids.add(_normalise_guid(guid))
        except ValueError:
            continue
    return guids


def _take_off_the_records_the_file_no_longer_pays_with_it(book, existing_tx, directive) -> None:
    """Under `--atomic`, take this transaction off an invoice or bill the file states unpaid by it, before the edit moves its split to another account (Q-053).

    A split settling an invoice or a bill is in the record's lot, and an edit
    moving it to another account is refused: moving it is the record's business.
    But the file states the record, and none of its `payment:` blocks states
    this transaction, so in the book the file leaves, this transaction does
    not pay it. That is what an owner imports to go back to the book as it
    was before a statement line was booked as the invoice it collected: the
    line on the suspense account again, and the invoice with `payment: none`.
    The record's block is applied after the transactions, so read one block
    at a time, the transaction cannot move while it pays the record, and the
    record cannot be left unpaid while its settlement draws on it.

    So the payment is taken off as `unapply-payment` takes it off, onto the
    account the file states for the split. Only under `--atomic`, where the book
    is checked once the file is applied and nothing is saved otherwise; and
    only where the run applies the file's invoice and bill blocks.
    """
    from use_cases.unapply_payment import unapply_payments

    here = _normalise_guid(existing_tx.GetGUID().to_string())
    by_guid = {split_guid(split): split for split in existing_tx.GetSplitList()}
    root = book.get_root_account()
    for child in directive.children:
        split = by_guid.get(the_guid_a_block_names(child.metadata) or '')
        if split is None or get_account_full_name(split.GetAccount()) == child.props['account']:
            continue
        lot = split.GetLot()
        if lot is None or not _lot_is_still_on_its_account(split, lot):
            continue
        raw = gc.gncInvoiceGetInvoiceFromLot(qof_instance(lot))
        if not raw:
            continue
        record = wrap_invoice_or_bill(raw)
        paid_by = _RECORDS_THE_FILE_STATES.get((kind_of(record), record.GetID()))
        # Not a record the file leaves out, which says nothing about it; nor
        # one it still pays with this transaction; nor the record's own
        # posting, which is no payment. The split stays in the lot, and the
        # edit refuses to move it.
        if (paid_by is None or here in paid_by
                or record.GetPostedTxn().GetGUID().to_string() == here):
            continue
        taken = unapply_payments(book, record, find_account(root, child.props['account']),
                                 kind=kind_of(record), txn_guids=[here])
        # Said, as `unapply-payment` says it: the record owes again, and
        # nothing else the run prints states the id of the record that changed.
        # Once the book is saved, by `cli/import_cmd.py`: a run that rolls
        # back took nothing off.
        for _guid, amount, currency in taken.unapplied:
            _PAYMENTS_THE_RUN_TOOK_OFF.append(
                f'{kind_of(record)} "{record.GetID()}": took off the payment '
                f'of {amount} {currency} by transaction {here}, onto '
                f'{taken.to_account}, as this file states the '
                f'{kind_of(record)} unpaid by it')


def payments_the_run_took_off() -> list:
    """What `_take_off_the_records_the_file_no_longer_pays_with_it` took off this run, a sentence each, for the import to print once it saves.

    Handed over once and forgotten, so no later run in the same process
    prints them again.
    """
    taken = list(_PAYMENTS_THE_RUN_TOOK_OFF)
    _PAYMENTS_THE_RUN_TOOK_OFF.clear()
    return taken
...
def _the_block_is_stating_the_bank_splits_memo(
    ...
    the memo the file states on the bank split is describing that split, and
...
def _set_the_entry_guid(book, entry, metadata) -> None:
    """Set on a fresh entry the guid its block states, if it states one.
    ...
    none is unaffected; the entry keeps the guid GnuCash assigned it.
    """
...
def _entries_paired_with_blocks(book, entry_directives, existing_entries):
    ...
    **A block states its line's guid in `guid:`, and that is what it edits.**
    As a split block states its split's, and for the same reason: position is
    what decides where a block states no guid, and nothing else.
    ...
    - **a guid the book assigned to something else** — another invoice's line, an
    ...
    for directive in entry_directives:
        ...
        # which `_set_the_entry_guid` sets on it.
    return _pair_entries(entry_directives, existing_entries)


def _the_guid_a_line_block_names(metadata) -> Optional[str]:
    """A line block's `guid:`, normalised, or None where it states none.

    The comparison that decides `unchanged` asks this, and a predicate that
    raises is one the caller cannot use. It does not: `import_invoice` and
    `import_bill` pair the lines through `_entries_paired_with_blocks` before
    they compare, and a guid nothing can parse is refused there.
    """
    declared = metadata.get('guid')
    if not _states_a_guid(declared):
        return None
...
def _lines_match_their_blocks(record, entry_blocks, is_bill: bool) -> bool:
    ...
    first, by position only where a block states none — because the two have
...
def _splits_and_the_accounts_their_blocks_state(book, existing_tx, directive,
                                                root_account) -> list:
    """Settle which split each block's `guid:` identifies, once, for the whole
    transaction, and which account each block states for that split.
    ...
    - **A split keeps its guid when its account changes.** Changing a block's
      ...
      and that split moves to the account the block states. Left
    ...
    **Nothing is moved here.** The moves are returned for the caller to
    ...
    for child in directive.children:
        ...
        if not _states_a_guid(declared):
            continue
        # Malformed refuses, as `_the_lot_guid_stated` and the line blocks
        # do, rather than being read as "this block states no split". Read
        ...
        # Never empty: the parser refuses a split line with no account.
        wanted_account = child.props.get('account')
        if get_account_full_name(split.GetAccount()) == wanted_account:
            continue
        # Found: `update_transaction` refuses a block stating an account the
        # book has not got before anything else reads the block.
        ...
        if in_lot is not None and _lot_is_still_on_its_account(split, in_lot):
            raise ValueError(
                ...
                f'this file puts it on another account. A split in a lot is '
...
def _splits_grouped_by_the_account_each_block_states(existing_tx, moves) -> dict:
    """account name → [splits], each split under the account the file states for it.

    The same grouping the rebuild works from, worked out without moving
    anything: a split whose block moves it to another account is grouped where it is
...
def _set_the_account_each_block_states(moves) -> None:
    """Make the moves `_splits_and_the_accounts_their_blocks_state` worked out.
...
def _splits_paired_with_blocks(split_directives, existing_splits):
    ...
    **A block states its split's guid in `guid:`, and that is what it
    updates.** Position decides only where a block states none, which is what it always
    ...
    for index, directive in enumerate(split_directives):
        ...
        if split is None:
            ...
            # stating no guid at all. What a guid may match is settled once
            # per transaction, before any of this, by
            # `_splits_and_the_accounts_their_blocks_state`: a split of another
    ...
    for index, already in enumerate(chosen):
        # A block stating a guid takes no spare, as a line block takes none:
...
def _the_new_splits_guid(book, split, directive, where: str) -> None:
    """Set on a split the update path creates the guid its block states.
...
def _the_guid_a_block_names(directive) -> Optional[str]:
    ...
    format reads one. Answered `None`, it read as a block stating no split:
...
def _refuse_figures_that_are_not_the_books(record, directive, is_bill: bool):
    ...
    for index, (child, declared, breakdown_declared) in enumerate(
            stated_by_line, 1):
        # Always one. Every caller has either paired this record's lines with
        # these blocks through `_lines_match_their_blocks`, or just built the
        # lines from them. A match compares every field, an absent key read as
        # the default the rebuild writes, so two blocks that match one line
        # match the same lines, and taking the first free one never leaves a
        # later block without its own.
        found = next(position for position in unpaired
                     if matches(entries[position], child))
...
def _entry_matches_invoice_directive(entry, ed: 'PlaintextDirective') -> bool:
    ...
    # the default the rebuild would set, not passed over, and a word is
...
def _entry_matches_bill_directive(entry, ed: 'PlaintextDirective') -> bool:
    ...
    # an unnamed key is compared against the default the rebuild sets.
...
def _refuse_a_lot_owner_naming_no_customer_or_vendor(directive) -> None:
    """Refuse a split's `lot_owner:` that states no customer or vendor.

    `lot_owner:` says the split is an owner's credit and puts it in a lot of
    theirs. The owner is written as `customer:ID` or `vendor:ID`, the two owners
    the format writes. An ID with no kind, or another kind of owner, states
    neither, and was read as nothing: the split was imported loose, reported
    as a transaction like any other, while the file said it was somebody's
    credit.

    Asked of the whole transaction before it is created or edited, so a
    refused file has changed nothing.
    """
    for child in directive.children:
        stated = child.metadata.get('lot_owner')
        if not stated:
            continue
        kind, owner_id, _ = _parse_lot_owner(str(stated))
        if kind in ('customer', 'vendor') and owner_id:
            continue
        # A `lot_guid:` beside it says which credit, and would be read by
        # nothing too, so it is quoted where the file has one.
        written_guid = _the_characters_a_block_wrote(child.metadata, 'lot_guid')
        and_its_lot = (f' Its lot_guid {written_guid} would then be read by '
                       f'nothing either.' if written_guid else '')
        raise ValueError(
            f'lot_owner {str(stated)!r} on the split on '
            f'{child.props.get("account")!r} states no customer or vendor. It '
            f'says the split is an owner\'s credit, and an owner is written as '
            f'`customer:ID` or `vendor:ID`, so nothing could put the split in '
            f'a lot of theirs.{and_its_lot} Write the owner that way, or remove '
            f'the lot_owner: line')


def _set_the_lines_bill_pointer(entry, bill) -> None:
...
def _refuse_a_payment_guid_nothing_can_parse(directive) -> None:
    ...
    the case `_the_lot_guid_stated` refuses for in the same release, and a
    ...
    own table prints — `Invalid GUID format: 'hello'` — wherever a guid is
    ...
    for block in directive.children:
        ...
        for child in getattr(block, 'children', []):
            if child.type == DirectiveType.PAYMENT_TRANSACTION:
                ...
                # Every child a `PaymentSplit`: the parser refuses any other line
                # under a `Transaction`.
                for split in getattr(child, 'children', []):
                    _normalise_guid(split.props['guid'])


def _refuse_one_split_stated_by_two_payment_blocks(directive) -> None:
    """Refuse two payment blocks stating the same split, before anything is compared.

    One split is one settlement, stated once. `the_settlement_a_block_states`
    refuses a `PaymentSplit` repeated inside one block, and nothing looked
    across two. Measured on 5.10, a paid invoice's own block read back with its
    payment block repeated read as a payment being added: it changed nothing,
    and the run reported `updated` and saved the book on every read of the
    file. With another memo, the second copy's memo replaced the first, with
    nothing said.

    By guid, whichever spelling states it, `txn_split_guid:` or `PaymentSplit`,
    and asked after `_refuse_a_payment_guid_nothing_can_parse`, so every guid
    here parses.
    """
    kind = 'bill' if directive.type == DirectiveType.BILL else 'invoice'
    seen = set()
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT:
            continue
        # The `Transaction` block decides where both are written, as it does
        # when the payment is applied.
        _txn, stated = the_settlement_a_block_states(block)
        declared = block.metadata.get('txn_split_guid')
        for guid in stated or ([declared] if _states_a_guid(declared) else []):
            wanted = _normalise_guid(guid)
            if wanted in seen:
                raise ValueError(
                    f'{kind} {directive.props.get("id", "")!r}: split {wanted} is '
                    f'stated by two `payment:` blocks. A split settles a {kind} '
                    f'once, so each block must state a split of its own. Remove '
                    f'the repeated block, or state in the second one the split it '
                    f'means.')
            seen.add(wanted)


_PAYMENT_FIGURES = (
    ('amount', 'payment amount must be a number, got {!r}'),
    ('prepayment', 'prepayment field must be a number, got {!r}'),
    ('settled_amount', 'payment settled_amount {!r} is not a number'),
    ('share_price', 'payment share_price {!r} is not a number'),
)


def _refuse_a_payment_figure_nothing_can_read(directive) -> None:
    """A payment block's figure that is not a number refuses, before the
    invoice or bill is compared to the file.

    `amount:`, `prepayment:`, `settled_amount:` and `share_price:` are numbers
    written with a point. A block stating its transaction is matched by guid,
    so on a record that otherwise matched nothing read them: `1OO` for `100`
    was answered `unchanged`. A decimal comma is the same mistake, as
    `docs/multi-currency.md` says of `60,00` for `60.00`; on a new book
    `amount: 100,00` was refused as `[<class 'decimal.ConversionSyntax'>]`,
    and `prepayment: 50,00` on a block stating no transaction was read as
    50.00.

    Asked once, here, so every reader after it takes a number: `#None` is not
    one, and an empty value states nothing.
    """
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT:
            continue
        for key, refusal in _PAYMENT_FIGURES:
            if key not in block.metadata:
                continue
            text = str(block.metadata[key]).strip()
            if not text:
                # An empty figure states nothing — except the amount of a block
                # stating no transaction, which records the payment from what it
                # states. Passed over, INV-001 read back with `amount: ""`
                # compared as changed and was unposted to be rebuilt before the
                # amount was read (measured on 5.10).
                if key == 'amount' and not _names_its_transaction(block.metadata, block):
                    raise Exception(refusal.format(text))
                continue
            try:
                Fraction(text)
            except (ValueError, ZeroDivisionError) as exc:
                raise Exception(refusal.format(text)) from exc
...
def _refuse_an_amount_that_is_not_the_applied_splits(record, pay_dir, post_acct,
    ...
    if not stated:
        return
    # A number: `_refuse_a_payment_figure_nothing_can_read` refused anything
    # else before the record was compared.
...
def _refuse_a_split_that_is_not_placeable(split, post_acct, record, kind: str,
    ...
    **Income, expense and equity.** A split there is not money waiting to be
    ...
    moving one to the receivable takes a sale out of the P&L: measured on a
    ...
    **Anything holding units rather than money.** A settlement is restated in
    ...
    if not holds_money(account):
        raise Exception(
            ...
            f'currency, and units are not money, so putting this split on that '
            f'account would overwrite them. State the guid of the split that '
            f'received the money.')
    ...
    # either. A rebate split was told to state the guid of a split this bill
    # posts to, and handed back the account it had just stated.
    #
    ...
    remedy = (f'This one posts to '
              f'{", ".join(repr(name) for name in posting_books)}. State the '
              f'guid of a split on an account this {kind.lower()} posts to, '
              f'or the guid of the split that received the money.'
              if could_move_it
              else 'State the guid of the split that received the money.')
    allowance = (f' — unless this {kind.lower()} posts to that same account, '
                 ...
                 f'it to the payable is what leaves the cost booked once'
    ...
    raise Exception(
        ...
        f'other side of what the entry records, so moving it to the '
...
def _park_the_declared_residue(record, book, lib, existing_tx, post_acct,
    ...
    `lot_owner:` put it in, and still counts toward the declared figure.
    ...
    """
    # A number, as `_refuse_a_payment_figure_nothing_can_read` made it.
    ...
    for sib in loose_siblings:
        ...
        _attach_record_owner_to_lot(lib, record, new_lot_ptr)
        # And it is money the book holds and owes back, so it opens a cost
        # basis, as the residue of a split the import divides does. Asked here,
        # once the split is in its owner's lot, since that lot is what makes
        # it a credit rather than a settlement. Only a split parked here: one
        # already in a lot came with the lot `lot_owner:` put it in, and with
        # whatever `cost_basis_balance:` its file stated. Measured without
...
def _refuse_a_stated_amount_the_applied_splits_contradict(book, directive,
    ...
    **The spelling that states its splits, and only that one.** `amount:` is a
    ...
    Weighed against the split the key states, that remedy is refused by the tool that
    ...
    for block in directive.children:
        ...
        if not stated:
            continue
        # Only the spelling that states its splits. `amount:` means a different
        ...
        _txn, stated = the_settlement_a_block_states(block)
        wanted = [_normalise_guid(guid) for guid in stated]
        if not wanted:
            continue
...
def _refuse_a_payment_account_its_transaction_did_not_use(book, directive) -> None:
    """A block stating its transaction and an account that transaction has no split on refuses.

    The block says where the money went twice: `txn_guid:` states the movement,
    and `account:` states the account it went through. Where the book holds that
    transaction and it has no split on that account, the two disagree, and
    nothing an import does makes them agree. Measured on 5.10, a paid invoice's
    own block read back with `bank_account:` changed read as a changed payment:
    the invoice was unposted, its posting transaction destroyed and posted
    again under a new one, and `updated` reported with exit 0. The export still
    wrote the old account, and every later read of the file did it all again.

    So it is asked before the comparison, beside the other refusals nothing is
    written ahead of. Only where the book holds the transaction: a guid that
    matches nothing is a page read into a book that never held the money, and
    the payment is recorded from the block. A `from_credit:` block states no
    account, because no bank moved anything.
    """
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT or _paid_from_credit(block.metadata):
            continue
        block_txn, _splits = the_settlement_a_block_states(block)
        txn_guid = block_txn or _the_characters_a_block_wrote(block.metadata, 'txn_guid')
        account = block.metadata.get('account') or block.metadata.get('bank_account')
        if not _states_a_guid(txn_guid) or not account:
            continue
        transaction = _find_transaction_by_guid(book, _normalise_guid(txn_guid))
        # A posting never reaches here: `_refuse_a_payment_stating_a_posting`
        # refuses one just before, and correcting `account:` would not change
        # that answer.
        if transaction is None:
            continue
        # A split always has an account (CLAUDE.md finding 12).
        if any(get_account_full_name(split.GetAccount()) == account
               for split in transaction.GetSplitList()):
            continue
        kind = 'bill' if directive.type == DirectiveType.BILL else 'invoice'
        raise Exception(
            f'{kind} {directive.props.get("id", "")!r}: this block says the money '
            f'is in {account!r}, and tx {_normalise_guid(txn_guid)!r} has no split '
            f'on that account. Check `account:` against the transaction, or state '
            f'the guid of the transaction the payment is really on.')


def _refuse_a_payment_stating_a_posting(book, directive, record) -> None:
    """A payment block stating a posting transaction refuses, before anything is compared.

    `refuse_a_posting_transaction` says why a posting pays nothing, and it was
    asked only when the payment was applied. On a record the book holds, that
    is after the rebuild: measured on 5.10, a paid invoice's ledger read back
    with its payment's `txn_guid:` set to the invoice's own posting read as a
    changed payment, the rebuild unposted the invoice and so destroyed that
    posting, the guid then matched nothing, and the payment the invoice had was
    put back. The run reported `updated` with exit 0, a corrected memo never
    landed, and every later read did the same under a new posting guid.

    `record` is the invoice or bill the book holds, or None for one this file
    creates, which has no posting of its own yet.
    """
    kind = 'Bill' if directive.type == DirectiveType.BILL else 'Invoice'
    asked_of = record if record is not None else _RecordName(directive)
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT or _paid_from_credit(block.metadata):
            continue
        block_txn, _splits = the_settlement_a_block_states(block)
        txn_guid = block_txn or _the_characters_a_block_wrote(block.metadata, 'txn_guid')
        if not _states_a_guid(txn_guid):
            continue
        wanted = _normalise_guid(txn_guid)
        transaction = _find_transaction_by_guid(book, wanted)
        if transaction is not None:
            refuse_a_posting_transaction(
                asked_of, transaction, kind,
                '`Transaction`' if block_txn else '`txn_guid:`', wanted)


def _refuse_a_payment_block_that_cannot_settle_its_record(book, directive, record) -> None:
    """A payment block stating a split or an account that cannot settle its record refuses.

    Asked before the comparison. A record whose block matches it on
    `txn_guid:` is answered `unchanged` and no payment is applied, so the
    refusals the payment path makes were never asked, while the block's memo
    was still written onto whatever split the block stated. Measured on 5.10,
    each of these read `unchanged` with exit 0:

    - a split in another invoice's or bill's lot: one deposit settling INV-A
      and INV-B, and INV-A's block read on its own stating INV-B's split, wrote
      INV-A's memo onto INV-B's settlement;
    - the payment's own side: `txn_split_guid:` stating the split on the
      account the block says the money moved through, which settles nothing,
      and the memo went onto the bank split;
    - the account the record posts to stated as the payment's account: the
      memo went onto the receivable split, and the bank split did not follow.

    A split or a transaction the book does not hold is passed over, as a page
    read into another book states them. A `from_credit:` block states no account
    and is checked where the credit is spent.
    """
    kind = 'bill' if directive.type == DirectiveType.BILL else 'invoice'
    doc_id = directive.props.get('id', '')
    asked_of = record if record is not None else _RecordName(directive)
    posted = _the_account_the_file_posts_to(book, directive)
    posted_name = get_account_full_name(posted) if posted is not None else None
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT or _paid_from_credit(block.metadata):
            continue
        account = block.metadata.get('account') or block.metadata.get('bank_account')
        if account and account == posted_name:
            raise Exception(
                f'{kind} {doc_id!r}: this payment block states {account!r} as the '
                f'account the money moved through, and that is the account this '
                f'{kind} posts to. A payment moves money between that account and '
                f'another one: state the account the money went through.')
        block_txn, stated = the_settlement_a_block_states(block)
        txn_guid = block_txn or _the_characters_a_block_wrote(block.metadata, 'txn_guid')
        declared = block.metadata.get('txn_split_guid')
        splits = stated or ([declared] if _states_a_guid(declared) else [])
        if not _states_a_guid(txn_guid) or not splits:
            continue
        transaction = _find_transaction_by_guid(book, _normalise_guid(txn_guid))
        if transaction is None:
            continue
        held = {split_guid(split): split for split in transaction.GetSplitList()}
        key = '`PaymentSplit`' if block_txn else '`txn_split_guid:`'
        for guid in splits:
            wanted = _normalise_guid(guid)
            split = held.get(wanted)
            if split is None:
                continue
            # A split always has an account (CLAUDE.md finding 12).
            # Said as the payment path says it, which is where a record that is
            # being changed met it before this.
            if account and get_account_full_name(split.GetAccount()) == account:
                raise Exception(
                    f'{kind.capitalize()} {doc_id}: {key} {wanted!r} is on '
                    f'{account!r}, and this {kind} posts to {posted_name!r}. Every '
                    f'split a payment applies has to be one that settles this '
                    f'{kind} — apply the {the_records_own_account(kind)} splits, '
                    f'not the bank side.')
            _refuse_a_split_settling_another_record(
                asked_of, split, kind.capitalize(), doc_id, wanted)


def _refuse_a_payment_account_with_no_commodity(book, directive) -> None:
    """A payment block whose account has no commodity refuses, stating the account.

    GnuCash keeps an account with no commodity through a save and a reload, so
    a book can hold one, and a payment through it has no currency to be
    recorded in. Measured on 5.10, a block paying into one ended the import
    with `'NoneType' object has no attribute 'get_mnemonic'`, read off the
    account while working out the rate the payment settles at.

    Asked before the comparison, beside the other refusals nothing is written
    ahead of. An account the book does not hold is left to the refusal that
    says so, and a `from_credit:` block states no account, because no bank moved
    anything.
    """
    root = book.get_root_account()
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT or _paid_from_credit(block.metadata):
            continue
        name = block.metadata.get('account') or block.metadata.get('bank_account')
        # Not asked with no name: `find_account` answers that with the root.
        account = find_account(root, name) if name else None
        if account is None or account.GetCommodity() is not None:
            continue
        kind = 'bill' if directive.type == DirectiveType.BILL else 'invoice'
        raise Exception(
            f'{kind} {directive.props.get("id", "")!r}: {name!r} has no '
            f'commodity, so a payment through it has no currency to be '
            f'recorded in. Set a commodity on the account, and import again.')


def _refuse_a_split_on_an_account_with_no_commodity(book, directive) -> None:
    """A transaction block with a split on an account with no commodity refuses, before anything is built.

    GnuCash keeps such an account through a save and a reload, and a split is
    an amount of its account's commodity, so a split on one has none to be an
    amount of. `export` refuses a book holding one. Measured on 5.10: `import`
    took the split at exit 0, as a new transaction and with `--strategy update`
    alike, stored an amount of 10/1 beside a value of 1000/100, and wrote a
    book the export then refused.

    Asked first on both ways in, so the cost and balance checks never meet
    such an account. An account the book has not got is left to the refusal
    that says so.
    """
    root = book.get_root_account()
    ...
    # transaction.
    for child in directive.children:
        name = child.props['account']
        account = find_account(root, name)
        if account is None or account.GetCommodity() is not None:
            continue
        raise Exception(
            f'{name!r} has no commodity, and the transaction '
            f'{directive.props.get("tx_desc") or ""!r} on '
            f'{directive.props.get("date", "?")} has a split on it. A split is '
            f'an amount of its account\'s commodity, so this one has none to '
            f'be an amount of. Set a commodity on the account, and import again.')
...
class _RecordName:
    ...
    point of it — so it is passed the id the directive states rather than one
    ...
    def GetPostedTxn(self):  # noqa: N802 - matches the GnuCash object it stands in for
        """None: a record the book does not hold yet has no posting."""
        return None

    def GetPostedLot(self):  # noqa: N802 - matches the GnuCash object it stands in for
        """None: a record the book does not hold yet has no lot."""
        return None
...
def _is_not_a_residue(split) -> bool:
    ...
    - one in a lot that names an invoice or a bill, which is that record's own
...
def _the_split_this_book_holds(book, guid: str):
    ...
    gnc_guid = GncGUID()
    # Parses: both callers pass a guid `_normalise_guid` has read, and it
...
def _refuse_a_credit_block_stating_a_transaction(directive) -> None:
    ...
    already states its transaction and split in the two keys — with a
    ...
    for block in directive.children:
        ...
        raise Exception(
            'a payment with '
            '`from_credit: true` states the credit it applies in `txn_guid:` '
            ...
            'would be a line the file states and the run ignores. State the '
            'credit in those two keys, or drop `from_credit:` if a bank '
            'transaction really settled this.')


def _refuse_a_credit_block_stating_an_account_or_a_date(directive) -> None:
    """`account:`, `bank_account:` or `date:` on a `from_credit:` block, before
    the record is compared to the file.

    A credit moves no money, so a block spending one has no account and no
    date of its own. Asked only where the credit was applied, an unchanged
    record never got there: measured on 5.10, INV-002 settled out of its
    customer's credit and read back from its own export with any of the three
    added to that block reported `unchanged` with exit 0, and the line was
    read by nobody.
    """
    for block in directive.children:
        if block.type != DirectiveType.PAYMENT or not _paid_from_credit(block.metadata):
            continue
        md = block.metadata
        # No `invoice "…":` prefix — `import_business_objects` wraps every
        # failure with one.
        for key in ('bank_account', 'account'):
            if md.get(key):
                raise Exception(
                    f'a payment with `from_credit: true` states no account — '
                    f'the money is already in the book, on the credit '
                    f'`txn_split_guid:` states. Drop `{key}:`, or drop `from_credit:` '
                    f'if a bank really paid this.')
        if md.get('date'):
            raise Exception(
                'a payment with `from_credit: true` has no date of its own — '
                'GnuCash records none for applying a credit. Use '
                '`credit_dated:` for the date of the transaction the credit '
                'arrived in.')


def _the_file_states_a_posting_this_book_has(book, directive) -> bool:
...
def _posted_matches_directive(invoice, posted_dir: 'PlaintextDirective',
    ...
    # Asked only of a posted record, which has its posted account and its
    # posting transaction: both callers check it is posted first.
    posting_acct = invoice.GetPostedAcc()
    if get_account_full_name(posting_acct) != md[ar_or_ap_key]:
        return False
...
def _spend_the_owners_credit_valued_in_another_currency(record) -> None:
    """Spend the owner's credit valued in another currency, oldest first, as a `from_credit:` block spends one.

    GnuCash's own application values such a credit at par on 3.8, 4.4 and 4.13
    (CLAUDE.md finding 19). Measured on 4.13 for part of one: 40.00 USD taken
    from a 100.00 USD credit received from a CAD bank at 1.37 left 97.00 USD of
    credit valued at 97.00 CAD, with no cost basis balance, and the import
    exited 0. So such a credit is divided here, where the value is divided with
    the amount and the cost basis moves onto what is left, and `AutoApplyPayments`
    applies whatever the record still owes after.

    Only a credit whose value is not its amount, which is the one the engine
    gets wrong. A credit valued in its own currency, a settlement an unpost
    loosened, and a credit note are left to the engine as before. So where an
    owner holds both kinds, the credit valued in another currency is spent
    first, whatever its date against the other.
    """
    from infrastructure.gnucash.engine import load_gnc_engine

    post_acct = record.GetPostedAcc()
    lot = record.GetPostedLot()
    is_bill = record.GetOwnerType() == _GNC_OWNER_VENDOR
    kind = 'Bill' if is_bill else 'Invoice'
    owner = _the_customer_or_vendor_of(record)
    credits = []
    for split in post_acct.GetSplitList():
        amount = numeric_to_fraction(split.GetAmount())
        # A customer's credit is a credit of the receivable, a vendor's a debit
        # of the payable.
        if amount == 0 or (amount > 0) != is_bill:
            continue
        if numeric_to_fraction(split.GetValue()) == amount:
            continue
        if not _sits_in_an_owners_credit(split) or _recorded_owner_of(split) != owner:
            continue
        credits.append(split)
    credits.sort(key=lambda split: (split.GetParent().GetDate(), split_guid(split)))

    for split in credits:
        owed = _still_owed(record, lot, post_acct)
        if owed <= 0:
            break
        covering = abs(numeric_to_fraction(split.GetAmount()))
        if covering > owed:
            _settle_from_one_split(
                load_gnc_engine(), post_acct.get_book(), record, split.GetParent(),
                split, post_acct, lot, owed, covering, kind, record.GetID(),
                from_credit=True)
        else:
            _attach_split_to_lot(split, lot)
            _mark_spent_credit(split)
...
def _apply_owner_credit(record) -> None:
    ...
    does not come here — it states the credit to spend, and one bigger than the
    ...
    _spend_the_owners_credit_valued_in_another_currency(record)
    ...
    # filtered to the splits carrying cost basis keys; now that it reads the whole
...
def _asks_for_credit(directive) -> bool:
    ...
    other way round, as a payment block stating the credit it spent, and
    carries no flag.
    """
    return _a_yes_or_no(
...
def _names_its_transaction(md, block=None) -> bool:
    ...
    Off the children rather than through `the_settlement_a_block_states`, which
...
def _bank_side_figure_of(md, block=None) -> Optional[Fraction]:
    ...
    if not stated:
        return None
    # Every figure here is a number: `_refuse_a_payment_figure_nothing_can_read`
    # refused anything else before any of this was asked.
    figure = abs(Fraction(stated))
    residue = str(md.get('prepayment') or '').strip()
    # Either spelling of "this block states its transaction". Read as the
...
def _single_payment_matches(split, pd) -> bool:
    ...
    if _paid_from_credit(md):
        ...
        # split the file stated and parks the rest as a new one, so what the file
        # stated is what settled it.
    ...
    # long as it exists there. The block states the *source* book's split,
...
def _pair_off_payments(pay_splits, payment_dirs, book=None):
    ...
    states its transaction by guid and another describes the same amount, date
    ...
    So a block is released again when the split holding it can be paired
    ...
    def find_a_block_for(split_index, tried):
        for slot_index, (block, only) in enumerate(slots):
            ...
            if only is not None:
                # A slot of a block that stated its splits fits that split and
...
def _refuse_a_block_nothing_reads_under_the_record(directive, kind: str) -> None:
    """Refuse a block under an invoice or bill that is not a line, its posting or a payment.

    The import reads those three under a record, and a line's own blocks
    under its `entry:`. Anything else, such as a line's `breakdown:` written
    under the invoice itself, was read by nothing: the record was answered
    `created` and the block went missing with nothing said.
    """
    line = DirectiveType.BILL_ENTRY if kind == 'bill' else DirectiveType.INVOICE_ENTRY
    for child in directive.children:
        if child.type not in (line, DirectiveType.POSTED, DirectiveType.PAYMENT):
            raise ValueError(
                f'a `{child.line.strip()}` block is not read under '
                f'{"a bill" if kind == "bill" else "an invoice"}, only `entry:`, '
                f'`posted:` and `payment:` blocks are')


#: The place of each block inside an invoice or bill: lines, then the posting, then payments.
_BLOCK_ORDER = {
    DirectiveType.INVOICE_ENTRY: 0,
    DirectiveType.BILL_ENTRY: 0,
    DirectiveType.POSTED: 1,
    DirectiveType.PAYMENT: 2,
}


def _refuse_blocks_out_of_their_order(directive, kind: str) -> None:
    """Refuse an invoice's or bill's blocks written out of their order.

    README states one order: the `entry:` blocks, then `posted:`, then the
    `payment:` blocks. The import reads the blocks as written, posting at
    `posted:` and paying at each `payment:`, so a `posted:` written before an
    `entry:` posted the record before that line existed. Measured on 5.10, a
    100.00 invoice put 0.00 on the receivable and the run said `Errors: 0`.

    Asked after `_refuse_a_block_nothing_reads_under_the_record`, so every
    block is one of the three.
    """
    furthest = None
    for child in directive.children:
        rank = _BLOCK_ORDER[child.type]
        if furthest is not None and rank < _BLOCK_ORDER[furthest.type]:
            raise ValueError(
                f'a `{child.line.strip()}` block is written after '
                f'`{furthest.line.strip()}`. '
                f'{"A bill" if kind == "bill" else "An invoice"} writes its blocks '
                f'in this order: its `entry:` blocks, then `posted:`, then its '
                f'`payment:` blocks')
...
def _emit_orphan_warning_before_unpost(record, kind: str, ident: str,
    ...
    from use_cases.unpost_business_objects import (
        find_lot_payment_transactions,
        refuse_an_unpost_that_would_delete_a_transaction,
    )
    # Before anything is written, as `unpost-invoices` asks it: GnuCash would
    # delete a journal entry with the posting.
    refuse_an_unpost_that_would_delete_a_transaction(record, kind, ident)
    # Every caller is about to unpost a record the book has posted.
    mark_splits_orphaned_by_unpost(record)
    orphans = find_lot_payment_transactions(record)
    # Every command that can unpost passes a callback; only a library caller
    # leaves it out.
    if orphans and on_orphan_warning is not None:
...
def _require_the_account_can_hold_the_total(record, ar_ap_account, kind: str,
    ...
    raise Exception(
        ...
        f'Price the entries so the total lands on that unit, or set a finer '
        f'`commodity_scu:` on the account.')
...
def _attach_posting_rate(record, book, directive, entry_type, post_date,
    ...
    for entry_directive in directive.children:
        if entry_directive.type != entry_type:
            continue
        # Never None: this is called from the `posted:` block, every `entry:`
        # block comes before it (`_refuse_blocks_out_of_their_order`), and each
        # of those refused an account the book has not got.
...
def _the_customer_or_vendor_behind(lib, owner_ptr):
    """('customer'|'vendor', id) of the customer or vendor behind an owner, or None.

    A job's invoice, and the lot an unpost leaves on one, report the job, and a
    job is for a customer, whose money it is, as `export` writes it. Read as
    the job, every check comparing two owners answered nothing, and nothing is
    refused on silence. Measured on 5.10: C-OTHER's invoice stating the payment
    an unpost left on C-JOB's job invoice was settled out of it at exit 0, and
    the job's invoice was settled out of C-OTHER's payment the same way. An
    owner that is still neither a customer nor a vendor returns None.
    """
    end = lib.gncOwnerGetEndOwner(owner_ptr)
    kind = _OWNER_KINDS.get(lib.gncOwnerGetType(end))
    raw_id = lib.gncOwnerGetID(end)
...
def _the_customer_or_vendor_of(record):
    """`_the_customer_or_vendor_behind` the owner of an invoice or bill."""
    from infrastructure.gnucash.engine import load_gnc_engine

    lib = load_gnc_engine()
    return _the_customer_or_vendor_behind(
        lib, lib.gncInvoiceGetOwner(qof_pointer(record)))
...
def _owner_of_a_credit_beside(lib, split, transaction):
    ...
    deposit that is partly Alpha paying in advance and partly Beta's payment puts
    beside Alpha's credit lot a split that was never Alpha's, and Beta stating
    ...
    for index in range(transaction.CountSplits()):
        ...
        # refused to the owner whose money it may be, and credited to the other.
        # A split always has an account (CLAUDE.md finding 12).
        ...
        # And a posted record's lot is its record's, not an owner's.
        if (qof_pointer(lot) not in live_lots[account_ptr]
                or _gc.gncInvoiceGetInvoiceFromLot(qof_instance(lot))):
            continue
        buffer = ctypes.create_string_buffer(256)
...
def _lot_is_still_on_its_account(split, lot) -> bool:
    ...
    """
    # A split always has an account (CLAUDE.md finding 12).
    return qof_pointer(lot) in _live_lot_pointers(split.GetAccount())
...
def _recorded_owner_of(split):
    ...
    transaction is not asked behind it. A lot carrying no owner is one GnuCash's
    View → Lots makes ("New Lot" attaches none); `find-prepayments` warns about
    the ones nothing states an owner for, and a `lot_owner:` line states one's
    owner. Reading the transaction for such a split would answer from some
    *other* lot on it, which is the shared-deposit misattribution this function
    exists to avoid, so the silence stands.
    ...
    a file states a transaction and no split at all, the caller asks this of
    the split the retarget would move — `_refuse_another_owners_split` on the
    answer `_retarget_choices` returns — so one question is asked about one
    ...
    if lot is None:
        ...
        # ordinary shape a guid can be copied out of. A split is always in a
        # transaction.
        transaction = split.GetParent()
        business_splits = 0
        for index in range(transaction.CountSplits()):
        ...
        if lib.gncOwnerGetOwnerFromTxn(
          ...
          or lib.gncOwnerGetOwnerFromLot(ctypes.c_void_p(int(lot)), owner_ptr) != 1):
        # Membership first: a pointer the account no longer lists is a lot the
        # book has let go of, and asking it anything is a question about freed
        # memory (`_lot_is_still_on_its_account` says how a split came to hold
        # one). Same membership-before-dereference rule the retarget readers
        # follow, and this is reached ahead of them —
        # `_refuse_another_owners_split` runs first.
        return None

    return _the_customer_or_vendor_behind(lib, owner_ptr)
...
def _refuse_if_not_this_owner(record, theirs, kind: str, doc_id: str, what: str) -> None:
    ...
    if theirs is None:
        return
    mine = _the_customer_or_vendor_of(record)
    # Silence is not a refusal anywhere in this check: a record whose owner is
    # neither a customer nor a vendor says nothing about whose money the split
    # is.
...
def _refuse_a_payment_that_would_fall_short(md, carried: Fraction,
    ...
    A block stating only its transaction leaves the choice of split to the
    ...
    # Both figures are the record's own money. `amount:` is written in the
    # record's currency, and `carried` comes from `the_settlement_amount`,
    # which reads a split parked in another currency off the bank, restated in
    # the record's currency.
    ...
    raise Exception(
        ...
        + (f'That figure is what the bank received: the split {key} states '
           ...
           f'The split {key} states is the one carrying that figure, so '
           f'state what really settles this {kind.lower()} now, or state the '
           f'guid of the split that does.'
           if names_its_split else
           f'State in {key} the guid of the split meant, or state what '
           f'really settles this {kind.lower()} now.'))
...
def _why_nothing_can_move(transaction, bank_acct_name: str, txn_guid: str,
    ...
    return Exception(
        ...
        f'taking one would leave that unpaid with no figure disagreeing. State '
        f'the guid of a transaction with money that settles nothing yet, or, '
        f'if one of these splits really belongs to this {kind}, state that '
        f'split\'s own guid in `txn_split_guid:` and unpick what it is '
        f'settling now.')
...
def _retarget_choices(transaction, bank_acct_name: str, own_lot, record):
    ...
    def placeable():
        return _placeable_lotted_splits(transaction, bank_acct_name, own_lot)
...
def _placeable_lotted_splits(transaction, bank_acct_name: str, own_lot):
    ...
    cost basis was consumed, and with it the gain realized.
    """
    mine_ptr = (qof_pointer(own_lot)
                if own_lot is not None else None)
    live_lots = {}
    ...
    for split in transaction.GetSplitList():
        ...
        if account_ptr not in live_lots:
            live_lots[account_ptr] = _live_lot_pointers(account)
        # A lot the account no longer lists was destroyed and holds no invoice or bill, so
        # it is placeable, and it is asked first: a pointer the book has freed
        # is not one to hand to the engine.
        if (lot_ptr not in live_lots[account_ptr]
                or not gc.gncInvoiceGetInvoiceFromLot(qof_instance(lot))):
...
def _sits_in_an_owners_credit(split) -> bool:
    ...
    than a split left carrying a cost basis for currency it no longer holds.
    """
    lot = split.GetLot()
    if lot is None:
        return False
    if is_a_bank_paid_orphan(split):
        ...
        # step and reachable, and calling it a credit there strips the cost basis
        # off currency the bank still holds and exports a block that named an
        # account and a date as `from_credit:` carrying neither.
        return False
    # A split always has an account (CLAUDE.md finding 12).
    account = split.GetAccount()
    # Membership first, so a lot the book has let go of is never asked for
    # its invoice.
    if (qof_pointer(lot) not in _live_lot_pointers(account)
            or gc.gncInvoiceGetInvoiceFromLot(qof_instance(lot))):
        return False
...
def _lot_membership_unchanged():
    ...
    _LIVE_LOT_MEMO = {} if outer is None else outer
    try:
        yield
    finally:
...
def _refuse_an_ambiguous_retarget(bank_acct_name: str, txn_guid: str,
    ...
    A block stating only a transaction is retargeted by moving the side that is
    ...
    the caller and passed to both this and the mover, so the two cannot disagree
    ...
    costs, so split order would decide which cost basis was consumed and with it the
    gain realized.
    """
    if len(candidates) < 2:
        return
    raise Exception(
        ...
        f'{kind.lower()}, and this block states only the transaction — which of '
...
def _refuse_a_split_settling_another_record(record, split, kind: str,
    ...
    Stating the split outright reached it unguarded, and this change set is what
    ...
    states the split already in it, and a rebuild re-attaches its own
    ...
    if mine is not None and qof_pointer(mine) == qof_pointer(lot):
        return
    raise Exception(
        f'{kind} {doc_id}: the split txn_split_guid states, {named!r}, is in '
...
def _refuse_an_amount_the_split_does_not_carry(pay_dir, split, record, kind: str,
    ...
    """Refuse a block whose `amount:` is not what the split it states carries, where that split is attached whole.

    A split already on the record's own receivable or payable, stated by
    `txn_split_guid:` with no `prepayment:` beside it, is attached whole. So
    a block stating another amount makes the file mean two things. In the book
    ...
    raise Exception(
        ...
        f'{pay_dir.metadata.get("amount")}, and the split txn_split_guid states, '
...
def _apply_credit_payment_directive(record, pay_dir, book, is_bill) -> None:
    ...
    date of its own: it states the split (`txn_guid:` / `txn_split_guid:`), how
    ...
    # A `Transaction`, an account or a date beside `from_credit:` is refused
    # by `_refuse_a_credit_block_stating_a_transaction` and
    # `_refuse_a_credit_block_stating_an_account_or_a_date`, which every path
    # here goes through: both are asked of the whole directive at the top of
    # `import_invoice` and `import_bill`, ahead of the comparison. Asked again
    # here they could never fire — an unchanged record does not reach this
    # function at all, which is the reason the checks moved out.
    ...
    if not txn_guid or not split_guid_declared:
        raise Exception(
            f'{kind} {doc_id}: a payment with `from_credit: true` must state '
            f'the credit it applies, in `txn_guid:` and `txn_split_guid:`. '
    ...
    # A credit is applied only to a posted record, and posting makes its lot:
    # `import_invoice` and `import_bill` refuse a `payment:` block on one the
    # file leaves unposted.
    lot = record.GetPostedLot()
    post_acct = record.GetPostedAcc()
    ...
    # Parses: `_refuse_a_payment_guid_nothing_can_parse` has read it already.
    ...
    if (amount > 0) != bool(is_bill) or amount == 0:
        raise Exception(
            f'{kind} {doc_id}: the split txn_split_guid states carries '
    ...
    with _lot_membership_unchanged():
        ...
        if (existing_lot is not None
            ...
            raise Exception(
                f'{kind} {doc_id}: the split txn_split_guid states is already '
        ...
        if is_a_bank_paid_orphan(target_split):
            raise Exception(
                f'{kind} {doc_id}: the split txn_split_guid states is a '
                ...
                f'and state the transaction\'s guid in `txn_guid:` to attach it as '
    ...
    if abs(amount) > outstanding:
        ...
        if target_split.GetLot() is None:
            raise Exception(
                f'{kind} {doc_id}: the credit txn_split_guid states is in no '
                f'lot, so nothing in the book says whose it is — and it is '
                f'bigger than this {kind.lower()}, so what is left of it would '
                f'be parked as a credit for an owner nobody can confirm. State '
                f'the credit\'s owner first, with `lot_owner:` on that split, '
...
def _the_transaction_this_record_orphaned(bank_account, record_guid: str,
    ...
    block has to be matched to its own. `pay_dir` is what says which: the figure it
    ...
    Nothing where nothing matches. A bare retarget cannot arrive here — both
    callers ask only when the block `describes_itself`, which needs a date, an
    ...
    Answering None sends it where it was going: a guid that resolves to
    nothing goes to the duplicate check and then to recording the payment the
    block describes, and a block stating no guid is recorded as a payment.
    ...
    which is every block of a printed page read anywhere but its own book, and
    for every block stating no guid, and it walks the whole bank account each
    time.
    """
    # `record_guid` is never empty: both callers pass the record's own, and a
    # record the book holds has one.
...
def _settles_another_owners_record(record, transaction) -> bool:
    ...
    memo the bank line carried. That happens between two customers and it
    ...
    A job's invoice is the customer's the job is for. False on silence, as
    every owner check here is: a record whose owner is neither a customer nor
    a vendor says nothing, and money in nobody's lot is what the guard is for.
    """
    mine = _the_customer_or_vendor_of(record)
    if mine is None:
        return False
    ...
    if other is None:
        return False
    theirs = _the_customer_or_vendor_of(other)
    if theirs is None:
        return False
    return theirs != mine
...
def _a_transaction_matching_the_payment_block(book, pay_dir, bank_account,
    ...
    three the block itself carries, and a retarget block states them precisely
    ...
    # Both are there. This is asked only of a block that describes itself — a
    # date, an amount and an account — and `_bank_side_figure_of` answers None
    # only where the block states no `amount:`.
...
def _apply_the_payment_directive(record, pay_dir, book, is_bill, fx_rates=None):
    ...
    # What the block states, read once into a local. The reattach below finds
    # the record's own settlement and the split the file states goes with the
    ...
    if (txn_guid and describes_itself
        ...
        # settlement that is (CLAUDE.md finding 10); stating its transaction
        ...
        if already is not None:
            ...
            txn_guid = ''
            named_split = ''
            # As above: the splits the block states belong to the transaction the
            # guid pointed at, and there is no such transaction here.
            block_splits = []
    elif not txn_guid and describes_itself:
        ...
        #
        # The same search the guid that matches nothing gets: a split this
        # record's unpost marked, on the day and for the figure the block
        # states. A block describing any other movement matches nothing and is
        # recorded as before.
        #
        # The memo is read first, as the payment made below reads it, so a
        # block with no `memo:` is refused the same way whichever it becomes.
        stated_memo = pay_dir.metadata['memo']
        record_guid = _swig_invoice_guid_str(record)
        mine = _the_transaction_this_record_orphaned(bank_account, record_guid,
                                                     pay_dir)
        if mine is not None:
            _echo_note(f'note: {kind_of(record)} {record.GetID()!r}: the '
                       f'payment of {pay_dir.metadata["date"]} is the settlement '
                       f'its unpost left, {mine} — put back rather than paid '
                       f'again')
            # And it takes what the block states of it. The day and the figure
            # found it, and the memo and the number are what a person edits:
            # a paid invoice whose memo is corrected is rebuilt, and the
            # payment made by the rebuild carried the new memo. Put back
            # unedited, the book kept the old one, and the next import read
            # the block as changed again.
            #
            # Written on the split the unpost marked, which is the one the
            # retarget below moves into the lot. Two splits marked for this
            # record are refused there as ambiguous, before anything is saved.
            put_back = _find_transaction_by_guid(book, mine)
            marked = next(split for split in put_back.GetSplitList()
                          if _orphaned_from(split) == record_guid)
            _write_the_payment_memo(
                put_back, marked, stated_memo,
                _its_counterpart(put_back, marked, pay_dir.metadata))
            # Absent is none, as it is for the payment made below: a block
            # stating no guid states the payment whole.
            number = pay_dir.metadata.get('num', '')
            if (put_back.GetNum() or '') != number:
                put_back.BeginEdit()
                put_back.SetNum(number)
                put_back.CommitEdit()
            txn_guid = mine
    if txn_guid:
        ...
        # once, each block stating its own portion, and asking the transaction
        ...
        #
        # Inside the branch that uses it, and not above: a block stating its
        # split needs none of this, and every block a book's own export writes
        # states one. Worked out unconditionally, re-importing an export paid
        # for that walk on every payment block and threw the answer away.
        choices = []
        # Nor where a `Transaction` block states its splits: the ambiguity this
            ...
            with _lot_membership_unchanged():
                ...
                if moving is not None:
                    _refuse_another_owners_split(
                        ...
                        record.GetID())
        post_acct = record.GetPostedAcc()
        # A payment is applied only to a posted record, and posting makes its
        # lot: `import_invoice` and `import_bill` refuse a `payment:` block on
        # one the file leaves unposted.
        lot = record.GetPostedLot()
        from infrastructure.gnucash.engine import load_gnc_engine
        lib = load_gnc_engine()
        ...
        if declared_split_guid:
            # Parses: `_refuse_a_payment_guid_nothing_can_parse` has read it.
            ...
            target_acct = target_split.GetAccount()
            # Whether the split still has to be moved to the receivable.
            ...
            with _lot_membership_unchanged():
                ...
                if not_there_yet:
                    refuse_to_move_a_split_out_of_its_lot(
                        target_split, declared_split_guid, txn_guid, record,
                        split_key)
            if not_there_yet:
                bank_split = the_split_on(existing_tx, bank_acct_name)
                refuse_when_the_amount_cannot_be_read(
                ...
                # Asked nowhere, stating the split let a 120.00 receipt settle
                ...
                owed_now = _still_owed(record, lot, post_acct)
                # Never None here: `refuse_when_the_amount_cannot_be_read`,
                # just above, turns away both shapes `the_settlement_amount`
                # has no figure for.
                weighed = the_settlement_amount(
                    existing_tx, target_split, post_acct, bank_acct_name)
                # This spelling already states its split, so the remedy the
                # other one ends on — stating the guid of the split meant —
                # would be circular here, and would point at a key a
                # `PaymentSplit` writer's file does not contain.
                _refuse_a_payment_that_would_fall_short(
                    ...
                    # split — so telling the reader the split stated
                ...
                # `carves=False`: this branch relinks the split stated and
        ...
        if counter_amount_abs is not None and \
            ...
            # past zero — `txn_split_guid:` states a split outright and attaches
        ...
        # that states the split outright does. Same two refusals first, and both
    ...
    #
    # A block stating a transaction states this record's own *slice* of it —
    ...
    # what that movement left over. A block stating none states the payment to
    ...
    # dollars produced amount 18 against value 18.19 on a same-currency split,
    # and the export wrote `amount: 18` back — the account arm of the check
    # exists for exactly that and never ran here.
    #
    # The figure paid is the money this reads, not a quantity: an amount is
    # held at its currency's unit, where a line's quantity or price is a ratio
    # read exactly. A bill passes it negated (CLAUDE.md finding 7).
    _posted = record.GetPostedAcc()
    paid = stated_money(
        amount_str, record.GetCurrency(),
        f'the payment amount on this {"bill" if is_bill else "invoice"}',
        scu=_posted.GetCommoditySCU() if _posted is not None else None)
    amount = paid.neg() if is_bill else paid
...
def _carried_cost_of(record):
    ...
    return next(cost_of(split)
                for split in record.GetPostedTxn().GetSplitList()
...
def _check_declared_prepayment(record, pay_dir, lot_before, kind) -> None:
    ...
    # Asked right after `ApplyPayment` on a posted record, which always has
    # its account and its lot.
    posted_account = record.GetPostedAcc()
    lot = record.GetPostedLot()
...
def _record_overpaid_basis(record, lot_before) -> None:
    ...
    # payment open a phantom cost basis on the first invoice's settling split, and
...
def _still_owed(record, lot, post_account) -> Fraction:
    ...
    """
    posting_guid = record.GetPostedTxn().GetGUID().to_string()
    # What the posting actually put on the account, which on one kept coarser
    # than the record's total is not that total.
    ...
    def _reduction(split):
        # A split is always in a transaction.
        if split.GetParent().GetGUID().to_string() == posting_guid:
...
def _posting_amount_of(record, post_account) -> Fraction:
    ...
    Asked only of a posted record, with the account it posted to. Posting
    makes the posting transaction, the lot and the split on that account
    together, so the split is always there to read.
    """
    posted_name = get_account_full_name(post_account)
    return next(numeric_to_fraction(split.GetAmount())
                for split in record.GetPostedTxn().GetSplitList()
                if get_account_full_name(split.GetAccount()) == posted_name)
...
def _lot_transaction_guids(record) -> set:
    ...
    """
    # Asked only before a payment is applied, of a posted record, which always
    # has its lot; and a split is always in a transaction.
    return {Split(instance=raw).GetParent().GetGUID().to_string()
            for raw in record.GetPostedLot().get_split_list()}
...
def holdable_unit(commodity, scu: int) -> int:
    ...
    Never asked of an account with no commodity: a transaction with a split
    on one is refused before any figure on it is judged, a payment through one
    is refused before its figures are read, and `import-beancount` refuses an
    `open` line that states no currency.
    """
...
def stated_money(text, commodity, what: str, scu: int = None) -> GncNumeric:
    ...
    *stored* at, and the figure is judged against whichever of the two is
    ...
    # thousands, and read as a point `value: "1,350"` booked 1.35 CAD with
    # the other 1348.65 in Imbalance-CAD and `Errors: 0`.
    raw = str(text).strip()
    try:
        value = Fraction(raw)
    except (ValueError, ZeroDivisionError) as exc:
    ...
    # The commodity is always there: `holdable_unit` below dereferences it
    # unconditionally, and so does the refusal further down. A guard here as
    # well said something the next line contradicts, which is the drift
    # `holdable_unit`'s own docstring was written to stop — and a
    # short-circuited `and` records no arc, so branch coverage would not have
    # caught it.
    is_currency = (commodity.get_namespace() or '').upper() == 'CURRENCY'
    ...
    if (value * holdable).denominator != 1:
        ...
        raise Exception(
            ...
            + (f'that currency: its smallest unit is '
               ...
               f'set a smaller unit on the account — `commodity_scu:` in '
...
def _basis_relevant_accounts(existing_tx) -> set:
    ...
    A cost basis rests on its own split — the amount that arrived, the value it
    ...
    (`_base_per_unit_of`). Each of those is then a figure the cost basis rests on.

    A split that is neither is the other side of the transaction, and moving
    it cannot change what any cost basis here holds or what it cost. Comparing every
    ...
    so `cost_of` returns the same figure before and after and the cost basis cannot
...
def _is_a_base_currency_account(root, name: str) -> bool:
    ...
    """
    # Always a name, the parser refusing a split line with no account, and
    # always an account: `update_transaction` refuses one the book has not got
    # before `_require_no_cost_basis_edit` reads the block.
    account = find_account(root, name)
    commodity = account.GetCommodity()
...
def _is_a_foreign_currency_account(root, name: str) -> bool:
    ...
    `Assets:Bank:EUR 60.00 EUR` to a CAD-stated purchase holding a USD cost basis:
    ...
    through. No cost basis opened for those euros — currency in the book reading
    `none recorded`, which nothing can sell and `--verify-costs` leaves out of
    both sides of its totals.
...
def _basis_figures_in_book(existing_tx, relevant):
    ...
    split is on, the amount it moves, the value it moves, and the cost basis it
    picks. Memos, descriptions, actions and dates are absent because none of
    them can change what a cost basis holds or what it cost, and so are the splits
    `relevant` leaves out, for the reason stated there."""
    rows = []
    for split in existing_tx.GetSplitList():
        account = split.GetAccount()
        name = get_account_full_name(account) if account is not None else ''
...
def _basis_figures_in_directive(directive, booked, relevant, book, priced_by_cad,
    ...
    disposal written onto an account no cost basis rested on is still a disposal,
    ...
    for child in directive.children:
        ...
        if raw_amount == RESIDUAL_AMOUNT:
            ...
            return None
        try:
            # A point, never a comma, as `stated_money` reads a figure.
            amount = Fraction(raw_amount)
        ...
        if stated_value is None:
            ...
            try:
                ...
                return None
        # The rate counts as much as the value. Where the block states no
        # value the update path applies a stated `share_price:` and GnuCash
        # recomputes value = amount × price, so editing `share_price:` alone
        # moves the figure `cost_of` derives a basis from — 1.35 re-priced to
        # 2.00 CAD/USD while the guard saw nothing move. Where it states both
        # amounts, those set the price and a `share_price:` beside them is
        # not applied, so the rate is theirs: value over amount, as
...
def _what_an_edited_split_spends(root, child, existing_tx, when):
    ...
    either stating no guid and go through. None for anything else — a split in
    the book's own currency, one on an income or expense account, one bringing
    units in, a `$residual$` split, and one stating a `cost_basis_split_guid:`,
...
def _refuse_an_edit_that_adds_a_disposal(existing_tx, directive, book) -> None:
    ...
    A new transaction doing that is refused unless it states the cost basis it
    ...
    bank fee edited into 100.00 US dollars spent, stating no guid, was accepted,
    ...
    for child, found in read:
        ...
        raise Exception(
            ...
            f'transaction and import it afresh, with `{COST_BASIS_SPLIT_KEY}:` '
...
def _require_no_cost_basis_edit(existing_tx, directive):
    ...
    400.00 USD against a cost basis holding 60.00, reported `Updated: 1`, and left
    the cost basis still reading 60.00.

    Deleting the transaction and importing it afresh is the route that works,
    and reaches the same end state: deleting a sale puts back on the cost basis
    exactly what that sale drew, and the new import runs every check.
    ...
    **`--atomic` defers most of this and not all of it.** An atomic run does
    ...
    book it left, before saving anything, which is what repairing a cost basis
    ...
    What the finished book cannot answer is how much a disposal took. A cost basis
    ...
    against what its cost basis holds — and the balance afterwards reads the same
    ...
    # block with none is dropped by GnuCash, and the book never holds it.
    book = next(split.GetAccount().get_book()
                for split in existing_tx.GetSplitList())
    incoming = _basis_figures_in_directive(
        ...
        already_used={get_account_full_name(split.GetAccount())
                      for split in existing_tx.GetSplitList()
                      if split.GetAccount() is not None})
    # The currency the transaction is stated in prices a cost basis as much as any
    ...
    # `currency.mnemonic:` from USD to CAD re-priced this fixture's cost basis from
    ...
    # finished book can be asked about, and a re-priced cost basis is caught there
    # only by the sales measured against it: those are compared in the book's
    # own currency, so a cost basis with no such sale under it moves at no cost to
    # any check.
...
def _the_figures_can_be_deferred(before, after, currency_moved: bool) -> bool:
    ...
    split, the cost basis it states, what it draws and its value — because a
    ...
    return (deferring_edits_in_place() and not currency_moved
...
def _defer_to_the_finished_book(book, existing_tx) -> None:
    ...
    raise Exception(
        ...
        f'on the cost basis exactly what was drawn from it, and the fresh '
...
def _an_edit_moving_a_cost_basis(book, existing_tx, rows, before, after,
    ...
    **And a route that can be taken.** A transaction whose cost basis
    ...
    printed deletes those first and then it, in one run, which is the order
    ...
    finely its currency then divided, `before` holds its cost bases, and
    ...
    # run would have deferred is not stated as one.
        ...
        if key[0] == 'basis':
            if new is None:
                said.append(f'cost basis {key[1]} on {old[2]!r} would be gone — '
                            f'every split drawing on it would draw on a cost basis '
                            f'the book no longer holds')
            else:
                if old[2] != new[2]:
    ...
    route = (
        ...
        f'deletes those first and puts back on each cost basis what they '
        f'drew from it, and the fresh import checks every figure.')
...
def _refuse_a_new_version_that_does_not_balance(transaction, leaving) -> None:
    ...
    if values:
        ...
        raise Exception(
            ...
            f'or write `$residual$` as the amount of one split to take the '
            f'difference.')
...
def _own_cost_bases_another_transaction_draws_on(book, transaction, before) -> set:
    ...
    Under `--atomic`, of the book the file leaves: a split whose transaction
    this file restates draws on what the file's version of it states (Q-053).
    """
    ...
    return {fact[1] for fact in before if fact[0] == 'basis'
            and any(split.GetParent().GetGUID().to_string() != here
                    and _still_draws_once_the_file_is_applied(split, fact[1])
                    for split in splits_drawing_on(book, fact[1]))}


def _the_transactions_the_file_draws_on_it_with(directive) -> set:
    """Under `--atomic`, the transactions whose version in this file draws on a split of this block's.

    They come after it, whatever the book holds of them when the block is
    applied: a fee the file states drawing on a deposit's cost basis was spent
    out of what the deposit brought in (Q-053). So the account's balance the
    deposit is read against leaves them out, as it leaves out what draws on
    it in the book. Counted in, the fee's version booked onto an invoice,
    still in the book when the deposit is put back, read the deposit as
    repaying 0.72 USD and bringing in 2,719.28.
    """
    if not running_atomic():
        return set()
    itself = the_guid_a_block_names(directive.metadata)
    drawing = _what_the_file_draws_on_each_split()
    # Its own splits drawing on each other are already taken off with it.
    return {transaction for guid in _SPLITS_THE_FILE_STATES_OF_EACH_TRANSACTION.get(itself, ())
            for transaction in drawing.get(guid, ()) if transaction != itself}


def _what_the_file_draws_on_each_split() -> dict:
    """Each split guid the file's blocks draw on, to the transactions whose blocks draw on it.

    Built once per run, the first time it is asked, and not as the file is
    read: under `--strategy update` a position is replaced with the guid it
    points at before any transaction is edited, and the first edit that asks
    comes after that. Asked for each edit touching a cost basis, a walk of
    every split the file states made a large update file quadratic.
    """
    global _WHAT_THE_FILE_DRAWS_ON
    if _WHAT_THE_FILE_DRAWS_ON is None:
        _WHAT_THE_FILE_DRAWS_ON = {}
        for split, transaction in _DRAWS_THE_FILE_STATES.values():
            drawn = _the_draw_a_split_block_states(split)
            if drawn is not None:
                _WHAT_THE_FILE_DRAWS_ON.setdefault(drawn, set()).add(transaction)
    return _WHAT_THE_FILE_DRAWS_ON


def _still_draws_once_the_file_is_applied(split, basis_guid: str) -> bool:
    """Whether the split draws on the cost basis in the book the file leaves.

    Under `--atomic` the book is checked once the file is applied, so a split
    whose transaction the file restates draws on what the file's version of
    it states, and on nothing where that version leaves the split out. A fee
    booked onto the invoice its deposit collected then no longer holds the
    deposit's cost basis in place, though the book still has the fee's old
    version when the deposit is edited. Without `--atomic` a refused block
    leaves its old version saved, so the book is asked as it stands.
    """
    if not running_atomic():
        return True
    if split.GetParent().GetGUID().to_string() not in _TRANSACTIONS_THE_FILE_RESTATES:
        return True
    stated = _DRAWS_THE_FILE_STATES.get(split_guid(split))
    return stated is not None and _the_draw_a_split_block_states(stated[0]) == basis_guid
...
def _what_the_book_holds_apart(transaction, taken: Dict[str, Fraction]) -> dict:
    ...
    `what_a_new_import_would_open` returns, less what the transaction's own
...
def _put_back_what_the_old_version_drew(book, taken: Dict[str, Fraction],
                                        put_back: Dict[str, Fraction]) -> None:
    """Put back, once the edit is committed, what the old version drew.

    Records in `put_back` what was put back, which is what a refused new
    version draws again when the old one is restored. Deleting a transaction
    puts back on each cost basis what its splits drew, and an edit read as a
    new transaction does the same before the new version draws.

    Each cost basis is recorded as soon as its amount is back, in the
    caller's own dict. Returned at the end instead, putting back failed on
    its second cost basis, over a `cost_basis_brought_in` that will not
    parse, and left the caller holding nothing: the first kept what was put
    back on it, and the saved book offered it twice.

    A balance this file states is left as the file states it, and nothing is
    put back on it: it is stated net of the file's own disposals, and the
    new version's are among them.
    """
    # Only on a cost basis holding a stored balance. One holding none is read
    # as holding nothing, whether it never had one or the file cleared it a
    # block earlier, and putting an amount back on it would open it at everything its
    ...
    for guid, amount in taken.items():
        ...
        if (basis is not None and has_cost_basis_balance(basis)
            ...
            put_back_on_cost_bases(book, {guid: amount})
            put_back[guid] = amount
...
def _clear_what_the_old_version_opened(book, transaction, before, apart: dict) -> set:
    ...
    Apart from putting back, so a failure here still leaves the caller
    holding what was put back, to draw it again when the old version is
    restored.
    """
    ...
    for guid in sorted(cleared):
        ...
        if held < 0:
            ...
            raise Exception(
                ...
                f'{money_text(-apart[guid], unit)} less than its figures add up '
                f'to, which a file stated as sold outside the book. So the '
...
def _put_the_old_version_back(book, transaction, stood: dict,
                              put_back: Optional[Dict[str, Fraction]] = None,
    ...
    commit. What it drew is put back, what was put back of the old
    version's draws on cost bases outside it is drawn again, and every field
    ...
    were before the edit put anything back.
    """
    put_back_on_cost_bases(book, taken_after or {})
    own = {each['guid'] for each in stood['splits']}
    for guid, amount in (put_back or {}).items():
...
def _splits_in_lot(record):
    ...
    the book records.
    """
    # Asked only before the owner's credit is applied, which both import paths
    # refuse for an unposted record, and a posted record always has its lot.
...
def _mark_applied_from_credit(record, lot_before) -> list:
    ...
    """
    # Asked only after the owner's credit is applied, which both import paths
    # refuse for an unposted record, and a posted record always has its lot.
    lot = record.GetPostedLot()
    marked = []
    for raw in lot.get_split_list():
        ...
        if is_a_bank_paid_orphan(split):
            ...
            # itself an orphan.
            #
            # A settlement holds no cost basis either, whatever brought it here. No
...
def _basis_splits_on(account):
    ...
    A split carrying cost basis keys has a balance and a cost to move. A bank-paid
    orphan usually carries neither — the cost basis of a settled invoice sits on
    ...
    with neither key on it and a sale still stating its guid. Filtered on the
    ...
    """
    # The posted account of a record whose owner's credit is being applied,
    # which both import paths refuse for an unposted record.
    found = {}
...
def _carry_basis_across_applied_credit(record, before, existed_before=frozenset()) -> None:
    ...
    Both halves are put right here: the applied part loses its keys, and
    ...
    """
    # The posted account of a record whose owner's credit is being applied, and
    # `before` is every split it held, the posting's among them, so never empty.
    account = record.GetPostedAcc()
    after = {split_guid(split): split for split in account.GetSplitList()}

    for guid, (old_amount, metadata) in before.items():
        # Still there: applying a credit reduces a split and carves a new one,
        # and destroys none. Measured with both halves of one divided credit
        # applied back into one invoice's lot on 5.10 and 3.8, where the two
        # stayed two splits.
        split = after[guid]
        ...
        # empty frame, carrying neither. So the only split with any keys to
        # remove is the applied one, handled below; the remainder is written
        ...
        for sibling in transaction.GetSplitList():
            ...
            if sibling_guid in existed_before:
                ...
                # a settlement written by an earlier payment carries no cost basis
                ...
                # from the listing, and refused as a cost basis.
                continue
            # So this is the remainder: the one split on the account the
            # engine's carve made. Measured across the whole suite on every
            # supported build, applying a credit makes exactly one. The only
            # other ever seen was the split 3.8, 4.4 and 4.13 add when they
            # value a foreign credit at par (CLAUDE.md finding 19), and a credit
            # valued in another currency is spent before the engine is asked.
            ...
            # A cost basis moves only where the source had one. A split reaches
            ...
            if (COST_BASIS_BALANCE_KEY in metadata
                ...
                if COST_BASIS_COST_KEY in metadata:
                    ...
                    set_custom_metadata(sibling, sibling_meta)
                if unreadable:
                    # A balance that will not parse goes across as it reads,
                    # as `_carry_basis_to_residue` carries it when this tool
                    # divides the credit. Read as absent, the whole remainder
                    # was written over it: `20,00` came out as a clean
                    # `60.00`, and `--verify-costs` had no text left to report.
                    sibling_meta[COST_BASIS_BALANCE_KEY] = raw
                    set_custom_metadata(sibling, sibling_meta)
                else:
                    # Through the same writer every balance goes through, so
                    # it lands at this split's own smallest unit and keeps the
                    # keys already on it.
                    write_cost_basis_balance(sibling, available)
            # And what was already measured against this credit is measured
            # against the remainder now. The guid the disposals state is the
            # applied part's, which lost its cost basis keys a few lines above
...
def _check_payment_split_lines(book, pay_dir, kind: str, bank_currency: str):
    ...
    No key states an account and nothing is configured: the split says where
...
def _check_stated_balances(book, directive) -> None:
    ...
    sale is measured against it, and valued at the cost basis cost. That makes it
    ...
    accounted for, while the cost basis, having no readable balance, was opened at
    ...
    for child in directive.children:
        ...
        # that line carries — and says nothing, because no cost basis lives on a
        # split in the book's own currency. The cost basis it was meant for is then
        ...
        if balance < 0:
            raise Exception(
                f'{where} is negative: {text} — a balance falls only by what a '
                f'sale draws down, and rises only by what an undone sale puts '
                f'back, so it is never below nothing')
        ...
        if amount_text == RESIDUAL_AMOUNT:
            # What the other splits leave over. `$residual$` states no figure
            # and the split still brings one in, so read off the line's text
            # the check had nothing to compare: 1500.00 stated on a split
            # taking 1000.00 USD was stored, and `fx-balances` offered
            # 1,500.00 USD (measured on 5.10). A residual is in its account's
            # own currency or the transaction is refused, so that currency is
            # the one to resolve it in.
            amount_text = _resolve_residual(directive, account_commodity, root)
        try:
        ...
        if balance > acquired:
            raise Exception(
                ...
                f'that split brings in {money_text(acquired, unit)} — a cost basis '
...
def _check_stated_costs(book, directive, existing_tx=None) -> None:
    ...
    rewritten and its cost basis unopened.
    ...
    # account sets the rate. The split's figure itself is never missing: one
...
def _warn_of_a_price_other_than_value_over_amount(split_directive, amount, value,
                                                  where: str) -> None:
    """Say so where a split states a `share_price:` beside both amounts that is not its value divided by its amount.

    GnuCash's transfer dialog takes one amount and then either the rate or the
    other amount. A block may state all three, and every export does, writing
    the price as information. Where it does, the two amounts decide: the price
    held is the value over the amount. A stated price that differs is not
    refused, because the two amounts say what moved, but it is not what the
    book holds either, and the reader is told.
    """
    stated = numeric_to_fraction(_stated_rate(
        split_directive.metadata['share_price'], f'the share_price on split {where!r}'))
    amount_held, value_held = numeric_to_fraction(amount), numeric_to_fraction(value)
    if stated * amount_held == value_held:
        return
    quotient = exact_text(value_held / amount_held) if amount_held else 'no price'
    _echo_note(
        f'warning: the share_price on split {where!r} states '
        f'{split_directive.metadata["share_price"]}, and its value divided by '
        f'its amount is {quotient}: the price is that division')
...
def _settlement_transaction_in(record, lot_before):
    ...
    """
    # Asked only after a payment is applied to a posted record, which always has
    # its lot; and a split is always in a transaction.
    for raw in record.GetPostedLot().get_split_list():
        parent = Split(instance=raw).GetParent()
        if parent.GetGUID().to_string() not in lot_before:
            return parent
...
def _refuse_a_payment_block_spending_a_cost_basis_balance(record, bank_account, kind, lot_before):
    """A payment block cannot spend foreign cash a cost basis still has a balance for.
    ...
    Allowed where the account's cost bases have nothing left: there is nothing to
    ...
    disposal uses — an ordinary transaction whose bank split states its cost basis's guid,
    attached with `txn_guid:` / `txn_split_guid:`.
    """
    # A payment's account always has a commodity:
    ...
    if payment_txn is None:
        ...
        return
    account_name = get_account_full_name(bank_account)
    # The bank split `ApplyPayment` wrote, which is always there. No
    # null-account guard. Nothing `ApplyPayment` writes lacks an account, and
    # nothing loaded from a book can either — GnuCash 5.x drops such a
    # transaction while reading the file and 4.x segfaults on it (CLAUDE.md
    # §12), so the guard was agreeing with its sibling about a state neither
    # can meet.
    split = next(split for split in payment_txn.GetSplitList()
                 if get_account_full_name(split.GetAccount()) == account_name)
    spent = numeric_to_fraction(split.GetAmount())
    if spent >= 0:
        # Cash arriving, which opens a cost basis rather than drawing one
        # down. `record_cost_bases` records a cost basis balance on it.
        return
...
def _book_payment_fx_difference(record, book, pay_dir, bank_account, is_bill,
    ...
    # Both are there: a posted record has its currency, and a payment's account
    # has its commodity (`_refuse_a_payment_account_with_no_commodity`).
    record_currency = record_commodity.get_mnemonic()
    ...
    # `derived_cost_of` answers 1 rather than None and the cost basis bail-out
    ...
    # Asked right after `ApplyPayment` on a posted record, which always has its
    # account and its posting.
    posted_account = record.GetPostedAcc()
    posting_txn = record.GetPostedTxn()

    # Posting puts a split on the account the record posts to, so the posting
    # always has one.
    basis_split = next(
        split for split in posting_txn.GetSplitList()
    ...
    # The payment `ApplyPayment` just made: a split on the bank and one on the
    # posted account, which an overpayment divides into the part in the
    # record's lot and a credit in a lot of its own. Nothing else is on it —
    # the split lines this block states are placed further down — and
    # measured on 5.10 and 3.8, paying beside a credit the owner already held
    # left that credit in its own lot and out of this transaction.
    settled_splits = []
    bank_split = None
    for split in payment_txn.GetSplitList():
        if get_account_full_name(split.GetAccount()) == get_account_full_name(posted_account):
    ...
    if len(settled_splits) > 1:
        ...
        for split in settled_splits:
            if split_guid(split) not in in_lot:
                ...
                settled_split = split
    # Both are found: the payment joined the record's lot through its split
    # on the posted account, and `ApplyPayment` writes the bank's split beside
    # it.
    ...
    # exactly even when the cost basis cost does not divide into cents.
    ...
    # settlement drew, and
    # nothing on this path puts it back — the transaction path pairs its
    # drawdown with `put_back_on_cost_bases`, and here the answer is to have
    # nothing to put back. The checks need only the block and the account
    ...
    # skipped — leaving the cost basis untouched there would keep offering USD that
    ...
    # book now holds still needs a cost basis. Returning here skipped all three
    ...
    # commodity, so a CAD bank split is not a cost basis at all.
    record_cost_bases(book, payment_txn)


def _draw_down_settled_basis(basis_split, units, kind, record_currency):
    """Take what a settlement converted out of the cost basis's balance.

    Refused rather than driven negative when the cost basis has less left than the
    ...
    if available is None:
        raise Exception(
            ...
            f'so settling this {kind} cannot draw it down — to record one, '
            f'state `{COST_BASIS_BALANCE_KEY}:` on that split in an import file')
    if units > available:
...
def _payment_exchange_rate(record, bank_account, pay_dir, is_bill):
    ...
    - `settled_amount: 137.00` — what actually arrived in (or left) the
      account. This is the figure a bank statement shows, and the rate is
      derived from it;
    ...
    Both may be stated only if they agree. One of them is required when the
    ...
    # The three figures are numbers: `_refuse_a_payment_figure_nothing_can_read`
    # refused anything else before the record was compared.
    ...
    if declared_settled:
        ...
        # only for "parses" and "positive", `137.005` into a CAD bank produced a
    ...
    if declared_rate:
        ...
        # sentence continues "states <figure>", and one ending in a verb printed
...
def _invoice_non_payment_matches(invoice, directive: 'PlaintextDirective',
    ...
    # same book carries no flag and states the credit in a payment block
...
def _is_only_unpost_diff(invoice_or_bill, directive: 'PlaintextDirective',
    ...
    **entry GUIDs are preserved**. A cross-tool external reference to an
    ...
    Any other field difference returns False, and the caller falls through
    to the full unpost-rebuild-repost path. A `posted:` block or a `payment:`
    block beside `posted: none` never gets here: `import_invoice` and
    `import_bill` refuse either pairing before they ask.
    """
    md = directive.metadata
...
def _bill_non_payment_matches(bill, directive: 'PlaintextDirective',
    ...
    # As the invoice side, and for the reason it states.
    if counting_a_credit_request and _a_credit_is_being_added(
...
def _the_residual_states_an_exchange_difference(book, directive, currency) -> bool:
    """Whether a transaction's `$residual$` split states the exchange difference a disposal realized.

    Only in a transaction stated in the book's own currency. In a book keeping
    cost bases, where a split states the cost basis it draws on: that is what
    makes a split a disposal. A book keeping none (Q-049) states none, and there
    a residual beside a split in anything but the book's own currency, another
    currency or a security, is the difference that holding realized, which the
    ...
    root = book.get_root_account()
    accounts = [find_account(root, str(child.props['account'])) for child in splits]
    return any(account is not None and not is_the_books_own_currency(account.GetCommodity())
               for account in accounts)


def _say_what_turning_cost_bases_off_cleared(book) -> bool:
    """Clear every cost basis key of a book the `company` block has just turned to keep none, and say how many splits held one; whether any did (Q-049)."""
    if book_keeps_cost_bases(book):
        return False
    cleared = clear_every_cost_basis(book)
    if cleared:
        _echo_note(
            f'this book keeps no cost bases now (`cost_bases: "off"`), so '
            f'the cost basis keys of {cleared} split(s) were cleared. '
            f'Realized gains stay as the transactions record them.')
    return bool(cleared)
...
class GnuCashImporter:
    ...
    def create_commodity(directive: PlaintextDirective, book: Book):
        ...
        if commodity is None:
            if namespace.upper() == 'CURRENCY':
                ...
                # is read back. A code the table does not have leaves it
    ...
    def create_account(directive: PlaintextDirective, book: Book) -> bool:
        ...
        # Every part of the path is an account, and an account needs a name.
        # `open ""` was created as a nameless account under the root and
        # counted like any other, and `open Assets::Savings` was refused for a
        # parent account it could not find, which sends a reader looking in
        # the book for what is missing from the line.
        parts = account_fullname.split(':')
        if all(not part.strip() for part in parts):
            raise ValueError(
                f"an account needs a name, and {account_fullname!r} has none")
        if any(not part.strip() for part in parts):
            raise ValueError(
                f"an account needs a name, and {account_fullname!r} has an empty part")
        ...
        # collision with an account the book does not contain — quoting a guid,
    ...
    def create_transaction(directive: PlaintextDirective, book: Book,
                           entered: Optional[datetime] = None) -> 'Transaction':
        """
        ...
        Args:
            ...
            book: GnuCash book
            entered: the moment the transaction is entered at, which GnuCash
                orders a day's transactions by after `num`; the current time
                when it is None
        ...
        refuse_a_cost_basis_key_where_none_is_kept(book, directive)
        _check_stated_costs(book, directive)
        _check_stated_balances(book, directive)
        _refuse_a_lot_owner_naming_no_customer_or_vendor(directive)
        ...
        with transaction_under_construction(book) as transaction:
            ...
            transaction.SetDateEnteredSecs(entered or datetime.now())
            ...
            residual_states_an_fx_difference = _the_residual_states_an_exchange_difference(
                book, directive, commodity)
            ...
            for child in directive.children:
                ...
                if 'value' in split_directive.metadata:
                    ...
                    if 'share_price' in split_directive.metadata:
                        _warn_of_a_price_other_than_value_over_amount(
                            split_directive, amount, value, split_account_str)
                ...
                _lot_guid = _the_lot_guid_stated(split_directive.metadata)
                if _lot_owner_str:
                    # A customer or a vendor: any other `lot_owner:` was
                    # refused before the transaction was built.
                ...
                # that no rollback undoes, and it should not outlive the cost basis
                # it describes. No file can be written that tells the two
                # orderings apart: a lost transaction takes its splits' guids
                # with it, so the sales below it are refused for stating a cost basis
                ...
                # stated it. A split can carry a balance the *book* recorded on it,
                # and noting that would tell the sales below to leave a cost basis
            ...
            replace_the_positions_with_their_guids(directive, made)
        ...
        # against that cost basis's balance and lowers it; a split that
        # brings foreign currency in opens a cost basis of its own. Both run after
        ...
        # What the picks drew is put back with the transaction. Nothing the
        ...
        # KVP writes that open a cost basis. A drawdown outliving its transaction
        # would leave a cost basis reading 60.00 USD available with nothing in the
        # book that took the other 40, so the two are undone together.
        # `apply_cost_basis_picks` checks every balance before it lowers the
        # first, so its own loop cannot stop part-way; putting back what it
        # drew is tested in
        # tests/unit/services/test_cost_basis_drawdown_is_reversible.py.
        #
        # None of it in a book that keeps no cost bases (Q-049): no disposal
        # is asked which cost basis it draws on, no refusal built on them
        ...
        try:
            ...
            refuse_a_disposal_that_states_no_cost_basis(book, transaction)
            ...
            try:
                ...
                # that stated no cost basis are written in the file's own positions.
                if isinstance(refused, SpendStatingNoCostBasisError):
                ...
                cost_bases_changed()
                put_back_on_cost_bases(book, taken)
            if message:
    ...
    def update_transaction(existing_tx, directive: PlaintextDirective, book: Book) -> None:
        ...
        **A block updates the split its `guid:` matches.** A block stating one
        the transaction holds on another account moves that split to the
        account the block states, keeping its identity; position decides
        only among the blocks that state no guid, within one account — which
        is what tells a meal from a tip when both are on `Expenses:Dining`
        and neither block states its split. Splits no block states are
        removed, and blocks stating no existing split are created; a split
        ...
        # was told to delete the transaction and import it again, and the fresh
        # import then said the account was not found.
        for child in directive.children:
            if find_account(book.get_root_account(), child.props['account']) is None:
                raise ValueError(f"Account not found: {child.props['account']}")

        # Q-035: a transaction that touches a cost basis is not edited in
        # place. The checks that govern a sale — that a cost basis has the currency
        # it is being sold, that the cost basis is lowered by what the sale takes —
        ...
        # of 400.00 USD against a cost basis holding 60.00, reported `Updated: 1`,
        # and left the cost basis still reading 60.00.
        #
        # Deleting the transaction and importing it afresh is the supported
        # route and reaches the same end state: deleting a sale puts back on the
        # cost basis exactly what that sale drew, and the new import runs every
        # check. That is what this refusal points at.
        _refuse_a_split_on_an_account_with_no_commodity(book, directive)
        refuse_a_cost_basis_key_where_none_is_kept(book, directive)
        # Under `--atomic`, a payment the file states the book no longer
        # holds is taken off before anything below reads what the
        # transaction settles and draws (Q-053), and after the checks above,
        # which read the block alone. A refusal still to come leaves it taken
        # off in memory: that is safe only because `--atomic` saves nothing
        # while a refusal stands at the end, and a later pass finds the split
        # already out of the lot.
        if running_atomic():
            _take_off_the_records_the_file_no_longer_pays_with_it(book, existing_tx, directive)
        # A book that keeps no cost bases asks an edit nothing about them
        # (Q-049): it holds none to compare, and none to move.
        keeps_cost_bases = book_keeps_cost_bases(book)
        to_compare = (_require_no_cost_basis_edit(existing_tx, directive)
                      if keeps_cost_bases else None)
        ...
        # not happen.
        splits_move = (keeps_cost_bases and not deferring_edits_in_place()
                       and _a_split_moves(existing_tx, directive))
        if keeps_cost_bases and not (splits_move and cost_basis_facts(existing_tx)):
        ...
        facts_before = cost_basis_facts(existing_tx) if keeps_cost_bases else []
        ...
        # (Q-051): what the old version drew is put back first, and where
        # the new version is refused, the old one is restored from this.
        # Read only where the edit can be read as new, the one path that
        # puts back and clears: an edit compared with nothing returns right
        ...
        # What draws on its cost bases, down the chain, read before the edit
        # opens: those came after it, and are what a refused edit lists to
        # delete first. After the commit, what each account held before the
        # transaction is read in GnuCash's order, less these where GnuCash
        # orders one before it: read without them, a fee drawn on the arrival
        # the same day was counted as though it came before it, and the
        # arrival recorded 2,719.28 brought in where it was 2,720.00.
        came_after = frozenset()
        if compare or facts_before:
            ...
            came_after = frozenset({guid for guid, _label in drawing}
                                   | _the_transactions_the_file_draws_on_it_with(directive))
        ...
        # Which splits were already cost bases, for
        # `open_what_an_edit_made_a_basis` after the commit.
        splits_before = the_bases_a_transaction_has(existing_tx)
        ...
        # undo.
        #
        # The cost check is passed the transaction as well, because an update
        ...
        for split_directive in directive.children:
            ...
            # attach below: that runs only for a split in no lot, so a
            # `lot_guid:` with no `lot_owner:` beside it reached no reader at
            # all and the run answered `unchanged` to a file asking for
            # something. A `lot_owner:` stating no customer or vendor was
            # refused above, beside the stated figures.
            _the_lot_guid_stated(split_directive.metadata)
        ...
        # What it works out is which splits their blocks move to another
        # account; those moves are made inside the edit below.
        accounts_the_blocks_state = _splits_and_the_accounts_their_blocks_state(
        ...
        grouped_splits = _splits_grouped_by_the_account_each_block_states(
            existing_tx, accounts_the_blocks_state)
        pairings = {
        ...
        # it, because that runs inside the edit, after a split has been moved
        # to another account and a surplus one destroyed. A file that refuses has
        ...
        for acct_name, blocks in desired_by_account.items():
            ...
            for block, split in zip(blocks, chosen):
                ...
                _refuse_to_move_a_split_to_another_owner(book, split, block.metadata)
        ...
        # is refused, as creating the transaction refuses it; read inside the
        # edit and passed over, the transaction kept its old currency and the
        # run reported it updated.
        if 'currency.mnemonic' in directive.metadata:
            stated_namespace = directive.metadata.get('currency.namespace', 'CURRENCY')
            stated_mnemonic = directive.metadata['currency.mnemonic']
            if book.get_table().lookup(stated_namespace, stated_mnemonic) is None:
                raise Exception(
        ...
        try:
            ...
            date_str = directive.props['date']
            date = datetime.strptime(date_str, '%Y-%m-%d')
            # Only where the day moves. A file writes a day, and the book
            # holds a moment: GnuCash 3.8 posts an invoice at midnight, the
            # GUI enters at other times, and normalizing puts every one at
            # 10:59 UTC. Moved on an edit that kept the day, a transaction
            # changed places among the day's others, since GnuCash orders
            # them by the moment, and the export changed order with it.
            if date_moves:
            ...
            # Update currency if specified. It is one GnuCash has: a currency
            # it does not have was refused before the edit began.
            namespace = directive.metadata.get('currency.namespace', 'CURRENCY')
            if 'currency.mnemonic' in directive.metadata:
                mnemonic = directive.metadata['currency.mnemonic']
                existing_tx.SetCurrency(commodity_table.lookup(namespace, mnemonic))
            ...
            # A split whose block states another account is moved there
            ...
            # thing the edit does.
            _set_the_account_each_block_states(accounts_the_blocks_state)
            ...
            for acct_name, split_directives in desired_by_account.items():
                ...
                for i, split_directive in enumerate(split_directives):
                    ...
                    if split is None:
                        ...
                        # free — `_splits_and_the_accounts_their_blocks_state`
                    ...
                    # Where the block states both amounts they decide the
                    # price, as on the create path. Set after the value,
                    # `SetSharePrice` revalued the split at amount × price:
                    # 45.00 USD stated at 63.23 with `share_price: "1.5"`
                    # became 67.50, and GnuCash parked 4.27 in `Imbalance-CAD`
                    # under `Errors: 0` (measured on 5.10).
                    if ('share_price' in split_directive.metadata
                            and 'value' in split_directive.metadata):
                        _warn_of_a_price_other_than_value_over_amount(
                            split_directive, amount, value, acct_name)
                    elif 'share_price' in split_directive.metadata:
                        share_price = _stated_rate(
                            split_directive.metadata['share_price'],
                    ...
                    if _lot_owner_str and split.GetLot() is None:
                        ...
                        # In a lot already. One with no owner takes this
                        # one; one of another owner's was refused before the
                        # edit began; one of this owner's is left alone.
                        _attach_the_owner_to_a_lot_with_none(book, split, _lot_owner_str)
        ...
        # malformed `cost_basis_cost:` left a split changed and with no cost basis.
        ...
        # a cost basis like any other. Which splits get one, and why not the rest,
        # is `open_what_an_edit_made_a_basis` — the same rule every writer that
        # edits a transaction asks.
        for split in stated_balance_splits:
            note_stated_balance(split)
        ...
        # Every edit of a book that keeps no cost bases returns here: it asked
        # nothing before the commit, so it reads nothing after it (Q-049).
        if not compare:
            mark_what_arrives_past_zero(existing_tx, came_after, as_it_was=as_it_was)
        ...
        taken_after: dict = {}
        try:
            mark_what_arrives_past_zero(existing_tx, came_after)
            ...
            if _cost_bases_moved_under_other_transactions(
                ...
                # states the date and not a figure the run would have deferred.
            ...
            # is put back.
            _put_back_what_the_old_version_drew(book, taken_before, put_back)
            ...
            refuse_a_disposal_that_states_no_cost_basis(book, existing_tx)
    ...
    def import_company(directive: PlaintextDirective, book: Book):
        ...
        changed = False
        had_any = False
        # Before anything is written, as every refusal of a block is.
        ...
        # A book turned to keep no cost bases holds none of their keys
        # (Q-049), so the ones earlier imports wrote go now, once the answer
        # each book was asked for is forgotten.
        if COST_BASES_KEY in md:
            forget_whether_books_keep_cost_bases()
    ...
    def import_taxtable(directive: PlaintextDirective, book: Book):
        ...
        # A tax table is found by its name, and a line states the one it uses
        # by that name. `taxtable ""` was created, and the export then wrote
        # no `tax_table:` on a line using it, so re-importing changed the line.
        if not tt_name.strip():
            raise ValueError('a tax table needs a name')
        # Read whole, or refused. A block nothing reads under a tax table went
        # missing with nothing said, and the entries were read as every block
        # after the first, so with a `breakdown:` written first the one entry
        # was added to the table twice. A table with no entry has nothing to
        # charge; it was answered `skipped`, which says the book already holds
        # it, and GnuCash's own dialog will not save one.
        for child in directive.children:
            if child.type != DirectiveType.TAXTABLE_ENTRY:
                raise ValueError(
                    f'a `{child.line.strip()}` block is not read under a tax '
                    f'table, only `entry:` blocks are')
        if not directive.children:
            raise ValueError('a tax table needs at least one `entry:` block')
        ...
        first_entry_directive = directive.children[0]
        ...
        # Every one an `entry:` block: anything else was refused above.
    ...
    def import_invoice(directive: PlaintextDirective, book: Book,
        ...
        posting_was_this_books = _the_file_states_a_posting_this_book_has(
            book, directive)
        # For an invoice being created as much as one being edited: the
        # readers below take a guid they cannot parse as a block stating
        # none, and a payment block's guids are read late or not at all.
        _refuse_a_payment_guid_nothing_can_parse(directive)
        _refuse_one_split_stated_by_two_payment_blocks(directive)
        _refuse_a_payment_figure_nothing_can_read(directive)
        _refuse_a_credit_block_stating_a_transaction(directive)
        _refuse_a_credit_block_stating_an_account_or_a_date(directive)
        _refuse_a_payment_stating_a_posting(book, directive, existing)
        _refuse_a_payment_block_that_cannot_settle_its_record(book, directive, existing)
        _refuse_a_payment_account_its_transaction_did_not_use(book, directive)
        _refuse_a_payment_account_with_no_commodity(book, directive)
        _refuse_a_stated_amount_the_applied_splits_contradict(
            book, directive, _the_account_the_file_posts_to(book, directive))

        _refuse_a_block_nothing_reads_under_the_record(directive, 'invoice')
        _refuse_blocks_out_of_their_order(directive, 'invoice')
        ...
        if existing is not None:
            ...
            # No slot keys to move onto fields, as the bill side has: an
            # invoice's `notes:` and `billing_id:` have been fields since
            # invoices were first imported, and were never written to its slot.
            ...
            if added:
                ...
                # Again, now the payments are in the lot: a block stating a
        ...
        for entry_directive in directive.children:
            if entry_directive.type == DirectiveType.INVOICE_ENTRY:
                ...
                # Written whether the block states one or not: a line being
                # edited keeps whatever its block does not state, and a block
                # with no `tax_table:` is a line with none — as it is on a line
                ...
                if linked_tx is not None:
                    ...
                    # so the roundtrip preserves the memo field exactly. Always a
                    # posting: `gncInvoicePostToAccount` returns none only for no
                    # invoice, no account or an invoice already posted, and this one
                    # is unposted with its account found above.
                    posting_txn = invoice.GetPostedTxn()
                ...
                # A payment: the only block left, any other refused before
        ...
        # not come through here: it states the credit to spend and divides
        # that one itself.
    ...
    def import_bill(directive: PlaintextDirective, book: Book,
        ...
        _refuse_a_block_nothing_reads_under_the_record(directive, 'bill')
        _refuse_blocks_out_of_their_order(directive, 'bill')
        ...
        for entry_directive in directive.children:
            if entry_directive.type == DirectiveType.BILL_ENTRY:
                ...
                # Written whether the block states one or not, as the invoice
                # side writes it: a line being edited keeps whatever its block
                # does not state, and a block with no `tax_table:` is a line
                # with none.
                ...
                if is_a_new_line:
                    ...
                    _set_the_lines_bill_pointer(entry, bill)
                entry.CommitEdit()
                ...
                if linked_tx is not None:
                    ...
                    # Override the transaction description GnuCash set automatically,
                    # so the roundtrip preserves the memo field exactly. Always a
                    # posting, for the reason the invoice side states.
                ...
                # A payment, as on the invoice side.
                # After the cash, for the reason stated on the invoice side.
                if _paid_from_credit(entry_directive.metadata):
                    credit_blocks.append(entry_directive)
                else:
        ...
        if _asks_for_credit(directive):
            ...
            # carves the split it lives on — so the cost basis follows what is left,
