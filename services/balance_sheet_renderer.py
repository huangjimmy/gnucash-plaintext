...
def render_text(result: BalanceSheetResult) -> str:
    ...
    if not result.fx_rates_provided:
        out.append(f"NOTE: No FX rates supplied. {BASE_CURRENCY} totals "
                   f"not available.\n")
    ...
    out.extend(_render_section_text(result.assets, result.fx_rates_provided, units))
    out.extend(_render_section_text(result.liabilities, result.fx_rates_provided, units))
    out.extend(_render_section_text(result.equity, result.fx_rates_provided, units))
    out.append("=" * 60)
