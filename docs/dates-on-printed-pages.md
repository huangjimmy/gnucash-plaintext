...
How to get the dates you want on a page `print-invoice` or `print-bill` produces, and why the obvious ways do not all work.
...
| `date_format` | posted date | entry row | "printed on" | warns? |
|---|---|---|---|---|
...
`%d %B %Y` is a perfectly reasonable thing to want. Set it and the run prints the page, and says this on stderr:
...
```
posted date : 09 March 2026     ← your format
due date    : 09 April 2026     ← your format
entry row   : 03/09/26          ← this machine's locale
```

That is not a bug and the page is fine to send; it is simply a page with two date formats on it, which you should know about rather than discover. If you need `09 March 2026` on the line items too, [write your own report](#if-you-need-a-format-the-four-do-not-cover).
...
| the invoice's dates in an unusual format, and the line items may differ | `date_format`, and take the warning as information |
...
A page has dates in two groups, and GnuCash writes them from different places.

| on the page | written from |
|---|---|
| the invoice's **posted date** and **due date**, at the top | a **book option** — GnuCash's *Fancy Date Format*, File → Properties → Business |
...
There is a `CUSTOM` value, but it belongs to the check-printing code and no public function sets a custom string through it. So a format string can reach the invoice's own dates — the book option really is a format string — and can never reach the line items.
...
**Setting it in the GUI's File → Properties → Business.** This is the same book option `date_format` writes, and a format set there does reach the invoice's own dates — that half is measured, since it is the option this tool reads and writes. What is *not* measured, and so is not claimed here, is the other direction: what GnuCash's dialog shows for an option this tool wrote, and what it writes back if you press OK. These containers have no GUI to measure it in.

What the GUI setting cannot do either way is travel: an export and re-import will not carry it unless the ledger says it, and it only ever covers the invoice's own dates, never the line items. Prefer the ledger, and if you do set it in GnuCash, put it in the ledger too so the next import restores it.
...
`tests/integration/test_a_book_says_how_its_dates_are_written.py` is the executable version of this guide. Each class answers one of the questions above:

| test | what it shows |
|---|---|
| `TestThePrintedPage::test_one_format_on_the_whole_page` | ISO everywhere, and nothing left reading the machine's way |
| `TestThePrintedPage::test_a_format_gnucash_has_no_style_for_says_so` | the warning above, and that the page still prints |
| `TestThePrintedPage::test_it_reaches_the_invoices_own_dates_and_not_the_entry_dates` | which dates each setting governs |
...
* [Q-037](issues/Q-037-a-printed-page-is-dated-by-the-machine-that-printed-it.md) — the measurements behind all of the above, including the negative results.
