...
| `txn_split_guid:` is the guid of a split that already settles another invoice | `invoice "X": the split given in txn_split_guid '…' is in another invoice's or bill's lot — it settles that one, and taking it here would leave it unpaid` |
