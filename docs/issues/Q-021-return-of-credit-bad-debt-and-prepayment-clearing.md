...
- **Q-014** — introduced the `lot_owner:` per-split marker for orphan-payment lot reconstruction. Q-021 folds that always-create handler into the unified join-or-create `_attach_lot_owner_split`; the 45-test Q-014 orphan / payment-roundtrip suite still passes, confirming no regression.
...
- **Q-016** — full GUID emission and import-order for payment round-trip. Q-021's clearing transactions and `lot_owner:` markers round-trip on top of that GUID continuity.
