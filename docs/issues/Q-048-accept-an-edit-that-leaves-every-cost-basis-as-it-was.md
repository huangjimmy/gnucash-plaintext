...
Q-051 reads an edit that changes a cost basis as a new transaction would be read, where nothing else draws on that cost basis, so several edits this issue refuses are accepted since; that issue lists them.
