...
- The same holds when the line turns out to be income or an expense, dollars moved from another of the owner's US dollar accounts with the fee on a split of its own giving its cost basis, or dollars whose fee was part of the exchange spread.
...
- The new version's cost bases are whatever its shape establishes, and its splits draw what they state, exactly as an import of that transaction into the book would. What the old version established and drew is given back first: a cost basis the old shape needed and the new one does not is not discarded, it is simply not established by the new version.
- The new version meets every check a new transaction meets. So an edit can turn a line that established a cost basis into one that consumes one: a withdrawal read as owed while it sat against the holding account, booked as an expense, gives the cost basis its dollars came out of, as a new spend must.
...
- **Linking the deposit to the invoice with a `payment:` block** is refused twice. Giving only the transaction, it asks which of the two holding-account splits is the payment. Giving the split, it cannot tell how much of the transaction settles the invoice, because the fee's splits are beside the 2,720.00.
...
| E7a | A withdrawal imported as `Due from director +0.72 CAD`, `Wise −1.00 USD`, which establishes a cost basis on the owed side, booked as `Expenses:Bank charges +0.72 CAD`, `Wise −1.00 USD`, giving with `cost_basis_split_guid:` the cost basis the dollars came out of | accepted; the new version establishes no cost basis and consumes the one it gives, as a new spend would; the owed-side cost basis the old version established is not established |
| E7b | E7a with no `cost_basis_split_guid:` on the booked withdrawal | refused, as a new spend giving no cost basis is, with the ways to write it; the transaction is as it was |
...
| E13 | A new version that would not be a correct new transaction: the fee's `cost_basis_split_guid:` left out, which keeps the pick it has on the deposit the new version establishes no cost basis for; or cleared with `""`, so the fee spends the collected dollars giving none | refused as a new import of it would be, with that import's reasons; the transaction is as it was, and the invoice unpaid |
| E13a | A new version that does not balance | refused before anything is written, giving what its values come to, which GnuCash would otherwise put on an Imbalance account |
| E13b | The deposit booked as collecting INV-USD-1, with the fee giving INV-USD-2's posting as its cost basis | refused: nothing of INV-USD-2 has been collected, by its lot or by this transaction, so its dollars are owed and the fee has none of them to spend |
| E14 | E1 under `--atomic` | as E1 |
| E15 | A file whose transaction is refused, with an invoice's `payment:` block giving that transaction's guid | the block is refused too, giving the transaction's reason, rather than recording a payment of its own |
...
**The new version is read as new.** What the old version drew is given back to each cost basis holding a stored balance, and the stored balance of each cost basis it established that nothing else draws on is cleared. The new version is then checked and applied as `create_transaction` checks and applies a transaction: what each split brings in past zero, read afresh; the refusals of a new transaction; the cost bases its own splits draw on, opened first; the draws; the cost bases its shape establishes. `$residual$` is read as the create path reads it. The splits keep their guids and stay in the lots they are in, so a split settling an invoice or a bill is still that record's.

**A balance a file stated stays stated.** A stored balance can hold less than the transaction's own figures give, because a file stated it: currency sold outside the book. Before the edit, the part of each balance the transaction's figures do not explain is read, and it is put back once the new version's cost bases are open. A deposit of 2,720.00 USD with a stated balance of 2,000.00, booked as income, still holds 2,000.00; opened afresh, it held 2,719.28 and offered again the 719.28 sold elsewhere. Where the new version's own splits then draw more than that leaves, the edit is refused. A cost basis that held no balance holds none after the edit. A balance the edit's own block states is kept as the block states it, and nothing is given back to it, because it is stated net of the file's own disposals.
...
**A `payment:` block giving a transaction the run refused is refused.** The money moved in that transaction as the file writes it, so recording a payment from the block would enter it a second way, and settling from the version the book still holds would settle from one the file replaces.
...
- a fee given a second split, or moved a day later, draws what it now states; an arrival moved a day later is still refused while its fee draws on it (`test_an_edit_may_move_the_split_that_sets_the_rate.py`);
...
Refusals that remain are given in the new transaction's words: a cleared pick is a spend giving no cost basis, a value neither the share nor what is left is refused giving what is left, and a sale beyond what the account holds is refused for the value its past-zero reading requires.
...
`find_split_by_guid` walked the whole book too, on every pick checked and every draw given back. And every block of the file was edited and committed, including every block stating its transaction as the book already holds it. GnuCash pays each commit in proportion to the size of the accounts.
...
2. `find_split_by_guid` asks GnuCash's `xaccSplitLookup`, present on 3.4, 5.10 and 5.15, and still gives only a split on an account.
3. Which splits draw on each cost basis is an index of the book (`splits_drawing_on`). It is built by walking the book once, and kept up to date from each change of `cost_basis_split_guid`, which `set_custom_metadata` logs. Each split it gives is looked up and asked again, so a split destroyed since is not given. `transactions_drawing_on` and the two per-edit steps ask it.
...
| E13b | `test_the_fee_giving_an_invoice_the_transaction_does_not_collect_is_refused` |
| E14 | `TestADepositBookedAsTheInvoiceItCollected::test_under_atomic` |
| E15 | `test_a_payment_block_giving_a_transaction_the_import_refused_records_no_payment` |
