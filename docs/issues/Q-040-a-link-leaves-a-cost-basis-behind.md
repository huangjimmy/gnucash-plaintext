...
2. The bank feed brings the deposit in against **Due from director**, which is CAD. The transaction is therefore stated in CAD, the USD split is given `share_price: "1.4029"`, and that rate is what says what the USD cost. The bank split opens a cost basis of 2,720.00. Total: 6,460.00 USD.
...
Booking the other side to a **USD** account gives no rate at all. Measured: `…Chequing… +2720.00 USD / Due from director USD −2720.00 USD` opens no cost basis, and a later link is harmless. This is worth documenting; nothing says it today.
...
- The overpaid bill link is refused with instructions to restructure that payment transaction into two A/P splits — and that restructuring is itself an edit to a cost basis-touching transaction, so the remedy the message gives is unreachable.
