...
1. **Standalone `*` transaction blocks emit per-split GUIDs.**
   ```
   2026-04-01 * "Acme"
     guid: "<tx-guid>"
     Assets:Bank  400.00 CAD
       split_guid: "<bank-side split guid>"
       memo: "..."
     Assets:Accounts Receivable  -100.00 CAD
       split_guid: "<ar-side split 1 guid>"
     Assets:Accounts Receivable  -120.00 CAD
       split_guid: "<ar-side split 2 guid>"
     Assets:Accounts Receivable  -180.00 CAD
       split_guid: "<ar-side split 3 guid>"
   ```
...
3. **Invoice/bill `payment:` blocks emit `payment_split_guid:` identifying the specific AR/AP-side split** that belongs to this invoice/bill. For a single-invoice payment this is unambiguous; for multi-invoice this is the disambiguation.
...
2. **Generalise `_set_object_guid` to accept any `QofInstance`** — splits, transactions, lots — not just business objects. Same ctypes `qof_instance_set_guid` call. Use it when creating splits if `split_guid:` is declared.

3. **Payment block resolution with `payment_split_guid:`.** When set, the importer:
   - Looks up the tx by `txn_guid:` (must exist after the standalone-tx pass)
   - Looks up the specific split by `payment_split_guid:` (must be a split of that tx)
   - Calls `xaccSplitSetLot(split, invoice.GetPostedLot())` to attach
   - No new splits created; no retargeting math; no inference

4. **Backward-compatibility — payment block with `txn_guid:` but no `payment_split_guid:`** keeps the existing iterative-retarget mechanic so plaintext written before Q-016 still loads.
...
- **`test_backward_compat_old_plaintext_without_split_guid_still_imports`** — a plaintext file generated by pre-Q-016 code (no `split_guid:`, no `payment_split_guid:`) imports cleanly via the iterative-retarget fallback. Ensures we don't break existing user files.
...
- `README.md` — document the new payment block fields (`txn_guid:` always present on export; `payment_split_guid:` for shared bank txs), the multi-invoice payment workflow, and the import-order guarantee.
