...
Probed by a research script that was not committed; `test_single_invoice_txn_guid_fresh_roundtrip` in `tests/integration/test_payment_fresh_roundtrip.py` holds the fixed behaviour. What goes wrong:
...
Integration tests for the full surface, in `tests/integration/test_payment_fresh_roundtrip.py`, all asserting on the end-state of an export → fresh-book re-import cycle:
...
- **`test_two_invoices_same_amount_disambiguated_by_split_guid`** — the fragility scenario for iterative retarget: two invoices both for $200 paid by one $400 bank tx. With per-split GUIDs each invoice claims its specific AR split unambiguously regardless of plaintext order.
