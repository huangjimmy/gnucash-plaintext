...
`infrastructure/pdf/webkit_page.py` is a child process — `WebKit2.PrintOperation` with GTK's "Print to File" printer. A child, because a GTK main loop inside the process holding an open book and an initialised Guile is two event loops over the same ctypes-shared globals; and because a machine printing from a script has no display to give it. One is arranged by `a_display`: the reader's own `DISPLAY` where there is one, else an `Xvfb` started for the run and shared across every page in it, else `xvfb-run` on a machine carrying the wrapper and no server.
