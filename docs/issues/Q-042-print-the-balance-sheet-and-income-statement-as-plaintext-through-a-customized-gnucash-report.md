...
Both kinds of gain are on the balance sheet at once, by different routes. A gain already taken is income, so it is inside `retained_earnings` — the 480.00 USD from the shares and the 240.00 CAD from the currency, each with its own line on the income statement. A gain on what is still held is held by no account, so it is the `unrealized_gains` key: 2,303.20 CAD on the 7,480.00 USD, 5,500.00 HKD and 12 AMZN the book still has, against the 2,500.00 USD it still owes.
