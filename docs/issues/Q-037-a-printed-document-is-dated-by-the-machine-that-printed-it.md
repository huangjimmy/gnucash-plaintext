# Q-037 — A printed document is dated by the machine that printed it
...
| `qof_date_format_set(3)` then print | entry rows become `2026-03-09`; the document's own dates unchanged |
| `qof_date_format_set(1)` then print | entry rows become `09/03/2026`; the document's own dates unchanged |
| book option set, QOF left alone | document `09 March 2026`, entry rows `03/09/26` — one page, two formats |
