...
If you rely on GnuCash's business features or custom metadata and need a lossless plaintext format, use the native [GnuCash plaintext format](../README.md#gnucash-plaintext) (`export` / `import`) instead — it round-trips every object type the importer supports, including invoices, bills, customers, vendors, payments (with full `txn_guid:` + `payment_split_guid:` GUID emission, including the one-bank-transaction-covering-many-invoices shape — see [docs/comprehensive-roundtrip-example.md](comprehensive-roundtrip-example.md)), tax tables, and custom KVP metadata on every object type.
