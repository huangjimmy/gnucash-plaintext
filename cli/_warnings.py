...
written to do or letting a page leave silently short of something the
...
def said_once():
    ...
    Printing a whole book is one process rendering many pages, and what a
    page can hold is a property of the report rather than of the invoice — so
    a page with nowhere for the seller's block has nowhere for it on every
    page in the run, and the same sentence would arrive once per page:
    two lines for a two-invoice book, a hundred for a book of fifty.

    To stderr, because stdout may be the page itself (`-o -`).
    ...
    page: the seller's block is one value for the whole book, but the
