...
**Linking a transaction to pay an invoice discards the cost basis that transaction opened.** A deposit that put 2,720.00 USD in the bank against a CAD account was read as buying or borrowing USD, so it opened a basis at the rate it stated. Linked to pay an invoice, it is neither: the invoice's own receivable has priced that money since the day it was posted, and two bases for one lump of currency is what this release stops. A part payment discards it too, and the currency is the receivable's from then on — selling it waits for the invoice to be collected, or says `cost_basis_force: true`.

**A bill keeps its basis instead**, because USD drawn on a credit line is still owed once the payable is paid. It is kept by writing the price onto the split, since the transaction can no longer supply one, and that price now survives an unapply: taking the link off drops the stored copy where the restored transaction prices the split itself.

**A link is refused where a sale already draws on the basis it would discard**, naming the sales. They were valued at the deposit's rate, which the invoice need not share, so they cannot simply be pointed at the invoice's basis: delete them, link, and import them again.

**`unapply-payment` and `unlink` say when a rate came from another day.** A rates file is carried forward rather than extrapolated, so a file quoting 2026-07-31 answers for 2026-08-13 with a figure thirteen days old — which decides what the restated split is worth and what any reopened basis is priced at. Both commands now say so on stderr, naming both days.

**`import --atomic` applies a file the way a database applies a transaction**: it all commits, or none of it does and the book is untouched. Use it for a correction, whose blocks are parts of one change; a bank feed still wants the old behaviour, which is unchanged without the flag. Under it the refusal to edit a transaction a cost basis rests on is deferred to the end and the finished book is asked the same questions instead, because a repair passes through states that refusal stops in either order. A block that restates what a disposal takes, or drops the line naming its basis, is still refused as it lands.
...
- a disposal drawing on a basis that holds another currency;
- a disposal no longer valued at what its basis cost, or drawing on a receivable nobody has collected.
