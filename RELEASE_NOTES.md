...
### The balance sheet and the income statement are printed by customized GnuCash reports, in the currency the book is kept in

**`balance-sheet`, `income-statement` and `report` print their pages through GnuCash's own report engine, and no figure on a page is calculated by gnucash-plaintext.** The plaintext page is drawn by two reports in `infrastructure/gnucash/reports/balance-sheet-and-income-statement-as-text.scm`, which the package ships: each is written from GnuCash's own `balance-sheet.scm` or `income-statement.scm`, registered with `gnc:define-report` as GnuCash's are, taking the same options, making the same calls for every figure, and changed to return its page as plain text rather than HTML. GnuCash runs it, adds up every balance and converts every currency through the book's price database. `--output-format html` prints the page of GnuCash's Balance Sheet or Income Statement report as GnuCash ships it, and `pdf` prints that page through WebKit, the way a printed invoice is.

**The page is in the currency the book is kept in.** GnuCash stores no currency for a book — measured on 3.4, 3.8, 4.4 and 5.10, the root account holds no commodity and a report told nothing comes out in US dollars — so the commands take the first of these that states one: `--currency`, the `company` block's `base_currency:` key, or the currency of the book's top-level accounts where every top-level account that holds a currency holds the same one. Where none of them does, the command is refused and the refusal says how to state it. A book kept in HKD that also holds USD and CAD prints in HK$, with GnuCash doing the conversion.

**The prices are GnuCash's, and a file may add some for one run.** `Price Source` is GnuCash's own default — `pricedb-nearest` on every build — unless `--price-source` is passed one of the three choices that read the book's price database: `pricedb-nearest`, `pricedb-latest`, and `pricedb-before` from GnuCash 4.8. Anything else is refused with those listed, GnuCash's averaging choices included: they build a rate out of the book's own transactions rather than reading a price, so the figure they would put on a line is a ratio no transaction was entered at. `--fx-rates` and `--prices` add their rates to the book's price database for the run alone: the book is opened read-only and never saved, so nothing is kept, and a price recorded for good still goes in a `price` block. A rate may be a fraction, and a price with no date is added at the end of each day the run's statements cover.

**Why a report runs rather than a conversion call.** `xaccAccountGetBalanceAsOfDateInCurrency` exists on every supported build and does not return what GnuCash shows: it prices at the latest price whatever the date on 3.4, 3.8 and 4.4, and at the last price up to the date from 4.8, where the report's own default is the price nearest in time — which may be a later one. On a book holding several prices of a pair those are three different figures. A person checking the page against GnuCash's report is checking it against the thing that drew it.

**The page is a plaintext block, not a text rendering of GnuCash's HTML.** A dated directive opens it, and keys and account lines sit under it with tabs, as a transaction's splits do. An account is a line carrying its whole path, its amount and its commodity, and that line is what the account **itself** holds — as GnuCash's own page reports it, where a brokerage holding 3,000.00 USD with a share account under it prints its own money in its own commodity and the section's sum is a `Total …` row. So a section's total is a key here, no account holding it: `total_assets`, `total_liabilities` and `total_equity` on the balance sheet, `total_revenue` and `total_expenses` on the income statement. There are no section headings, because each line's path already says which section it is in. **Every account is listed, however deep** — GnuCash's statements show three levels by default and fold anything below into its parent, which on a page a program reads would be an account that simply does not appear.

An account held in another commodity states `account.commodity.mnemonic:`, `share_price:` and `value:` under it, read against the block's `currency.mnemonic:`. **Those two are not the split keys of the same name**: on a split they are recorded together and multiply out exactly, while on a statement line `share_price:` is the price the report read from the price database and `value:` is what GnuCash converted the holding to — a valuation on that date, which moves as the date or the book's prices move. Each block says so in comment lines of its own, `#` opening a comment as `;` and `;;` now do.

Every other figure GnuCash computes that no account holds is a key too, stating its currency: `retained_earnings`, `unrealized_gains`, `trading_gains`, `net_income`, `total_liabilities_and_equity`. One key whatever the sign, where GnuCash's page switches between `Gains` and `Losses`. The first three are written whenever the book holds the money behind them and left off when it holds none of it, as is an account left holding nothing — one the period never touched, and one whose entries cancel each other out exactly. A figure of zero because the book prices that money at nothing is still stated. The totals and `net_income` are always written.

On the book the issue records — a Canadian book over two years holding US and Hong Kong dollars bought against a cost basis and partly sold, NASDAQ:AMZN bought and partly sold, consulting billed every quarter, and a US dollar loan drawn and partly repaid — the balance sheet at 2026-12-31 states USD Bank 10,621.60 CAD, AMZN 4,771.20 CAD at its market price and total assets 38,532.80 CAD, with net income 9,129.60 CAD for that calendar year: the figures GnuCash's own reports print, on all eleven supported builds. Both kinds of gain appear at once — what has been taken is income and sits in `retained_earnings`, and what the holdings have made since they were bought is `unrealized_gains`. A book with GnuCash's "Use Trading Accounts" option on states that second figure as `trading_gains` with no `unrealized_gains` beside it, which is what GnuCash's report does for such a book.

**A fiscal year need not be the calendar's.** The quarters differ, so a year ending 2026-03-31, 04-25, 06-30 or 12-31 picks up a different four and reports 5,150.00, 6,050.00, 7,512.40 and 9,129.60 of net income. The year ending 06-30 catches the share sale on its own last day and values the gain at the rate nearest that day rather than the calendar year end's.
...
**An invoice paid without entering any money can be unpaid.** The bank feed arrived first, or a director paid a supplier out of pocket, so the transaction was already in the book and a `payment:` block naming it made one of its splits the settlement; there was no way back. `unlink` undoes that, and what it leaves is a payment with nothing to do with the invoice any more: the split comes off the receivable, leaves the record's lot, and moves to the account passed as `--to`, while the invoice owes again. The transaction itself survives whole — nothing is deleted, and the only thing that changes is the one split the link wrote on.

```bash
gnucash-plaintext unlink ledger.gnucash INV-2026-001 --to "Assets:Due From Director"
```

**And the figure is restated for the account it lands on.** This is a fix as much as a feature: `unapply-payment` set the account and nothing else, so a 100.00 USD settlement moved to a CAD account kept the figure 100.00 and became 100 Canadian dollars. Nothing disagreed, because the split's value — the transaction's own side — was never touched.
...
### A payment link discards the cost basis it strands, and `import --atomic` applies a file whole

**Linking a transaction to pay an invoice discards the cost basis that transaction opened.** A deposit that put 2,720.00 USD in the bank against a CAD account was read as buying or borrowing USD, so it opened a cost basis at the rate it stated. Linked to pay an invoice, it is neither: the invoice's own receivable has priced that money since the day it was posted, and two cost bases for one lump of currency is what this release stops. A part payment discards it too, and the currency is the receivable's from then on — selling it waits for the invoice to be collected, or says `cost_basis_force: true`.

**A bill keeps its cost basis instead**, because USD drawn on a credit line is still owed once the payable is paid. It is kept by writing the price onto the split, since the transaction can no longer supply one, and that price now survives an unapply: taking the link off drops the stored copy where the restored transaction prices the split itself.

**And refused where the balance it would discard is below what the split brought in.** That difference is currency sold outside this book — a file states such a balance, and that is how a book carrying sales this tool never saw gets one — so discarding it is the only record of that currency going. The refusal states both figures and says to clear the balance first if the difference is not currency the book should account for.

**Unlinking the transaction restores the cost basis, at everything the purchase brought in**, and so does unapplying the payment. The transaction is a purchase of USD again, and nothing has been sold out of it — a settlement is no cost basis, so while it settled the invoice nothing could draw on that split. It is not a rule of either command's own: an edit that turns a split into a cost basis opens a balance for it, which is why a CAD placeholder corrected into `Assets:Bank:USD 100.00 USD` comes out with 100.00 USD of cost basis. `unlink` and `unapply-payment` were the writers not asking. A cost basis that was *already* one and carries no balance — made in the GnuCash GUI, or older than this — is still left alone, because how much of it has been sold is not recorded.

**A link is refused where a sale already draws on the cost basis it would discard**, listing the sales. They were valued at the deposit's rate, which the invoice need not share, so they cannot simply be pointed at the invoice's cost basis: delete them, link, and import them again.

**`unapply-payment` and `unlink` say when a rate came from another day.** A rates file is carried forward rather than extrapolated, so a file quoting 2026-07-31 answers for 2026-08-13 with a figure thirteen days old — which decides what the restated split is worth and what any reopened cost basis is priced at. Both commands now say so on stderr, stating both days.

**`import --atomic` applies a file the way a database applies a transaction**: it all commits, or none of it does and the book is untouched. Use it for a correction, whose blocks are parts of one change; a bank feed still wants the old behaviour, which is unchanged without the flag. Under it the refusal to edit a transaction a cost basis rests on is deferred to the end and the finished book is asked the same questions instead, because a repair passes through states that refusal stops in either order. A block that restates what a disposal takes, or drops the line stating its cost basis, is still refused as it lands.

**`fx-balances --verify-costs` asks four more questions, and they set the exit code.** A book that reported clean under v0.4.0 can report a fault now with nothing about it having changed — which is the point, since each of these was invisible before:

- a `cost_basis_balance` stored on a split that is no cost basis, with the reason it is not one;
- a disposal whose `cost_basis_split_guid:` matches no split, or one that is no cost basis;
- a disposal drawing on a cost basis that holds another currency;
- a disposal no longer valued at what its cost basis cost, or drawing on a receivable nobody has collected.
...
**`export-transaction --guid` and the undo copy `delete-transactions -o` writes follow the same rule.** The undo copy needed it most: a cost basis cannot be deleted while a sale measures against it, so the guids have to be passed sale-first, and the copy came out in that order with the transactions it was the only copy of already gone.

**And that copy states the balances the book held when the command began.** Deleting a sale puts its currency back on the cost basis it drew on, so a deposit written out after the fee had gone stated the whole of what it brought in rather than what was left of it — and re-importing the copy left the book offering currency its bank does not hold, silently, because a balance a file states is taken as already net of the sales below it. Every transaction such a run deletes is written out before any of them goes.

### The credit an overpayment leaves is one cost basis, however the payment is divided

**A credit the file divides is a cost basis, as one the import divides is.** A 200.00 USD wire against a 100.00 USD invoice can be stated as one receivable split, which `txn_guid:` with `prepayment:` divides, or already divided, with `txn_split_guid:` stating the settling split. Stated divided, the 100.00 USD left over opened no cost basis balance: `fx-balances` listed it as "none recorded" where the wire came from a CAD bank, and did not list it at all where it came into a USD bank in an entry stated in USD. It now opens one the same way the import's division does: at the rate the payment converted at, or, where the entry states no cost, at the rate the invoice was posted at, stored on the split as `cost_basis_cost`.

**The payable side is the same.** What is left over after paying a vendor beyond the bill is this book's money sitting with them, and it is priced the same two ways: at what the payment converted at where it went out of the CAD bank, and at the rate the bill was posted at where it went out of the USD one.

### A corrected payment memo reaches the bank split on a bill posted to a plain liability

**The bank split follows a corrected memo whatever type the record's account is.** A payment's memo is written on the split that settles the record, and the bank split beside it follows where it still reads the same. On a bill posted to a plain `type: Liability` account, only the settlement took the correction, and the bank split kept the old memo. What settles a record is the split in its lot, and that is what is asked now.

### `unpost-invoices` and `unpost-bills` report no payment that went through no bank

**A payment with no split off a receivable or a payable is not listed as orphaned.** GnuCash lets a journal entry net an invoice against a bill, typed as a payment, with each split put in its record's lot. Measured on 3.8 and 4.4, that entry kept its payment type, and unposting the invoice listed it as an orphaned bank-side payment with a blank account and 0.00, with the advice to delete it, while it still settled the bill. On 5.10 GnuCash reads it back as a link and deletes it with the posting, and nothing was listed there either.

### A cost basis balance that will not parse survives GnuCash dividing a credit

**`auto_apply_credit: true` carries an unreadable balance across as it reads.** A book another program wrote can hold a credit whose `cost_basis_balance` is not a figure, such as `20,00`. Divided by a `from_credit:` block, the remainder has always kept that text, so `fx-balances --verify-costs` goes on reporting it. Divided by GnuCash through `auto_apply_credit: true`, the remainder was written the whole of what was left, `60.00`, and the text the check reports was gone. It is kept now.

### `fx-balances` prints two column headings differently

**`BROUGHT IN` stands over the units a cost basis brought in, and `COST BASIS BALANCE` over what is left of them.** The first was `ACQUIRED`, a word used for that column and nowhere else; what the figure is called everywhere it is computed, checked and reported is what a cost basis *brought in*. The totals line under the listing reads `Total USD cost basis balance:`, where it read `Total USD basis balance:` — **not** a substring of the new text, so a script matching that line stops matching rather than matching something wrong. The last column is four characters wider to fit its heading.

**The `--verify-costs` report block changed with it.** Its `available` label — which has said `basis balance` since v0.4.0 — now reads `cost basis balance`, and every label in that block is two characters wider so the values still line up: `split guid`, `tx guid`, `amount`, `value`, `transaction rate`, `computed cost`, `stored cost`, `used` and the factor rows. The samples in README and `docs/multi-currency.md` show what the command prints.

### What the commands cost, measured across the supported GnuCash versions

**[docs/performance.md](docs/performance.md) states what each command costs and how the engine version changes it.** Newer GnuCash reads faster and writes slower: the same 2,000-transaction `import --new` takes 2.8 s on GnuCash 3.4 and 5.2 s on 5.16, while re-importing an unchanged ledger, exporting, `fx-balances --verify-costs` and `find-transactions` are two to three times faster on 5.16 than on 3.4. The Python version is not the cause — a fixed arithmetic loop runs the same on 3.7 and 3.14 — and the private copy this tool makes before reading a book below GnuCash 4.8 is 0.2 ms of a 2.0 ms open. The doc also carries the suite times per build, for contributors choosing which one to run.

### Every line and branch is covered, and three gates hold it there

**The suite covers 100% of the lines and branches in `cli/`, `services/`, `infrastructure/`, `use_cases/` and `repositories/`, counted across all eleven supported GnuCash builds.** That is 14,603 statements and 5,294 branches with none missed, against 94% at v0.4.0 and 89% when the measurement started. The lines were the instrument and not the goal: nearly every remaining gap was a behaviour nothing had asked about, and asking found the defects listed above — the credit an overpayment leaves counted twice when the file divides the payment, a corrected memo that never reached the bank split of a bill posted to a plain liability, a payment through no bank reported as an orphan by `unpost-invoices`, and an unreadable cost basis balance destroyed when GnuCash divided the credit carrying it.

**Only the union of the eleven reaches it.** One build on its own measures between 99.04% and 99.44% of the same tree, because a path a particular GnuCash takes runs on the builds carrying that version and nowhere else: the date setter 3.4 needs, the lot wrapper 5.15 returns, the private copy taken before reading a book below 4.8. Each of those is ordinary code on one build and unreachable on the other ten.

**That figure is the Python.** The report file that draws the plaintext balance sheet and income statement is Scheme, run by GnuCash rather than by this process, so no line counter sees it. What covers it is the statement tests, which assert every figure on both pages against what GnuCash's own reports print — for a book kept in HKD, a CAD book holding US dollars, Hong Kong dollars and shares, a book owing US dollars it borrowed with Canadian ones, a book running a loss, a book using trading accounts, and a book with no accounts at all.

**`./scripts/coverage.sh` gates at 100 now, the pre-commit hook gates the same figure, and CI gates twice** — each of the eleven builds against 98% of its own run, then all eleven combined against 100%. The hook runs the eleven suites it already ran, with coverage on, so the gate costs it nothing extra. CI runs one suite fewer than before: the job that adds the eleven together re-runs nothing, where a twelfth full run of the `latest` suite used to exist only to produce a Codecov figure no gate read. Codecov is sent the union now, and [docs/issues/T-009](docs/issues/T-009-line-and-branch-coverage-is-not-measured-or-gated.md) carries the per-build figures.

### Breaking: ledger shapes that no longer import

**A line the import cannot read is refused rather than passed over.** Each of these imported before — some silently, some by being ignored — and a file holding one now fails with the line it is on, before anything is written. A hand-written ledger is the file to check; an export written by this tool holds none of them.

- **Blocks out of their order inside an `invoice` or a `bill`.** They come as `entry:` blocks, then `posted:`, then `payment:` — the order the export and a printed page write, and the order README states.
- **A line that is not a block, not a split and not a `key: value`.** A note typed where a key belongs, or a split whose amount is written in a way the format does not read, went missing from the book with nothing said.
- **A block indented under a `transaction`**, such as `posted:`. Every line under a transaction was taken for a split, so the run failed with `'account'` — the name of a key the block did not have — and no line number.
- **Anything but keys and `open_prepayment:` blocks under an `open` line**, and anything but `PaymentSplit` lines under a payment's `Transaction` block. A `payment:` block under an `open` line, or a split line written among a payment's splits, parsed cleanly and did nothing.
- **A tax table with no name (`taxtable ""`) or with no `entry:` block.** GnuCash's own dialog saves neither. The empty-name one was created, and an export then wrote no `tax_table:` on the lines using it, so re-importing that export changed every one of them.
- **An account with no name: `open ""`, and `open Assets::Savings`**, which has an empty name between two colons. The rest of the file is still imported.
- **The old spellings `split_guid:` and `payment_split_guid:`.** A split states its guid with `guid:` and a payment states its settling split with `txn_split_guid:`. Read as unknown keys, the old ones were stored beside a guid GnuCash had minted itself, or ignored so that the payment was recorded against the wrong split.
- **`lot_owner:` without an owner kind.** It is `customer:ID` or `vendor:ID`; an id with no kind matches no lot.
- **Two `posted:` blocks on one record, and two `payment:` blocks stating one split.** A record is posted once, and one split is one settlement stated once.
- **A payment figure nothing can read.** `amount:`, `prepayment:`, `settled_amount:` and `share_price:` are numbers written with a point.
- **A payment stating its own record's posting as `txn_guid:`**, and one stating a `bank_account:` its transaction never used. The export prints the posting's guid a few lines above the `payment:` block that takes `txn_guid:`, so it is an ordinary thing to write by mistake.
- **A split on an account GnuCash keeps with no commodity.** A split is written as an amount of its account's commodity, and `export` already refused such a book; `import` took it at exit 0.
- **A file indented two ways.** The first indented line sets whether the file uses tabs or spaces and how many make a level; a later line indented the other way cannot be placed.

**And a save GnuCash refuses is a failed command.** GnuCash keeps a backup under the second the save happens in, and when that name is already taken the save stops with `ERR_FILEIO_BACKUP_ERROR` and the book is not written — measured on 5.10, a book option set before such a save reads back unset afterwards. That was read as a harmless collision, so every affected command reported itself done while nothing had been saved.

### Refusal wording

**A guid is stated, a split is applied, and a guid the book has nothing for matches nothing.** Saying a payment "names" a split told a reader nothing — every split has a name — and a payment does not choose or select one either. Every refusal that used the word has been reworded, including `posted_txn_guid '…' names no transaction in this book`, which now reads `matches no transaction in this book`.
...
An edit now rewrites the lines the file's blocks state and leaves the rest of the invoice alone. A line no block states is removed; a block stating no line is added.

**A hand-written file states no guid, and that goes on working**: a block without one edits the line in the same position, so the second `entry:` block edits the second line. A file that mixes the two is read guid-first, the unnamed blocks taking whichever lines are left over in order — which is what a file edited by hand out of an export looks like when a guid is deleted along with the line it was on.

**Two files are refused rather than guessed at.** The same guid on two lines of one invoice: a guid is one line, so the second block would fall through to position and edit some other line, and the invoice would come out of the import stating something the file did not say. And a guid the book assigned to something else — another invoice's line, an account, a transaction: forcing it leaves two objects with one guid, and GnuCash finds an object by looking its guid up in a hash, so the loser is unreachable. The check that answers "is this guid free" now asks the collections a line, a split and an invoice live in; before, it asked about accounts, transactions, customers, vendors and tax tables only, and answered "free" about all three of those.

**A split block is matched by its `guid:` too.** Position decided it before, whatever the block said, so a transaction whose two `Expenses:Dining` blocks were rewritten the other way round moved the amounts between the two splits and reported `Updated: 1` — the book then contradicting the file just imported into it. Two splits of the same amount are the case that moved in silence: 15.00 for coffee and 15.00 for cake swap their *memos* and nothing else, so no total changes, no balance changes, and no figure looks wrong. Two blocks stating one split's guid are refused, as two blocks stating one line's guid are: the second would fall through to position and update a split the file never mentioned. So is a block stating a guid the book holds on something that is not a split of that transaction. And **a split keeps its guid when its account changes**: the blocks are paired within one account, so a block whose account line was changed used to find an empty group, build a new split with a guid GnuCash minted, and leave the split its guid matched to be destroyed as an orphan of the account it had been in — the identity lost on the commonest edit anyone makes to an exported ledger. The split is moved to the account its block states, and pairs there. Not one sitting in a lot, which is settling an invoice or standing as an owner's credit: moving it, or dropping it because no block states its guid, takes a settlement out of its invoice's lot while the account's balance stays put — nothing looking wrong, and the invoice reading unpaid. Both are refused, and the refusal prints the lot's guid. **And a guid nothing can parse is refused** rather than read as a block stating no split at all: read that way it fell straight back to position, the pairing the guid is written to end, and a block creating a split lost the guid as quietly — the split came back under one GnuCash minted, and the next export contradicted the file that made it.
...
### An invoice or bill line carries every column GnuCash keeps for it
...
**A bill entry's `action:` is written now**, on both `export` and `print-bill --format plaintext`. It was left out on the belief that GnuCash stored the action on the invoice side only; an entry whose action is set to `Material`, saved and reopened, reads back `Material`.
...
The sheet follows the machine, as GnuCash's does: no paper size is named, so GTK takes the locale's — A4 in most of the world, US Letter under `en_US` and `en_CA`. WeasyPrint used its own default instead, which is how the same book printed A4 here and Letter from GnuCash on a Canadian desktop.
...
**New: `set-invoice-style` sets the footer and the page's CSS.** The footer and the CSS are the two boxes GnuCash's report options list as Display → Extra Notes and Layout → CSS, and until now setting either meant opening GnuCash — no use on a machine printing from a script:
...
**A file stating a guid the book assigned to another object fails.** An `entry:` line stating another invoice's line guid, or the same guid on two lines of one invoice, was written into the book before, leaving two objects with one guid — GnuCash finds an object by looking its guid up in a hash, so one of the two became unreachable. The refusal prints the guid and what the book has it on. Remove the `guid:` line to add the block as a new line instead. A guid the book simply has not got is not affected: that is the guid a new line asks for, and it is created with it.
...
**`guid: 0`, `guid: 000` and thirty-two zeros fail.** The first two used to read as a block stating no guid at all — an unquoted number is a number and zero is falsy — so the block fell through to positional matching while `guid: "0"` was refused. Thirty-two zeros used to be accepted: that is GnuCash's null guid, meaning *no* guid, and every writer here treats it as absent, so a lot stamped with it exported without its `lot_guid:` line.
...
**A `payment:` block may no longer spend a foreign account whose cost bases still have a balance.** Cash leaving a foreign account is a disposal and has to state the cost basis it comes out of, and a payment block has nowhere to state one — GnuCash's own `ApplyPayment` writes the bank split. Such a payment is now refused, and the refusal prints the account's name, what the payment spends, and what balance its cost bases still have between them.

This reaches ledgers that imported cleanly before, because settling *into* a foreign account is itself what opens a cost basis on it, and it is asked of every foreign bank rather than only one in a third currency — paying a USD bill out of a USD bank whose cost bases still have a balance drifts the same way and reaches none of the cross-currency arithmetic. Write the settlement as an ordinary transaction whose bank split carries `cost_basis_split_guid:`, and attach it to the invoice with `txn_guid:` / `txn_split_guid:`. README's foreign-currency section shows the shape, and [docs/multi-currency.md](docs/multi-currency.md) lists the refusal beside the others.
...
- **a rate stated in a third commodity, or in shares**. A split's value is in the transaction's currency, so the rate has to be too: 15,000 yen quoted per USD inside a CAD entry was valued at 100.00 CAD instead of 135.00, and an exchange ratio (`-100 OLDCO @ 0.5 NEWCO`) valued the shares disposed of at 50.00 CAD — a figure with no source in the file;
...
A printed payment now also states `prepayment:`, the residue an overpayment leaves on the owner's account, which only the ledger export used to write. Without it a printed 250.00 deposit against a 100.00 invoice was rebuilt as a 100.00 payment — the bank 150.00 lower than the deposit, the customer's credit never created, and the run exiting 0. On a block that names a transaction, `amount:` is this invoice's own slice and `prepayment:` what the movement left over, so rebuilding one makes the whole movement; a block naming no transaction still reads `amount:` as the payment to make, unchanged.
...
Two spellings of the same destination also disagreed: on `export-transaction` and `delete-transactions`, `-o file.txt` wrote UTF-8 while `-o -` wrote in whatever encoding the locale set. Both write UTF-8 now, on those two and on the print commands, which matters because `-o -` is the form piped back into `import`.
