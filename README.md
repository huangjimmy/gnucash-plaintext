...
**USD in against CAD out is buying or borrowing USD, and gets a cost basis.** `Assets:Bank:USD 2720.00 USD / Assets:Due from director -3815.89 CAD` does not balance without a `share_price:` on the USD split, and that rate is what says what the USD cost — so importing it opens a cost basis of 2,720.00 USD. Correct for what the transaction says — a USD asset is debited and a CAD asset is credited, which is buying USD, and crediting a CAD liability instead is borrowing it. Nothing reads what the other account is *for*: GnuCash has no suspense or clearing account type, and a director who really owes you CAD and settles it in USD writes exactly these two lines. It is still a price you may not have meant to give, so book the other side to an account kept in USD and there is no rate to state, no cost, and no basis.

**A transaction that turns out to be paying an invoice is not a borrowing, and gives up its cost basis.** Link it with a `payment:` block that gives `txn_guid:` and the invoice is paid in full; the invoice's own posting split has had a cost basis for that same USD since it was posted, so keeping both would give one lot of USD two. The link is refused while a disposal already draws on the basis being discarded, listing those disposals with their dates and amounts: the `share_price:` on the USD split and the rate the invoice was posted at need not agree, so a disposal valued against the first cannot be pointed at the second. Delete it, link, and import it again. A **bill** is the mirror and keeps its basis: USD drawn on a credit line to pay a supplier is still borrowed once the payable is paid, so the transaction is still a borrowing.

**A book linked by an earlier version** still has that balance, on a split nothing reads. `fx-balances --verify-costs` finds it and prints the split's guid. If nothing drew on that basis, one `--strategy update` file with `cost_basis_balance: ""` on the line takes it off and the book is done. If something did — a fee spent out of the deposit — that disposal cannot be re-pointed in place; delete it with `delete-transactions … --by-guid -o fee.txt`, clear the balance, then import `fee.txt` again with `cost_basis_split_guid:` changed to the invoice's basis. [Multi-currency](docs/multi-currency.md) walks both.
...
**Nor is an edit the finished book could not check.** The deferral rests on those questions being asked afterwards, and one of them — is a disposal still valued at what its basis cost — can only be asked of a disposal stated in the book's own currency, a transaction between two foreign currencies stating its values in neither. So an edit to a transaction whose basis has such a disposal drawing on it is refused as it lands, and the refusal says which disposal makes it so.

**What is not deferred either is a block restating what a disposal takes**, and that one is refused whether the flag is passed or not. A basis balance falls only where a block draws it down, and no edit does, so the finished book reads the same balance whether such a block landed or not: a 10.00 USD fee restated as 400.00 against a basis offering 90.00 satisfies every question that can be asked afterwards. Re-pointing that fee at another cost basis moves no figure and is what a repair does, so it goes through. Taking the `cost_basis_split_guid:` line off altogether is refused for the same reason as restating it: the currency came out of the basis when the fee was imported, and nothing on this path gives it back. To change what a disposal takes, or to stop it drawing on anything, delete the transaction and import the new version — `delete-transactions --by-guid` gives the basis back exactly what it took, and the fresh import checks the new figures against it. The finished book is read once, before a byte is saved — every cost against the ledger it comes from, every balance within what its cost basis brought in, no figure stored where nothing reads it, every disposal drawing on something that is a cost basis in the book the file leaves, holding the currency that disposal sells, valued at what that basis cost, and against a receivable that has been collected. A file that leaves any of those wrong **that it introduced** is rolled back with all of them listed, and the book is untouched. The same questions are asked of the book before the file is read, and a fault already there is not this file's to answer for: refusing over one would leave a damaged book unable to accept any import, including the one repairing it. Such a fault is still reported by `fx-balances --verify-costs` afterwards, which is where a reader is meant to see it. The per-currency totals are not among them: `--verify-costs` prints those as a warning and says on the page that nothing is refused over them, and a book with a divided credit cannot level them at all — the arrived side counts what the remainder holds while the sale that drew on the pool is still the size it was.

A stated `cost_basis_balance:` is what it should be **when the file has landed** — net of the file's own disposals — so a repair that moves a disposal onto another basis states that basis's balance after the move.
...
`--verify-costs` re-checks every cost against the ledger it is derived from: that no basis balance is above what its basis brought in or below zero, that any stored `cost_basis_cost` parses and agrees with the transaction, that no split stores a `cost_basis_balance:` while being no cost basis at all — a figure the listing cannot show you, which the export writes back out, so a book rebuilt from that ledger holds it in the same place and only `cost_basis_balance: ""` clears it — that no sale gives a `cost_basis_split_guid:` matching a split which is no cost basis, which is what is left once such a balance is cleared and the guid on the sale below it is not, and that every sale which does give one is still valued at what that basis cost and draws on a receivable that has been collected — the two questions the import asks of a sale in a file, asked here of the sales already in the book, which is the only way either reaches a sale nobody is importing. A basis re-priced *under* the sales measured against it is what the first catches. The first two say why the split is not a basis: its currency is the book's own, it spends rather than brings in, or nothing in its transaction says what its currency cost. A basis the book consumed is not reported — an owner's credit spent on their next invoice ends the pool it was, and the sale that drew on it beforehand keeps giving its guid. It runs the whole book, prints each basis that disagrees with the full computation behind it — both guids, the amount and value, every factor multiplied, and both answers with the one used — and exits 1 at the end if anything did. A basis whose figures cannot be read is reported with its traceback rather than ending the run. Rates are not checked, because rates run forward only: a file states one, the amount is multiplied by it and rounded, and the effective rate the ledger is then left with — 45.00 USD at 1.405 books 63.23 CAD, a ratio of 6323/4500 — is that rounding rather than a discrepancy. Reading a figure back to ask which rate produced it has no answer. (Nor is a split's `share_price` checked against its value: GnuCash computes the rate *as* value over amount, so the comparison is one number against itself.)
