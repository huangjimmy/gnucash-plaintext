...
`gnucash-plaintext` provides a round-trippable plaintext format for your
...
- `Trading`, the accounts GnuCash keeps for a book with "Use Trading Accounts" on, which its export writes
...
A block with any other key is refused, and the refusal lists the key. A price has no custom metadata to keep it in, so a key `import` does not read would be dropped, and a correction written as `valeu:` would be lost with the price reported `unchanged`.
...
`import` takes a time with any offset. A bare date, `time: "2026-01-02"`, is stored at the time GnuCash's own transfer dialog and CSV price import store a date at: 10:59 UTC in most timezones. The measurements behind this, for every GnuCash path and every supported version, are in [Q-041](docs/issues/Q-041-a-price-cannot-be-recorded-for-a-past-date-or-kept-through-export-and-import.md).
...
- **creates a price** otherwise, with the block's `guid:` when it has one, and reports it `created`;
- **refuses the block** when the book already holds another price for that commodity and currency on that day, either way round, or when an edit would move a price onto such a day. The refusal lists that price's guid, time and source; to change that price, state its guid;
- **refuses both blocks** when one file states two prices for the same commodity and currency on the same day, either way round;
- **refuses every block stating a `guid:` that another `price` block in the same file also states**, since a guid is one price: two new prices would both take it, and of two edits the later would win;
...
- **applies edits before new prices**, so one file may move a price off a day and state a new price for that day, in either order. An edit moving a price onto a day that another edit in the file frees waits for that edit, so the order of the blocks does not matter. Two edits that wait for each other, such as two prices swapping days, are both refused;
- **refuses a block whose `guid:` another kind of object in the book holds**, such as a transaction, an account or a customer, because GnuCash keeps each guid to one object. A block of another kind stating a price's guid is refused the same way;
- **refuses a block that would change an existing price's commodity or currency.** A price is of one commodity in one currency, and GnuCash files it under that pair; a price in another currency is recorded as a price of its own.
...
A price's source is what put it in the book, and GnuCash uses it to decide which of two prices on one day to keep when its own Price Editor or Finance::Quote adds one. `export` writes the source and type exactly as the book holds them, and `import` stores exactly what the block states. gnucash-plaintext never sets a source of its own on a price.
...
The book reports in CAD. Currency in any other denomination is held at a **cost basis**: so many units, at what they cost in CAD. Every split that brings foreign currency in establishes one — an invoice's A/R split, a bill's A/P split, currency bought or borrowed — and selling that currency is measured against the cost bases it came from.
...
2026-03-01 * "Sell 100 USD"
	...
	Assets:Bank:USD -100.00 USD
		share_price: "1.40"                        # valued at what the cost basis cost
		value: "-140.00"
...
```
error: Buy 20 AMZN at 200.00 USD: this transaction is a purchase of 20.0000
AMZN with 4000.00 USD the book held: Assets:USD Bank, a Bank account in USD,
is credited 4000.00 USD; Assets:AMZN, a Stock account in AMZN, is debited
20.0000 AMZN. A purchase requires a consumption of one or more cost bases, but
no split says which. State `cost_basis_split_guid:` on the split that disposes
of it, with the guid of the cost basis the USD came out of — `fx-balances`
lists them.
```

The refusal says what kind of disposition the transaction is, read from the account types of its splits, and lists the splits it read that from, each debited or credited. There are only so many kinds: a purchase, where a security is debited; a repayment, where a balance owed in the same currency is reduced; a payment, where a receivable or a payable in the same currency is debited; a sale, where a balance-sheet account in another currency is debited, whether dollars sold for Canadian or Hong Kong ones or shares sold for dollars; an expense, where an expense account is debited; and a refund, where an income account is debited, as a refund to a customer debits the sale it returns. A US dollar bank credited and a Canadian dollar asset account debited is a sale, whatever the transaction's description says.

**When the cost basis of a disposal is not chosen yet, write `cost_basis_split_guid: $pending$`** on the split that disposes of the holding. Quotes around `$pending$` make no difference, because no split's guid is `$pending$`. On any other split the line is refused. The disposal is imported without drawing down any cost basis. Until it is resolved, the book's cost bases add up to more than its accounts hold, by the amount of the pending disposals. `fx-balances` lists each pending disposal and their total:

```
3 disposal(s) pending their cost basis: 2,720.00 USD. No cost basis is drawn down for them until an edit states the one each draws on.
2026-08-13   Assets:Wise USD   0.72 USD   Charges for the deposit
```
...
- Money back into that account repays what it owes, and states the guid of the owed cost basis, as any repayment does. 1,000.00 USD of income at 1.40 into an account at −500.00 is one split that does both: it repays the 500.00 owed and brings in 500.00 held. It states the owed cost basis, and is valued at the 500.00 repaid at what it cost, 675.00, plus the 500.00 brought in at 1.40, 700.00, so 1,375.00 against the 1,400.00 of income. `$residual$` takes the 25.00 lost on repaying at 1.40 a debt taken on at 1.35, and the held cost basis opens at 1.40. Valued at 1,400.00, the day's rate throughout as GnuCash's own register writes it, the split is refused and the refusal states the 1,375.00, as it does for a sale valued at anything but its cost:
...
- Paying off more than is owed is the mirror. 500.00 from a US dollar bank onto a card owing 300.00 repays 300.00, stating the card's owed cost basis and the bank's held one, and moves 200.00 to the card's credit. The bank's cost basis falls by the 300.00 the held side lost, and the bank's split is valued at what all 500.00 cost.
...
A book imported before a balance past zero had a side of its own has no owed cost basis for an account an earlier import took below zero, so the next money into that account repays a debt the cost bases never held. Bring it forward through its export, as a book whose disposals state no cost basis is: the rebuilt book opens the owed cost basis, and a later deposit repaying it is refused until it states that cost basis's guid. [Q-047](docs/issues/Q-047-record-a-cost-basis-on-the-other-side-for-the-part-of-a-balance-past-zero.md) works through each case.
...
```
error: Repay the loan, written wholly in US dollars: this transaction pays off
1500.00 USD owed, which cost 1.35 CAD/USD, with 1500.00 USD held, which cost
1.3 CAD/USD, so it realizes a difference of 75.00 CAD. It is written wholly in
USD, so no split in it can state that figure. Write it in CAD, each USD split
valued at what its cost basis cost, and put the difference on a `$residual$`
split.
```

Where both sides cost the same a dollar nothing is realized, and the repayment is imported as written.
...
**A transfer shares a transaction only with a fee written as a split of its own that states its cost basis.** 4,000.00 USD moved from one US dollar account to another draws down no cost basis and opens none. A fee paid out of those accounts in the same transaction, or more dollars bought into them, does, and one split cannot say both: 4,010.00 USD leaving the bank and 4,000.00 arriving in savings does not say which 10.00 was the fee. So that is refused, and written as the transfer and the fee, or the transfer and the purchase. Written as a bank writes it — 4,000.00 out of the bank, 4,000.00 into savings, and the 10.00 fee on a split of its own with `cost_basis_split_guid:` — the fee says it is what left, the rest is a transfer, and the transaction is imported, or an edit into it accepted: the fee draws its cost basis down by 10.00 and nothing else moves one. Two splits on one account are not a transfer: 100.00 USD bought with the bank keeping 1.00 of it as its fee is a purchase and a spend, and the fee states the purchase's position as the cost basis it draws on (below), which leaves 99.00. A fee stating no cost basis is refused, and the refusal shows the two ways to write it: the arrival net of the fee, so the fee is part of what the dollars cost and no fee is recorded, or the fee kept, with every cost basis it could draw on listed as the line to add.
...
- state `cost_basis_split_guid:` on the split that spent the currency — `fx-balances` on the book the import made lists the cost bases to choose from;
- state that split at what its cost basis cost, and put the difference on a `$residual$` split, as every disposal is written;
- and take off the `cost_basis_balance:` the export wrote on the cost basis it draws on. That figure is the old book's, which drew nothing down for the disposal, and a stated balance is taken as already net of every disposal.

An earlier release also stored a `cost_basis_balance` on the receiving split of a transfer between two accounts of one currency stated in Canadian dollars, reading it as dollars arriving. They arrived nowhere, so that split is no cost basis, and `fx-balances --verify-costs` reports the figure and says so. Take the `cost_basis_balance:` off that split in the export, and state on each disposal out of that account the guid of the cost basis the dollars came from.

A cost basis with no stored `cost_basis_balance` — one made in GnuCash's register — is read as holding nothing, as `fx-balances` lists it `none recorded`, so a disposal the book makes out of that currency is not asked for a guid on its account alone.

Run the import with `--verify-integrity`, and it says whether the cost bases then hold what the accounts hold. `tests/integration/test_a_book_whose_disposals_state_no_cost_basis_is_brought_forward.py` does it for a spend written in GnuCash's register.

**A file that opens a cost basis and spends it in one run states the arriving split by its position in the file.** `cost_basis_split_guid:` takes the guid of the split the cost basis sits on, and GnuCash assigns that guid as the file is read, so there is nothing to copy out of `fx-balances` yet. Write instead the variable `$transactions_to_import[n].splits[m].guid$`: the file's n-th transaction block and its m-th split line, each counted from 0 in the order the file writes them. `open`, `commodity`, `price`, `invoice`, `bill` and `company` blocks are not counted. Write it unquoted, as `$residual$` is written; in quotes it is a string, read as a guid, and matches no split. It is read only as `cost_basis_split_guid:` on a split of a transaction; written anywhere else, such as a payment block's `txn_split_guid:`, the file is refused before any of it is applied.
...
The arrival opens a cost basis of 2,720.00 USD costing 3,800.00, and the fee draws 0.72 of it, leaving 2,719.28. The position points at a split of the same transaction or of one above it. One below has not been imported when it is read, so the whole file is refused before any of it is applied, saying to move that block above. What the book keeps is the guid of the split it points at, exactly as though the file had written it, so `export` writes the guid. One transaction may mix the two: a split stating a guid draws on a cost basis the book already holds, and another stating a position draws on one the file brings in. Under `--strategy update`, and for a transaction the import passes over as already in the book, a position is the `guid:` on the line it points at, which an export writes on every split.

The other way is to write `guid:` on the arriving split yourself and state the same guid on the disposal below it:

```
2026-01-10 * "Buy 45 USD at 1.405"
	currency.mnemonic: "CAD"
	Assets:Bank:USD 45.00 USD
		guid: "0c0c0c0c0c0c0c0c0c0c0c0c0c0c0045"
		share_price: "1.405"
		value: "63.23"
	Assets:Bank -63.23 CAD

2026-03-01 * "Sell 45 USD for 62.00 CAD"
	currency.mnemonic: "CAD"
	Assets:Bank:USD -45.00 USD
		share_price: "1.405"
		cost_basis_split_guid: "0c0c0c0c0c0c0c0c0c0c0c0c0c0c0045"
	Assets:Bank 62.00 CAD
	Income:FX Gain $residual$ CAD
```

`cost_basis_balance:` on a split is how much of *that cost basis* has not yet been sold; it is not the balance of an account. One bank account can hold currency from several cost bases at different costs, and a paid invoice's cost basis keeps its balance after the money has moved to the bank. The KVP is the record of that balance: it opens at everything the split brought in, each sale lowers it, and deleting a sale raises it again. A file that states a balance is stating it net of that file's own sales, so re-importing an export never counts a sale twice. A cost basis carrying no such KVP — one written in the GnuCash GUI, or predating this — reads as `none recorded` rather than as its full amount, because how much of it was already sold is not known. Writing `cost_basis_balance:` on that split in an import file is how it gets a balance — checked as it lands, since nothing after that questions it: it must be on a split that holds foreign currency, and then a number, not negative, no finer than the unit its own account is held to, and no more than that split brought in.

**USD in against CAD out is buying or borrowing USD, and gets a cost basis.** `Assets:Bank:USD 2720.00 USD / Assets:Due from director -3815.89 CAD` does not balance without a `share_price:` on the USD split, and that rate is what says what the USD cost — so importing it opens a cost basis of 2,720.00 USD. Correct for what the transaction says — a USD asset is debited and a CAD asset is credited, which is buying USD, and crediting a CAD liability instead is borrowing it. Nothing reads what the other account is *for*: GnuCash has no suspense or clearing account type, and a director who really owes you CAD and settles it in USD writes exactly these two lines. It is still a rate you may not have meant to pay, so book the other side to an account kept in USD and there is no rate to state, no cost, and no cost basis.

**A transaction that turns out to be paying an invoice is not a borrowing, and its cost basis is discarded.** Link it with a `payment:` block that states `txn_guid:` and the invoice is paid in full; the invoice's own posting split has had a cost basis for that same USD since it was posted, so keeping both would put two cost bases on one lot of USD. The link is refused while a disposal already draws on the cost basis being discarded, listing those disposals with their dates and amounts: the `share_price:` on the USD split and the rate the invoice was posted at need not agree, so a disposal valued against the first cannot be pointed at the second. Delete it, link, and import it again. A **bill** is the mirror and keeps its cost basis: USD drawn on a credit line to pay a supplier is still borrowed once the payable is paid, so the transaction is still a borrowing.

**The link is refused where the balance it would discard is below what the split brought in.** That difference is currency sold outside this book — a balance a file states is authoritative, and it is how a book carrying sales this tool never saw gets one — so discarding it is the only record of that currency going, and taking the payment off later would open the split at its full amount. The refusal states both figures; write `cost_basis_balance: ""` on that split first if the difference is not currency this book should account for.

**A book linked by an earlier version** still has that balance, on a split nothing reads. `fx-balances --verify-costs` finds it and prints the split's guid. If nothing drew on that cost basis, one `--strategy update` file with `cost_basis_balance: ""` on the line takes it off and the book is done. If something did — a fee spent out of the deposit — re-point that disposal at the invoice's cost basis with one `--strategy update` file: the edit is read as a new transaction would be, puts back on the deposit's cost basis what the fee drew from it, and draws it from the invoice's. Then clear the balance. [Multi-currency](docs/multi-currency.md) walks both.

**Paying out of a foreign account whose cost bases still have a balance.** Cash leaving a foreign account is a disposal like any other, so it has to say which cost basis it comes out of — and a `payment:` block has nowhere to say it, because GnuCash's own `ApplyPayment` writes the bank split and `cost_basis_split_guid:` cannot be put on it. Such a payment is therefore **refused**, and the message states the account's name, what the payment spends, and what balance its cost bases still have between them.
...
The way to spend a cost basis balance is the way every other foreign disposal is written — an ordinary transaction whose bank split states its cost basis's guid, attached to the invoice by `txn_guid:` / `txn_split_guid:`:
...
#### A book that keeps no cost bases

Cost bases are gnucash-plaintext's, not GnuCash's: GnuCash asks no one which lot a dollar came out of. A book whose owner does not want them says so in its `company` block:

```
company
	name: "Example Books Inc"
	cost_bases: "off"
```

The switch belongs to the book, so every command reads the same answer. `"on"`, or no line, keeps cost bases; any other value is refused. `set-book-key` refuses the key and leaves it to the `company` block. A book that keeps none is not turned on in place: `"on"`, or the line cleared, is refused there (see below).

With cost bases off:

- **The import neither checks nor records them.** No disposal is asked which cost basis it draws on, no refusal built on cost bases runs, and no `cost_basis_*` key is written. A file stating one into such a book is refused, so a book is never half on.
- **Turning them off clears the keys earlier imports wrote**, and the import says how many splits held one.
- **Realized gains are what the transactions record.** A realized gain needs no cost basis: a `$residual$` split beside a split in another currency or a security, whichever way that split moves, is the difference the file states, and `realized_gains_fx` and `realized_gains_other` are read from those splits as in any book. The gain itself is the balance of the income or expense account the file posted it to, and it is in `retained_earnings`; the two keys list it by kind where the transaction disposes of one kind. A `$residual$` beside both a currency and a security is in that account and in `retained_earnings`, and on neither key, since one split cannot say how much of it is which. So a bank charge beside a split in another currency gets a line with its amount, not the `$residual$`: taken as the `$residual$`, it is stated as a realized difference.
- **Unrealized gains are measured from GnuCash's own revaluation.** The balance sheet writes `measured_from: gnucash_revaluation # this book keeps no cost bases` under each currency and security. The figures are then GnuCash's, not what the book's own costs would come to.
- `fx-balances` says the book keeps no cost bases and lists what the accounts hold, and `--verify-integrity` lists the cost basis checks as not checked, with the reason.

**Such a book's export rebuilds it** in a new book, imported with `--include-business-objects`: that is what imports the `company` block, and it is applied before the transactions, so the disposals that state no cost basis meet a book that keeps none. Without the flag they are refused.
...
GnuCash sets on a fresh entry — an empty note, no discount, `percent`, `pretax`, not
...
Each option works on its own, none requires another, and any of them can be combined:
...
| `--latest N` | the N most recent prices of each commodity in each currency, counting back from `--end-date`, or from today without it. With `--start-date`, a commodity priced fewer than N times since that day is exported with the prices it has. N is 1 or more |
...
- Each commodity in each currency is counted separately, and so is each direction: a book holding both USD in CAD and CAD in USD exports one of each for `--latest 1`.
...
The output is self-contained — it includes the commodity and account declarations needed to re-import or process the transactions independently. When several GUIDs are passed, shared commodities and accounts are emitted only once. Output goes to stdout by default; use `-o` to write to a file:
...
**`--atomic` applies the file the way a database applies a transaction**: it all commits or none of it does. Use it for a correction, whose blocks are parts of one change — half a correction applied leaves a book in a state nobody chose, and the reader then has to work out which half landed. It is the wrong flag for a bank feed, where a statement of two hundred lines should not be thrown away over one account nobody has opened yet.

**Under it the book is checked once the whole file is applied**, as a database with every constraint deferred checks them when the transaction commits. A block is not refused over where it sits in the file. Transactions are applied before invoices and bills, so, read one block at a time:

- a fee drawing on an invoice's cost basis is refused, because the invoice's `payment:` block has not collected it yet;
- a deposit is refused another account while its split still settles an invoice, whose `payment: none` comes after it.

No order of the blocks suits every file: a file booking a statement line wants the transactions first, and the file that undoes it wants the payment blocks first. So under `--atomic`:

- **A refused transaction block is applied again once the rest of the file is in the book**, and again for as long as another pass applies something, through the same checks as the first time.
- **What depends on another block is asked of the book the file leaves.** A split drawing on a cost basis draws on what the file's version of its transaction states, so a deposit whose fee the file moves onto the invoice's cost basis may stop establishing its own. A split settling an invoice or a bill, moved to another account, is taken off that record, as `unapply-payment` takes it off, where the file states the record and none of its `payment:` blocks states that transaction. A record the file does not state, one it still pays with that transaction, and an owner's credit are not taken off, and the edit is refused as it is without the flag.
- **What no order can apply is applied in place, and checked at the end.** That is two edits each refused while the other still reads as it was: a cost basis another transaction draws on, and that transaction's draw, restated together.

A block wrong in itself is still refused wherever it is: an account the book has not got, a guid nothing can parse, a transaction that does not balance.

**This is how a booking is undone.** Export the book before booking a statement line, keep the file, and import it with `--strategy update --include-business-objects --atomic` to go back to it. The deposit returns to the suspense account, the invoice to unpaid, and every cost basis to what it held. gnucash-plaintext keeps no history and does not know the file is an earlier state; it applies the file and checks the book it leaves. Importing the export taken after the booking books it again. One thing stays: the memo a `payment:` block wrote onto the settlement's splits. The earlier export has no `memo:` line there, and a line a file leaves out says nothing about it (see [What a key says, and what leaving it out says](#what-a-key-says-and-what-leaving-it-out-says)).

**Nor is an edit the finished book could not check.** The deferral rests on those questions being asked afterwards, and one of them — is a disposal still valued at what its cost basis cost — can only be asked of a disposal stated in the book's own currency, a transaction between two foreign currencies stating its values in neither. So an edit to a transaction whose cost basis has such a disposal drawing on it is refused as it lands, and the refusal says which disposal makes it so.

**A block restating what a disposal takes, re-pointing it, or clearing its pick is read as a new transaction would be**, whether the flag is passed or not: what it drew is put back, and it draws what it now states, meeting every check a new disposal meets. A 10.00 USD fee restated as 400.00 out of an account holding 100.00 draws all 100.00 and owes the other 300.00; re-pointed at another cost basis, it puts back on the first what it drew and draws on the second; with its `cost_basis_split_guid:` line cleared, it spends dollars the book holds without stating a cost basis, and is refused as a new spend is. A block moving the date of a transaction whose cost basis another transaction draws on is refused as it lands: no question asked of the finished book reads what a cost basis held on each day in between, which is what a date moves. The finished book is read once, before a byte is saved — every cost against the ledger it comes from, every balance within what its cost basis brought in, no figure stored where nothing reads it, every disposal drawing on something that is a cost basis in the book the file leaves, holding the currency that disposal sells, valued at what that cost basis cost, and against a receivable that has been collected. A file that leaves any of those wrong **that it introduced** is rolled back with all of them listed, and the book is untouched. The same questions are asked of the book before the file is read, and a fault already there is not this file's to answer for: refusing over one would leave a damaged book unable to accept any import, including the one repairing it. Such a fault is still reported by `fx-balances --verify-costs` afterwards, which is where a reader is meant to see it. The per-currency totals are not among them: `--verify-costs` prints those as a warning and says on the page that nothing is refused over them, and a book with a divided credit cannot level them at all — the arrived side counts what the remainder holds while the sale that drew on the pool is still the size it was.

A stated `cost_basis_balance:` is what it should be **when the file has landed** — net of the file's own disposals — so a repair that moves a disposal onto another cost basis states that cost basis's balance after the move.

Exit code 1 if the run reported any error, whether or not the rest imported: what did import is saved first, and the exit code is how the run says the rest did not. `--atomic` also exits 1 when it rolls back, including where every block applied and only the finished book was wrong. A file the parser could not read at all imports nothing and is refused outright. A book `--new` created for a run that saved *nothing* is removed again, so a retry is not blocked by a file you never made — but a run that saved some of the file keeps its book, because that book now holds work, and the fix is to import the corrected ledger into it rather than to start again. Exit code 0 means nothing was reported as an error — read `Skipped:` for objects that matched an existing GUID and were left as they were, which under the default strategy includes a transaction you edited and re-imported without `--strategy update`.
...
`cost_bases: "off"` keeps the book's foreign currency as GnuCash does, with no cost bases (see [A book that keeps no cost bases](#a-book-that-keeps-no-cost-bases)).
...
* The new payment block has no `txn_guid:` — the importer calls `ApplyPayment` to create a fresh bank-side transaction. Where the bank statement is imported too, link its line instead, as the next bullet does (see [Pay by linking the bank's own transaction, and import with `--atomic`](#pay-by-linking-the-banks-own-transaction-and-import-with---atomic)).
...
Every split that brought foreign currency into the book, with what one unit cost in CAD and how much of that cost basis is still available to sell. The split guid is what a sale names in `cost_basis_split_guid:` (see [Foreign currency](#foreign-currency)).
...
Total USD cost basis balance: 160.00 USD
```

Read-only. The cost is stated with its direction — CAD per unit of the currency held — and the cost basis balance shown is the cost basis's own, not any account's balance.

`--verify-costs` re-checks every cost against the ledger it is derived from: that no cost basis balance is above what its cost basis brought in or below zero, that any stored `cost_basis_cost` parses and agrees with the transaction, that no split stores a `cost_basis_balance:` while being no cost basis at all — a figure the listing cannot show you, which the export writes back out, so a book rebuilt from that ledger holds it in the same place and only `cost_basis_balance: ""` clears it — that no sale states a `cost_basis_split_guid:` matching a split which is no cost basis, which is what is left once such a balance is cleared and the guid on the sale below it is not, and that every sale which does state one is still valued at what that cost basis cost and draws on a receivable that has been collected — the two questions the import asks of a sale in a file, asked here of the sales already in the book, which is the only way either reaches a sale nobody is importing. A cost basis re-priced *under* the sales measured against it is what the first catches. The first two say why the split is not a cost basis: its currency is the book's own, it spends rather than brings in, or nothing in its transaction says what its currency cost. A cost basis the book consumed is not reported — an owner's credit spent on their next invoice ends the pool it was, and the sale that drew on it beforehand still states its guid. It runs the whole book, prints each cost basis that disagrees with the full computation behind it — both guids, the amount and value, every factor multiplied, and both answers with the one used — and exits 1 at the end if anything did. A cost basis whose figures cannot be read is reported with its traceback rather than ending the run. Rates are not checked, because rates run forward only: a file states one, the amount is multiplied by it and rounded, and the effective rate the ledger is then left with — 45.00 USD at 1.405 books 63.23 CAD, a ratio of 6323/4500 — is that rounding rather than a discrepancy. Reading a figure back to ask which rate produced it has no answer. (Nor is a split's `share_price` checked against its value: GnuCash computes the rate *as* value over amount, so the comparison is one number against itself.)
...
read as a block without a guid: a block without one is matched by
position, so a guid the reader could not use would put the block's values on
...
Editing an invoice rewrites the lines its blocks state and leaves the rest of
the invoice alone. A line no block states is removed; a block stating a line
the invoice does not have adds it. So correcting one word of one description
changes that one line, and every other line keeps the guid the book assigned
it — which is what anything
...
the blocks without one taking whichever lines are left over in order.
...
Where you kept the export from before the payment was booked, import it with `--atomic` instead: that undoes the booking in one file, and the export taken after it books it again (see [Pay by linking the bank's own transaction, and import with `--atomic`](#pay-by-linking-the-banks-own-transaction-and-import-with---atomic)). `unapply-payment` is for a book whose earlier export nobody kept.
...
As with `unapply-payment`, importing the export kept from before the booking, with `--atomic`, is the recommended way to undo it; `unlink` is for a book whose earlier export nobody kept.

An invoice can be paid without entering any money. The bank feed arrived first, or a director paid a supplier out of pocket, so the transaction is already in the book; a `payment:` block stating its guid puts the receivable's account on one of its splits, and that split becomes the settlement. `unlink` undoes that, and what it leaves is a payment with nothing to do with the invoice any more — the split comes off the receivable, leaves the record's lot, and moves to the account `--to` states, while the invoice owes again:
...
| the account `--to` states | the split takes | rate |
|---|---|---|
...
The third row is the case where nothing on the split is in the currency of the account you are moving it to — an invoice and its payment both in USD, going back to a CAD `Due From Director`. That genuinely converts, so it takes a rate:
...
**What a converting settlement drew out of a cost basis comes back; the realized difference does not.** Settling a 100.00 USD invoice booked at 1.40 into a CAD bank at 1.37 draws 100.00 USD out of the receivable's cost basis — that USD was spent — and realizes 3.00 CAD. Taking the payment off puts back on the cost basis what it drew, because the currency was not sold, it went back to being owed; without that the record was unsettleable, since re-applying the money hit "that USD has already been sold against it".
...
**So a CAD `--to` takes 140.00, not the 137.00 the bank received.** Where a difference was realized the entry is quoted in CAD and the settlement split's value is written at the **`share_price:`** of the posting split that opened the cost basis — its value over its amount, 1.40, so 100.00 USD is valued 140.00 CAD — while the bank split carries what the bank was credited and your `$residual$` line carries the 3.00 between them. The value is the figure a CAD account takes, so it is 3.00 away from the receipt, and it is the only figure that leaves the entry balancing: writing 137.00 would put the transaction 3.00 out, and the only way to close that is to rewrite your line. Naming a USD account instead takes the 100.00 USD unchanged. Measured on that book, the result is a ledger saying both things — 100.00 USD owed and undisposed, and 3.00 CAD realized on disposing of it. Both are true of what happened. **If you then apply the money to another record with another `$residual$` line, the difference is recorded twice** — remove the first line, or rebuild that payment transaction.

**The transaction is a purchase again, and its cost basis comes back holding everything it brought in.** Writing the base-currency account the money came from into `--to` leaves `Due From CAD / Bank USD`, which is buying USD — so the split holding that USD is a cost basis once more, priced by the transaction and open for all of it. Nothing was sold out of it in the meantime: while it settled the record it was a settlement, and a settlement is no cost basis, so nothing could draw on it. A cost basis that read `none recorded` before the link comes back holding all of it too, because by then there is nothing to tell a balance this tool removed from one that was never written.

**Except where part of what the transaction brought in is somebody else's**, in which case the split comes back listed and priced with **none recorded**, and the listing says to state `cost_basis_balance:` on it. That is any transaction which already carried a cost basis of its own — an overpayment's parked credit — or one still holding a split on a receivable or a payable: a share that settles another record, an orphan an unpost loosened, or a share not linked to its invoice yet. One deposit covering two invoices is the plain case: unapplying the first must not open the second's share, whether or not that second invoice has been linked. A split spent out of an owner's credit holds it back too, because spending takes the balance off to record that the currency went, which says nothing about how much of it had been sold beforehand.
...
#### Pay by linking the bank's own transaction, and import with `--atomic`

This is the recommended way to record a payment, and to correct one:

1. Import the bank statement's lines as transactions. A line nobody has identified yet goes to a suspense account, such as `Assets:Due from director`.
2. Once you know what a line paid, export the book, keep the export, and book the line: move its split to the receivable's or payable's account, and state the line's `txn_guid:` and `txn_split_guid:` in the invoice's or bill's `payment:` block.
3. Import the file with `--strategy update --include-business-objects --atomic`.
4. To undo the booking, import the export you kept, the same way. To book it again, import the export taken after the booking.

The bank line is then the one record of the money, and the file states the whole of it. An undo is one import of a file you kept, and `--atomic` checks the book the file leaves, whatever order its blocks are applied in (see `--atomic` under [Import plaintext transactions to GnuCash](#import-plaintext-transactions-to-gnucash)).

Two other ways are still supported, and not recommended with `--atomic`:

- **A `payment:` block with no `txn_guid:`.** GnuCash's `ApplyPayment` makes a new bank transaction for it. When the bank statement is imported too, the same money is then in the book twice.
- **`unapply-payment` and `unlink`.** Each writes to the book on its own, outside any file. A booking undone that way cannot be put back by importing the export taken after it, since the file does not hold what the command did. Use them on a book whose earlier export nobody kept.
...
The importer looks up the existing bank transaction by `txn_guid:`, finds the AR-side split whose guid `txn_split_guid:` states, and attaches it to the invoice's posted lot in-place — no new transaction is created and all original bank metadata is preserved. The block's `account:` (or `bank_account:`) has to be an account that transaction has a split on. Where the book holds the transaction and it has no split there, the import is refused before anything changes, stating the account and the transaction: the file and the book disagree about where the money went, and nothing an import does makes them agree.
...
- **overpayment**, with `txn_guid:` alone: pay 150.00 against a bill owing 100.00 and state the rest with `prepayment: 50`. That spelling divides the split, exactly as it does when the split sits on a bank account. Stating `txn_split_guid:` as well refuses an overpayment instead, because a single split's guid divides nothing;
...
A ledger states transactions in the order the book keeps them, which is GnuCash's own and the one every register shows: the posted date, then `num` where the transactions carry one, then when each was entered, then the description, then the guid. Two transactions of one day are ordered by the `num` you write on them, then by the moment each was entered.

With one exception, which is about the file rather than the book: **a transaction holding a cost basis is stated above any transaction that draws on it.** `cost_basis_split_guid:` is resolved as each block is applied, so a sale whose cost basis the file states further down is refused with "matches no split in the book", and one stating that cost basis by its position is refused before any of the file is applied. Two transactions of one day entered at the same second, one a deposit and one a fee drawn on it, are in fee-first order by description, and written in that order the ledger would not rebuild the book it came from.
...
`export-transaction --guid` and the undo copy `delete-transactions -o` writes follow the same rule, whichever order the guids are passed in.
...
`set-invoice-style` writes the two boxes GnuCash's report options show as **Display → Extra Notes** and **Layout → CSS**, without opening GnuCash:
...
4. **A report of your own.** `--report-file` loads a `.scm` before the report is looked up, so a file of yours calling `gnc:define-report` is registered by the time `--report` names it:
   ...
   **If you copy one of GnuCash's, set a `report-guid` of its own in your report** — `uuidgen` prints one, in any case and with or without its dashes. Keeping the guid you copied is the mistake this warns about twice: registered under a guid GnuCash already has, your report is refused as a duplicate and never registers at all, so `--report` cannot find it by name; registered under that guid in another case, both answer to every spelling of it and `--report <that guid>` is refused as ambiguous. Use a name of its own for it too, unless you mean to select it by guid from then on: GnuCash accepts a report that reuses another's *name* — the guids differ, so both register — but two of them then answer to it, and `--report "<that name>"` is refused as ambiguous, for the report you copied as well as yours.
   ...
   **What does reach a report loaded with `--report-file` is what the book carries**: a footer or a stylesheet set with `set-invoice-style` goes on whatever report draws the page, a report from a `.scm` file included. Both settings come from the book — `set-invoice-style` puts each one there, for every invoice the book prints — so a report declaring an option under one of the names the shipped reports use has the book's value written into it. Those names, every one of which is written:
   ...
   Five pairs, because the invoice family and the Tax Invoice family disagree about both boxes and GnuCash 3.8 spells one of them lowercase. Put an option of your own under a section and a name outside that table — a `Display / CSS` is untouched, for instance — or take both settings off the book with `--clear-note` and `--clear-css` (`--note ""` is a setting rather than an absence; `set-invoice-style book.gnucash --show` prints what the book holds).
...
**The `action:` field** is optional, on an invoice entry and on a bill entry alike — one entry field, shown in the Action column of both windows. Omitting the line is equivalent to `action: ""` — the entry's action is set to empty. To keep a non-empty action such as `Hours` across re-imports, name it every time; an entry block is the full source of truth for its line, not a partial patch, for the reason stated below the table.
...
**A transaction with no new changes is up to date, and is not edited.** Most of an exported book comes back as it went out: only the transactions you changed differ. A block that reads line for line as `export` writes that transaction states it as the book already holds it, so importing it would change nothing. It is left as it is, and the summary counts it apart from the updated ones:

```
  Updated:      10
  Up to date:   4990 (no new changes, not edited)
```

A file with no new changes saves nothing. A block that states the same figures written another way, such as `5.0` for `5.00`, is edited as any changed block is, and counted as updated.

Since an up-to-date transaction is not edited, nothing is read from it again either: a figure an earlier release stored on it, such as what a split brought in past zero, stays as that release stored it. To have a book's figures read again, bring it forward through its export into a new book, as the section "Foreign currency" above describes for a book an earlier release wrote.
...
**An edit to a transaction touching a cost basis is read as the transaction would be read if it were new, and is accepted when the new version is a correct transaction and the book is correct afterwards.** Where it changes a split's account or amount, or anything a cost basis rests on, what the old version drew is put back and the new version is checked and applied as an import of it would be: the cost bases its shape establishes, what its splits draw, every refusal a new transaction meets, and `$residual$` for what it realizes. A cost basis the old shape needed and the new one does not is simply not established. A new version that does not balance is refused, stating what its values come to. A memo, a description or an unchanged figure is edited in place as before.

The book is correct afterwards when every other transaction drawing on the transaction's cost bases still draws on one the new version establishes, at the figures it drew at. So a US dollar arrival stated in US dollars, booked against the director's account for 3,791.14 CAD, can have that Canadian dollar split moved to income whatever else draws on it: the cost basis is priced the same. An edit that re-prices a cost basis another transaction draws on, moves it to another account or dates it later, or stops establishing it, is refused, because that transaction drew at the old figures: the refusal says which splits the book and the file hold differently, what that would change, and states one `delete-transactions --by-guid` command deleting the transactions drawing on it first and then the transaction, which is the order a delete accepts. A refused edit leaves the transaction exactly as it was. [Q-048](docs/issues/Q-048-accept-an-edit-that-leaves-every-cost-basis-as-it-was.md) and [Q-051](docs/issues/Q-051-let-a-statement-line-on-a-holding-account-be-edited-into-the-invoice-or-bill-it-settles.md) work through the cases.

**A statement line imported before its owner knows what it is can be booked later by editing it.** A US dollar deposit imported against a holding account such as "Due from director", its bank fee in the same transaction drawing on it by position, is edited, once the owner knows the customer paid an invoice, into the receivable settling that invoice, the bank charge, and the fee drawing on the invoice's cost basis, with the invoice's `payment:` block stating the transaction and the receivable's split. Dollars collected off an invoice whose posting is a cost basis are not dollars bought, so the deposit establishes no cost basis once it is booked so; the fee's check counts the invoice collected by the split this file's block applies. A withdrawal booked as a bill's payment is edited the same way. Nothing else may have drawn on the deposit's cost basis in between.

A balance an update restates exactly as the book holds it states nothing: an export restates every one, and only a figure the file changes is read as a balance stated net of the file's own disposals.
...
A transaction with a guid no transaction in the book has is a new transaction, even when its date, accounts, amounts and description are the same as a transaction in the book.

**The order of a day's transactions is the order the import creates them in.** GnuCash orders the transactions of one day by `num`, then by the moment each was entered, to the second, and an account's running balance follows that order. To keep the order of the file, an import enters the transactions it creates one second apart, the last one at the moment the import started. The transactions of an import started a second or more after another come after that import's. Transactions imported by separate runs within the same second are entered at the same second; write a `num` on each to set their order. A `num` made of digits sorts the same way on every supported GnuCash; one starting with a letter does not, because GnuCash 3.4 to 4.4 compare only the number at its start.
...
**A currency the price database cannot price is valued at nothing, and the totals come to that much less.** The line states what the account holds and a `value:` of `0.00`, with no `share_price:` at all — a price of zero would read as "a US dollar is worth nothing", which is a claim the book does not make — and that zero is what the `Income`, `Expenses` and `net_income:` figures add up. So a period holding 500.00 USD of income the book has no USD price for reports a net income 500.00 USD lower than the truth, with nothing on the page marking the gap. Every figure here is GnuCash's own and this is what GnuCash's own report does, which is the point of printing it this way; it is also why the price matters before the statement does. Record it in the book with a `price` block (see [Prices](#prices)), or pass it for the one run with `--fx-rates` (see [Rates and prices for one run](#rates-and-prices-for-one-run---fx-rates---prices-and---price-source)), and the same command reports the whole figure.
...
2026-12-31 balance-sheet
	...
	# into CAD. This page converts it at the rate of its own date,
	...
	retained_earnings: 12726.00 CAD
	unrealized_gains_assets_fx: 605.60 CAD
	...
	total_unrealized_gains: 2256.80 CAD # unrealized_gains_fx + unrealized_gains_other
	gnucash_balancing_amount: 2256.80 CAD
	total_equity: 34982.80 CAD
	total_liabilities_and_equity: 38532.80 CAD
```

The page explains itself: the comment lines are written by the report, and `#`, `;` and `;;` all open a comment, so anything reading the block passes over them. Its gains are shown as single figures — the shape `--no-itemize` prints. By default each of them carries the items it is made of, set out further down. This book borrowed its US dollars in a transaction stated wholly in US dollars, so the debt has no Canadian cost, its cost bases cannot account for what the accounts hold, and that currency keeps GnuCash's own revaluation: the items list it under `measured_from: gnucash_revaluation` with the cost and the worth that revaluation used, and they come to the 505.60 of the 605.60 the key states. [Q-044](docs/issues/Q-044-state-a-realized-gain-with-no-took-the-residual-key-and-say-what-the-balancing-amount-is.md) says what that measurement can and cannot answer for.

**This book also keeps `Expenses:Interest` in US dollars, which is why the page opens with a warning**, and it is the only page in this file that carries one: a book keeping its income and expense accounts in its own currency has none of those lines. The 100.00 USD of interest was paid with dollars costing 1.30, so it cost 130.00 CAD, and the page converts the account's balance at the sheet's own 1.42 and states 142.00 instead.

Every account line is the balance GnuCash's Balance Sheet report states for it, and every section total is the total that report states, but two: `total_equity` and `total_liabilities_and_equity` carry the unrealized gain measured from the book's own cost bases, where GnuCash's own page carries the amount it calculates to balance itself. The gain figures described below are this tool's own amounts rather than GnuCash's, measured from the cost bases the book records, and GnuCash's amount is stated beside them as `gnucash_balancing_amount` so the two can be read against each other:
...
- **A figure no account holds is a key**, and states its currency: `retained_earnings`, `trading_gains`, the gain figures below, `total_liabilities_and_equity`, `net_income`. These carry no quotes — quotes are for text, and a money figure in them reads as a string that happens to look like money. **Under an account line** `value:` and `share_price:` are quoted, because there they are split keys and this format writes a split's values quoted; **inside a gain's items** the same two words are figures of the block's own and carry no quotes, like every other figure there. What is always quoted is text: an account path in `account:` or `name:`, and a mnemonic in `commodity.mnemonic:`. `retained_earnings` and `trading_gains` are written whenever the book holds the money behind them, and left off when it holds none of it at all, each being a thing the book either has or has not. Holding the money and valuing it are separate questions: a book earning only in a currency it holds no price for states `retained_earnings: 0.00 CAD`, because the earnings are there and GnuCash values what it cannot price at nothing. `net_income` and `total_liabilities_and_equity` are always written, a statement having a bottom line whatever it comes to.
...
- **`gnucash_balancing_amount` is an amount GnuCash adds to equity as the unrealized gain, and it is not always the unrealized gain** — which is why it is stated here under a name saying whose figure it is rather than under a name of its own. It is carried across as GnuCash states it, sign included, so a reader with GnuCash's page beside them can find the same number, and nothing adds it in. GnuCash takes the summed values of the splits in a holding's own accounts from what that holding is worth at the sheet's date; that is the unrealized gain only where every one of those splits carries a figure in the book's own currency. Where foreign currency arrived carrying no figure in the book's own currency and was later spent, GnuCash's amount comes out as the negative of the realized gain — the realized gain is in the income and expense accounts, so `retained_earnings` carries it already, and adding an amount of the opposite sign cancels it rather than leaving it — which is why GnuCash's own page can state more liabilities and equity than it has assets while this one balances. Currency bought with the book's own money and sold again does not do this: both splits carry the book's own figures, so GnuCash's subtraction cancels exactly and states `0.00`. Beneath the key the page states the subtraction GnuCash made rather than a verdict on it, commodity by commodity — the commodities its own figure is summed from, read out of the collector GnuCash sums rather than grouped by anything this page decided, with the splits valued in each and the accounts holding it beside the amount — so which case a book is in is worked out from its own figures. [Q-044](docs/issues/Q-044-state-a-realized-gain-with-no-took-the-residual-key-and-say-what-the-balancing-amount-is.md) has the measurements.
- **`realized_gains_not_recorded` is a realized gain the book's own splits leave out**, and it reaches `total_equity`. Each disposal is valued at what the currency it draws cost, rounded to the cent, and the residual split beside it states the gain; several such values can add up to a cent or so more or less than what the currency cost. The import values the disposal that takes the last of a cost basis at what is left of its cost, so a book it writes records the whole gain. A book an earlier import wrote, or one edited in GnuCash, may not: 2,720.00 USD that cost 3,815.89 CAD, spent at 1.01, 12.06 and 3,802.81, records a realized loss of 44.60 where it is 44.61, and the page states `realized_gains_not_recorded: -0.01`. It is stated only where there is one, and itemized by cost basis. A `--strategy update` block restating that last disposal at what is left, 3,802.82, with its residual split, corrects the book in place. A book rebuilt from its own export is rebuilt as it was: the export states each cost basis's balance, so which disposal came last is not known from it, and each is taken at its own share or at what is left.

  A disposal in a transaction stated in another currency is stated here too, whatever its size. Its Canadian dollar split is what the currency fetched that day, and a transaction stated in US dollars has no split that can record the difference from what the dollars cost: 2,710.68 USD sent at 3,758.36 CAD and 8.60 at 11.92, dollars that cost 3,778.15 and 11.99, state `realized_gains_not_recorded: -19.86`. That is read where the disposal is the only split of its transaction drawing on a cost basis and the Canadian dollar splits value the whole of what it sold; otherwise no figure of the book's says what it fetched, and nothing is stated for it. A Canadian dollar split of no value beside it, one recording the gain or loss in an income or expense account, counts as part of what it fetched, so that gain is not stated again. To record the gain instead, write the transaction in CAD, the dollars at what their cost basis cost, and put the difference on a `$residual$` split.

  The Canadian dollar figure is taken as written, since only the file knows what the dollars fetched that day. A figure typed wrong is imported as any other is, and the page states the difference as a gain not recorded; the page balances, so no check reports it. That gain is the place to look.
...
- **A book that uses trading accounts** (File → Properties → Accounts → "Use Trading Accounts" in GnuCash) keeps those gains in its Trading accounts, and the page states them as `trading_gains:` with none of the other gain figures beside it. The two are the same money by different bookkeeping, and a book has one or the other.
- **A currency or a security the price database has no price for is valued at nothing** — the line states what the account holds and a `value:` of `0.00`, and no `share_price:`. Record the price in the book with a `price` block (see [Prices](#prices)), or pass it for one run with `--fx-rates` or `--prices`.
- **Each gain figure shows its working**, as keys nested under it, because it is the one amount on the page a reader cannot check: an account line can be checked against the book and a section total by adding the lines above it, but a gain is measured against costs that appear on no line at all. **These are keys, not comment lines** — `cost_basis:`, `split:`, `account:`, `value:` — so a program reading the block sees them, where it passes over the `#` comments the page opens with. Each states its own arithmetic in a trailing `#` comment, so a reader can follow the figure down to the splits and cost bases it came from. **The items are a reference, never the computation** — the page states the figure it computed and then shows what stands behind it, which is why one column of the listing is not a column to add up (below):
...
```
  ...
  That is a book that earned US dollars three times and spent every one of them out in four payments: four disposals, each stating the date, the account the difference was booked to, and what it came to. A currency still held is itemised the other way — by the cost bases it is measured against and the accounts holding it:
	...
	unrealized_gains_assets_fx:
		commodities:
			commodity:
				...
				type: asset
				share_price: 0.2 # current price of commodity in balance sheet currency
				cost_bases:
					cost_basis:
						split_guid: 133a45a926f642d6888413c3693d7b28
						account: "Assets:HKD Bank"
						cost_basis_balance: 5500.00
						cost_share_price: 2/11 # HKD in CAD, on the day it was bought
						cost_rate: 1 # CAD per CAD, on that same day
						cost_share_price_in_base: 2/11 # cost_share_price * cost_rate
						cost_value: 1000.00 # cost_basis_balance * cost_share_price_in_base
...
```
  ...
  **A cost basis states what a unit cost in three lines, not one**, because the figure every gain is measured against cannot be checked from the figure alone. `cost_share_price` is the price the trade happened at, in the currency the transaction was stated in; `cost_rate` is the price of that currency in the report's currency on the same day; and `cost_share_price_in_base` is the two multiplied out, which is what `cost_value` is reached with. A currency bought with the book's own money makes the first and the third the same number and the rate 1, as the Hong Kong dollars above do. A share bought in a transaction written wholly in US dollars is where they come apart: 99.00 USD a share at 1.30 is `cost_share_price_in_base: 128.7`.

  `cost_basis_balance` is how much of that currency the book's cost bases still account for and `balance_value` is what the accounts actually hold, so the two can be read side by side. Where they disagree the currency keeps GnuCash's own revaluation, which is the case the US dollars above are in: that group states no cost basis and no account, and states instead the cost and the worth GnuCash's own subtraction used, under `measured_from: gnucash_revaluation`. Both kinds of group are added into the same totals, so the items still come to the key above them however each was measured. A key with nothing behind it says so in place of the list: `commodities: # there is no cost basis on the asset side`, `splits: # there is no split`, `securities: # there is no security or fund`, which is an answer a reader can check where silence is not. That first note says which side it speaks for, because the owed side is measured from cost bases of its own and has its own key — a book that borrowed a currency has a cost basis for it while its asset side has none. **`--no-itemize` turns the items off**, leaving each key as the single figure it was.

  **Every entry is listed, however many there are**, because a list with entries missing is one a reader cannot add up. A book of 2,500 foreign purchases draws a page of 40,154 lines and 1.4 MB that way, nearly all of it those four keys, so **`--max-items N` shortens each list to `N` entries** where that is too much to read. `-1`, the default, is no cap; `0` lists none; every other number means itself. A shortened list says on its own line what it is not showing — `cost_bases: # 2500 cost bases, 5 listed and the rest under not_listed; --max-items -1 for all` — and **ends with a `not_listed:` entry stating how many were left out and what they come to**, so the entries above still add up to the total beneath them, which is what that total's own comment says of it. One list carries the count and no figures: the `splits:` beneath a security's account, where each line reads one split's units beside what they cost in that transaction's own currency, and several transactions' currencies add to nothing — the figure those lines stand behind is `cost_value`, which is converted rather than summed from them. Grouping the entries instead was measured and does not shorten a real book at all: on one whose purchases differ from each other, 2,500 cost bases fall into 2,500 distinct account-and-price groups.

  **One column of a cost basis listing is not a column to add up: `value`, and the gain worked out from it.** A holding is converted once, at the commodity's price, and that single conversion is the figure the commodity's own `value:` states and the one the key is built from. The `value:` on each cost basis line converts that basis alone, so it is rounded to the commodity's places one basis at a time, and several of those need not come to the same figure as one conversion of the whole holding. Measured on `tests/fixtures/a_cad_book_with_two_cost_bases_that_round_apart.txt`: two 1.00 USD bases priced at 1.3833 each print `value: 1.38` and a gain of `0.08`, while the commodity prints `value: 2.77` and a gain of `0.17`. Converting basis by basis and adding is what produced a sheet stating 100.17 of assets against 100.16 of equity, so the page does not do it — the per-basis line says what that basis is worth on its own, for a reader checking one of them, and `cost_basis_balance` and `cost_value` are the columns that do add exactly.

  **A figure here is padded to its own commodity's places**, which GnuCash decides: `250000` for a yen balance beside `2500.00` for what those yen cost, because a yen divides into 1 and a Canadian dollar into 100. GnuCash's own page writes the same figures as `JP¥250,000` and `C$2,500.00` — this page drops the symbol and the thousands separators, because a figure it writes has to be one `import` can read back.

`--output-format html` and `pdf` work as they do for `income-statement`, with one difference worth knowing: **those two formats are GnuCash's own Balance Sheet, not this one.** The gain figures above belong to the plaintext page, and GnuCash's page states its single `Unrealized Gains` figure instead — the figure this section exists to replace. On a book whose foreign currency arrived in its own currency, that figure is the one that comes to nothing where the gain is real, so an HTML or PDF sheet of such a book can state a gain on money the book no longer holds, and need not balance. Draw the plaintext page for the figures described here.

### The currency a report is in

GnuCash keeps no currency for a book, and its reports come out in US dollars unless they are told otherwise. `balance-sheet`, `income-statement` and `report` put their reports in the first of these that states a currency:
...
When none of them states a currency — top-level accounts in two currencies, or no accounts at all — the report is refused, and the refusal says how to state it. A currency GnuCash does not know is refused too.
...
- A security's price with no currency is in the currency of the book's own prices of that security, or, where the book has none, the currency of the transactions that hold it. Where that is more than one currency, the file has to state it.
...
The plaintext page is printed by two customized GnuCash reports in `infrastructure/gnucash/reports/balance-sheet-and-income-statement-as-text.scm`, which the package ships, "Balance Sheet (plain text)" and "Income Statement (plain text)". Each is written from GnuCash's `balance-sheet.scm` or `income-statement.scm` and changed to output plain text instead of HTML. It is registered with `gnc:define-report` as GnuCash's reports are, takes GnuCash's own report options, makes the same GnuCash calls for every figure, and GnuCash runs it. That file decides how the page is laid out; what the figures are is decided by GnuCash.
...
You list the statements explicitly (`income-statement`, `balance-sheet`) — no hidden bundle. The income statement covers the fiscal period; the balance sheet is as of the period end (override with `--as-of`). `--currency`, `--fx-rates`, `--prices`, `--price-source` and `--output` apply to both. `--no-itemize`, `--max-items` and `--fx-gain-account` apply to the balance sheet, which is the only one of the two with gain figures to show the working of — passed with `income-statement` alone they are accepted and change nothing. It is read-only.
...
### Check whether a book is consistent and balanced: `--verify-integrity`

`validate` reads the ledger. `--verify-integrity` reads the statements, which is a different question: whether the figures the book produces hold together.

```bash
# On its own, on a book nobody is changing
gnucash-plaintext --verify-integrity mybook.gnucash

# On its own, to a date — its options come after the book
gnucash-plaintext --verify-integrity mybook.gnucash --as-of 2028-06-30

# After a command that writes, on the book it saved
gnucash-plaintext --verify-integrity import mybook.gnucash today.txt
```

Four things, each an exact comparison:

- **The balance sheet balances.** `total_assets` equals `total_liabilities_and_equity`. Every other figure on that page is GnuCash's own and only `total_unrealized_gains` is measured here, so a page that does not balance is a page whose unrealized figure contradicts everything around it.
- **The income statement accounts for the retained earnings.** Drawn over the book's whole life, `net_income` is the sheet's `retained_earnings` together with what closing entries moved into equity: the income statement leaves out every entry carrying GnuCash's closing flag, which is what `close-books` writes, and the sheet's `retained_earnings` is only the profit not closed yet. Where the two disagree, the income statement left out a transaction that is no closing entry — one whose description matches its Closing Entries pattern. A book holding an income or expense account in another currency has this left unchecked, and says so: neither figure can be read for it.
- **Every income and expense account is kept in the book's own currency**, which is the account the statements warn about above.
- **No cost basis holds more of a currency than the accounts do**, per currency and side. Holding less is an ordinary state and is not reported: currency that arrived in a transaction stating no figure in the book's own currency has no cost, so no cost basis was opened for it. Holding more is the book offering currency it does not have. `fx-balances --verify-costs` runs as well, so one pass reports everything.

It prints what it looked at, what it could not look at and why, and then either that the book is consistent or each thing that is wrong:

```
Integrity check
  as of 2028-12-31, in CAD

  checked: every income and expense account is kept in the book's own currency
  checked: the balance sheet as of 2028-12-31 balances
  checked: the income statement's net income is the sheet's retained earnings and what closing entries moved into equity
  ...
  The book is consistent and balanced.
```

On its own it exits 1 where something does not hold, so a daily run says so without anyone reading the page. After a command that writes, the book is reopened from disk — the book the next command will read, rather than a session a failed save never wrote.

`--as-of` draws the statements to a date, and without it they are drawn to the date of the book's last transaction, so every transaction is on the page. `--currency` states the currency they are in, and without it the currency the book is kept in. With a currency that is not the book's own, the balance sheet and the income statement are listed as not checked: both convert every income and expense account at the rate of the page's own date, so the sheet cannot balance on the book's own figures, and that is the currency the reader chose rather than a fault in the book.

**It is expensive**, which is why it is a flag rather than something every command does: each statement is a GnuCash report drawn through Guile, and the cost-basis pass walks every split in the book.
...
The commit-msg hook then refuses a hard-wrapped message or an attribution line, and runs an independent AI review of the staged diff, with the commit message. It runs there because git runs the pre-commit hook before it reads the message; a review started from the pre-commit hook was handed none. For a `git commit` it runs only once the pre-commit checks have passed. A merge commit, which runs the commit-msg hook and not the pre-commit one, is not reviewed, and neither is a commit with nothing staged. `SKIP_AI_REVIEW=1` skips the review and nothing else.
