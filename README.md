...
of it, giving the guid of the cost basis the USD came out of — `fx-balances`
lists them.
```
...
**A disposal whose cost basis is not decided yet gives `cost_basis_split_guid: $pending$`** (quoted or not: no split's guid is `$pending$`), on the split that gives the holding up; on any other split it is refused. It is imported drawing on no cost basis. The book's cost bases then offer more than its accounts hold, by what the pending disposals took, and `fx-balances` lists each one with what they add up to:

```
3 disposal(s) pending their cost basis, drawing on none until an edit gives it: 2,720.00 USD
...
Give a pending disposal its cost basis later by editing the same split, `cost_basis_split_guid: "<guid>"`, and the import draws that cost basis down then, with every check a new transaction meets. The export writes `$pending$` back unquoted, so a book holding one is rebuilt from its export.
...
valued at what its cost basis cost, and give the difference to a `$residual$`
split.
```
...
- give `cost_basis_split_guid:` on the split that spent the currency — `fx-balances` on the book the import made lists the cost bases to choose from;
- state that split at what its cost basis cost, and give the difference to a `$residual$` split, as every disposal is written;
...
An earlier release also stored a `cost_basis_balance` on the receiving split of a transfer between two accounts of one currency stated in Canadian dollars, reading it as dollars arriving. They arrived nowhere, so that split is no cost basis, and `fx-balances --verify-costs` reports the figure and says so. Take the `cost_basis_balance:` off that split in the export, and give a spend out of that account the guid of the cost basis the dollars came from.
...
The way to spend a cost basis balance is the way every other foreign disposal is written — an ordinary transaction whose bank split gives its cost basis's guid, attached to the invoice by `txn_guid:` / `txn_split_guid:`:
...
- **Realized gains are what the transactions record.** A realized gain needs no cost basis: a `$residual$` split beside a split in another currency or a security, whichever way that split moves, is the difference the file states, and `realized_gains_fx` and `realized_gains_other` are read from those splits as in any book. The gain itself is the balance of the income or expense account the file posted it to, and it is in `retained_earnings`; the two keys list it by kind where the transaction gives up one kind. A `$residual$` beside both a currency and a security is in that account and in `retained_earnings`, and on neither key, since one split cannot say how much of it is which. So a bank charge beside a split in another currency gets a line with its amount, not the `$residual$`: taken as the `$residual$`, it is stated as a realized difference.
- **Unrealized gains are measured from GnuCash's own revaluation.** The balance sheet writes `measured_from: gnucash_revaluation # this book keeps no cost bases` under each currency and security. The figures are then GnuCash's, not what the book's own costs would give.
...
**Such a book's export rebuilds it** in a new book, imported with `--include-business-objects`: that is what imports the `company` block, and it is applied before the transactions, so the spends that give no cost basis meet a book that keeps none. Without the flag they are refused.

**Turning cost bases on later** is bringing the book forward through its export, as for a book an earlier release wrote: remove the `cost_bases:` line, import the export into a new book, and give each disposal the cost basis it draws on.
...
- **What depends on another block is asked of the book the file leaves.** A split drawing on a cost basis draws on what the file's version of its transaction gives, so a deposit whose fee the file moves onto the invoice's cost basis may stop establishing its own. A split settling an invoice or a bill, given another account, is taken off that record, as `unapply-payment` takes it off, where the file states the record and none of its `payment:` blocks gives that transaction. A record the file does not state, one it still pays with that transaction, and an owner's credit are not taken off, and the edit is refused as it is without the flag.
...
**What a converting settlement drew out of a cost basis comes back; the realized difference does not.** Settling a 100.00 USD invoice booked at 1.40 into a CAD bank at 1.37 draws 100.00 USD out of the receivable's cost basis — that USD was spent — and realizes 3.00 CAD. Taking the payment off gives the cost basis back, because the currency was not sold, it went back to being owed; without that the record was unsettleable, since re-applying the money hit "that USD has already been sold against it".
...
2. Once you know what a line paid, export the book, keep the export, and book the line: give its split the receivable's or payable's account, and give the invoice's or bill's `payment:` block the line's `txn_guid:` and `txn_split_guid:`.
...
```
  ...
  That is a book that earned US dollars three times and spent every one of them out in four payments: four disposals, each giving the date, the account the difference was booked to, and what it came to. A currency still held is itemised the other way — by the cost bases it is measured against and the accounts holding it:
