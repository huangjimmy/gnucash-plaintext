...
Under it **one** cost-basis check is **deferred to commit time**, as a database defers a constraint to the end of a transaction: the refusal to edit a transaction whose cost basis another transaction draws on. It has to be, because a repair of such a cost basis runs through states that refusal stops in either order: each block is refused while the others still read as they were. Every other check still runs wherever it ran before, which on a block that *creates* a transaction means as the block is applied, and on a block that *edits* one nothing else rests on means as it is applied too, the edit being read as a new transaction would be (below). The finished book is asked the same questions afterwards.
