...
Under it **one** cost-basis check is **deferred to commit time**, as a database defers a constraint to the end of a transaction: the refusal to edit a transaction a cost basis rests on. It has to be, because a repair runs through states that refusal stops in either order — clear the cost basis first and the disposal draws on a split that is no cost basis, re-point the disposal first and it is refused before its figures are read. Every other check still runs wherever it ran before, which on a block that *creates* a transaction means as the block is applied. A block that *edits* one draws no cost basis down at all, so those checks never reached it and the deferred refusal was standing in for them — which is why the finished book is asked the same questions.
...
**An edit to a transaction holding a cost basis stands wherever the cost bases are what they were.** Every cost basis the transaction opens keeps its account, what it brought in and what it cost, and every disposal in it draws on the same cost basis, by as much, at the same value. So a US dollar arrival stated in US dollars, booked against the director's account for 3,791.14 CAD, can have that Canadian dollar split moved to income, or divided between income and the director's account: the cost basis is priced at the Canadian dollar splits' amounts added up over their values added up, and neither the accounts nor the division moves that. An edit that re-prices a cost basis, moves its split to another account, changes what a disposal draws, or moves the date of either is refused: a cost basis is dated when its currency arrived and a disposal when it drew one down. The refusal says which splits the book and the file hold differently, what that would change and why it cannot be done in place, and where other transactions draw on the cost basis it lists them and gives one `delete-transactions --by-guid` command deleting them first and then the transaction, which is the order a delete accepts. [Q-048](docs/issues/Q-048-accept-an-edit-that-leaves-every-cost-basis-as-it-was.md) works through the cases.
